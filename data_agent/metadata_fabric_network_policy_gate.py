...
DEFAULT_PROFILE_PATH = (
    REPO_ROOT / "config/metadata-fabric-network-policy.production.yaml"
)
...
OCI_REFERENCE_PATTERN = re.compile(
    r"^oci://[A-Za-z0-9.-]+(?::\d+)?/[^\s@]+@sha256:[0-9a-f]{64}$"
)
...
def _traffic_errors(traffic: Mapping[str, Any]) -> list[str]:
    errors = _inventory_errors(
        traffic, {"decision_status", "entries"}, "traffic matrix"
    )
...
def _profile_errors(profile: Mapping[str, Any]) -> list[str]:
    ...
    if kubernetes_version is not None and not SEMVER_PATTERN.fullmatch(
        str(kubernetes_version)
    ):
    ...
    if (
        cni.get("provider") is not None
        and cni.get("provider") not in ALLOWED_PRODUCTION_CNIS
    ):
        errors.append("production CNI provider is invalid")
    if cni.get("version") is not None and not SEMVER_PATTERN.fullmatch(
        str(cni.get("version"))
    ):
    ...
    if namespace_admission is not None and not _reference(
        namespace_admission, "policy"
    ):
    ...
    for key in ("runbook", "rollback_runbook"):
        ...
        errors.extend(
            _inventory_errors(runbook, {"uri", "version"}, key.replace("_", " "))
        )
...
def _profile_blockers(profile: Mapping[str, Any]) -> list[str]:
    ...
    if not OCI_REFERENCE_PATTERN.fullmatch(
        str(policy.get("policy_bundle_reference") or "")
    ):
    ...
    for name in sorted(EXPECTED_WORKLOADS):
        ...
        service_account = item.get("service_account")
        if not isinstance(service_account, str) or not DNS_LABEL_PATTERN.fullmatch(
            service_account
        ):
    ...
    if not _reference(
        tenancy.get("namespace_label_admission_policy_reference"), "policy"
    ):
    ...
    *,
    ...
    try:
        ...
        stable = {
            ...
            "profile_valid": False,
            "profile_errors": [
                f"NetworkPolicy profile is invalid: {type(exc).__name__}"
            ],
            "profile_blockers": [],
    ...
    workload_fingerprint = recovery._canonical_sha256(
        _mapping(profile.get("workloads"))
    )
    traffic_fingerprint = recovery._canonical_sha256(
        _mapping(profile.get("traffic_matrix"))
    )
...
def verify_report_integrity(report: Mapping[str, Any]) -> list[str]:
    ...
    if recovery._sensitive_paths(report):
        errors.append(
            "NetworkPolicy readiness report contains credential-bearing fields"
        )
    errors.extend(
        _inventory_errors(
            report,
    ...
    if (
        ...
        errors.append(
            "NetworkPolicy readiness report schema or environment does not match"
        )
    stable = {
    ...
    expected_profile_valid = (
        isinstance(report.get("profile_errors"), list)
        and report.get("profile_errors") == []
    )
