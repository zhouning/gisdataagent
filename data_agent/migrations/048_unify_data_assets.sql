...
-- Only run if source is a real table (not already a view)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='agent_data_catalog') THEN
...
-- Step 3: Create backward-compatible view (idempotent)
