-- 199: Admit a real DuckDB provider receipt to the Blueprint success authority.
--
-- Migration 197 remains the immutable history. This replacement preserves its
-- deterministic-local branch and adds a plan/definition/output-bound provider
-- branch; both converge through the same PlatformRun transition authority.
...
BEGIN
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'DuckDB Blueprint success observation was not found'
            USING ERRCODE = '23514';
    END IF;

    IF NOT (
        (
            v_observation.evidence->>'schema' =
                'gda.blueprint_test_executor_receipt.v1'
            AND v_observation.evidence->>'executor_mode' = 'deterministic_local'
        )
        OR
        (
            v_observation.evidence->>'schema' =
                'gda.data_product_blueprint_duckdb_provider_receipt.v1'
            AND v_observation.evidence->>'executor_mode' = 'duckdb_provider'
            AND v_observation.evidence->>'tenant_id' = p_tenant_id
            AND v_observation.evidence->>'run_id' = p_run_id::text
            AND v_observation.evidence->>'definition_version_id' =
                v_run.definition_version_id::text
            AND v_observation.evidence->>'output_artifact_id' =
                v_output_artifact_id::text
            AND v_observation.evidence->>'quality_result_id' =
                v_quality_result_id::text
            AND v_observation.evidence->>'output_content_sha256' =
                v_output.content_sha256
            AND COALESCE(v_observation.evidence->>'receipt_sha256', '')
                ~ '^[0-9a-f]{64}$'
            AND EXISTS (
                SELECT 1
                FROM gda_control.artifact AS plan
                JOIN gda_control.platform_definition_version AS definition
                  ON definition.tenant_id = v_run.tenant_id
                 AND definition.definition_version_id = v_run.definition_version_id
                WHERE plan.tenant_id = p_tenant_id
                  AND plan.artifact_id =
                      (v_observation.evidence->>'execution_plan_artifact_id')::uuid
                  AND plan.run_id = p_run_id
                  AND plan.artifact_role = 'execution_plan'
                  AND plan.manifest->>'plan_sha256' =
                      v_observation.evidence->>'execution_plan_sha256'
                  AND definition.definition_sha256 =
                      v_observation.evidence->>'definition_sha256'
            )
        )
    ) THEN
        RAISE EXCEPTION 'Blueprint provider observation does not satisfy success policy'
            USING ERRCODE = '23514';
    END IF;
