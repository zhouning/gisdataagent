-- Metric query PlatformRun admission and provider execution evidence.
    ...
    IF gda_control.current_tenant() IS NULL
       OR p_tenant_id IS DISTINCT FROM gda_control.current_tenant() THEN
        RAISE EXCEPTION 'metric query tenant context is missing or mismatched'
            USING ERRCODE = '42501';
    END IF;
    IF p_actor_subject !~ '^workload:[^[:space:]]{1,128}$'
...
BEGIN
    ...
    IF p_actor_subject !~ '^workload:[^[:space:]]{1,128}$'
       ...
       OR p_rows_returned < 0 OR p_rows_scanned < 0 OR p_bytes_scanned < 0
