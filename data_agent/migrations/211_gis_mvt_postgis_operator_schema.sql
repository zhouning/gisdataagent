-- 211: Resolve the PostGIS overlap operator inside the governed MVT function.
--
-- Migration 210 qualified PostGIS functions while preserving the restricted
-- SECURITY DEFINER search_path.  The geometry overlap operator is also owned
-- by the PostGIS public schema and must use PostgreSQL's explicit OPERATOR
-- syntax; leaving it bare makes valid release-bound tile reads fail at runtime.
...
BEGIN
    ...
    tile_sql := format(
        ...
        'AND %I OPERATOR(public.&&) public.ST_MakeEnvelope($4, $5, $6, $7, %s) '
        'AND %I OPERATOR(public.&&) public.ST_Transform(public.ST_TileEnvelope($1, $2, $3), %s) '
