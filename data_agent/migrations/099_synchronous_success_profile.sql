-- 099: Evidence-gated success for the synchronous public DataOps profile.
--
-- Migration 096 remains checksum-frozen and continues to own the
-- DolphinScheduler verdict. This separate entry point accepts only the local
-- inline observation profile while preserving the same output, quality,
-- lineage, actor, tenant, state, fingerprint, and replay checks.

CREATE OR REPLACE FUNCTION gda_control.finalize_synchronous_platform_run_success(
    p_tenant_id TEXT,
    p_run_id UUID,
...
BEGIN
    ...
    IF v_run.orchestration_class <> 'synchronous' THEN
        RAISE EXCEPTION 'synchronous success profile requires synchronous Run'
            USING ERRCODE = '23514';
    END IF;
    ...
    IF v_run.status <> 'running' THEN
        RAISE EXCEPTION 'synchronous success finalization requires running Run'
            USING ERRCODE = '23514';
    END IF;
    ...
    WHERE tenant_id = p_tenant_id
      ...
      AND run_id = p_run_id
      AND framework_kind = 'legacy'
      AND lower(observed_state) = 'success'
      AND evidence->>'schema' = 'gda.public_dataops_attempt.v1'
      AND evidence->>'execution_mode' = 'local_inline';
    IF NOT FOUND THEN
        RAISE EXCEPTION 'local inline success observation was not found'
            USING ERRCODE = '23514';
    END IF;
...
REVOKE ALL ON FUNCTION gda_control.finalize_synchronous_platform_run_success(
    text, uuid, integer, text, text, jsonb
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION gda_control.finalize_synchronous_platform_run_success(
    text, uuid, integer, text, text, jsonb
