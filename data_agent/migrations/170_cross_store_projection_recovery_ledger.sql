-- 170: Durable append-only recovery events and snapshots for projection repairs.

CREATE TABLE IF NOT EXISTS gda_control.cross_store_projection_recovery_event_history (
    ...
    PRIMARY KEY (tenant_id, plan_sha256, event_index),
    CONSTRAINT uq_gda_projection_recovery_event_sha
        UNIQUE (tenant_id, event_sha256),
    CONSTRAINT ck_gda_projection_recovery_event_tenant
        CHECK (tenant_id ~ '^[a-z0-9][a-z0-9._-]{0,63}$'),
    CONSTRAINT ck_gda_projection_recovery_event_plan
        CHECK (plan_sha256 ~ '^[0-9a-f]{64}$'),
    CONSTRAINT ck_gda_projection_recovery_event_index
        CHECK (event_index >= 1),
    CONSTRAINT ck_gda_projection_recovery_event_sha
        CHECK (event_sha256 ~ '^[0-9a-f]{64}$'),
    CONSTRAINT ck_gda_projection_recovery_event_document
        CHECK (
...
CREATE TABLE IF NOT EXISTS gda_control.cross_store_projection_recovery_snapshot_history (
    tenant_id TEXT NOT NULL,
    plan_sha256 CHAR(64) NOT NULL,
    ...
    recorded_at TIMESTAMPTZ NOT NULL DEFAULT clock_timestamp(),
    PRIMARY KEY (tenant_id, plan_sha256, snapshot_version),
    CONSTRAINT uq_gda_projection_recovery_snapshot_sha
        UNIQUE (tenant_id, snapshot_sha256),
    CONSTRAINT ck_gda_projection_recovery_snapshot_tenant
        CHECK (tenant_id ~ '^[a-z0-9][a-z0-9._-]{0,63}$'),
    CONSTRAINT ck_gda_projection_recovery_snapshot_plan
        CHECK (plan_sha256 ~ '^[0-9a-f]{64}$'),
    CONSTRAINT ck_gda_projection_recovery_snapshot_version
        CHECK (snapshot_version >= 1),
    CONSTRAINT ck_gda_projection_recovery_snapshot_idempotency
        CHECK (plan_idempotency_key ~ '^[0-9a-f]{64}$'),
    CONSTRAINT ck_gda_projection_recovery_snapshot_projection
        CHECK (projection_id ~ '^[A-Za-z0-9][A-Za-z0-9._:-]{2,127}$'),
    CONSTRAINT ck_gda_projection_recovery_snapshot_engine
        CHECK (target_engine IN ('postgis', 'rdf', 'vector', 'object_store', 'lakehouse')),
    CONSTRAINT ck_gda_projection_recovery_snapshot_target
        CHECK (NULLIF(btrim(target_ref), '') IS NOT NULL AND octet_length(target_ref) <= 512),
    CONSTRAINT ck_gda_projection_recovery_snapshot_sha
        CHECK (snapshot_sha256 ~ '^[0-9a-f]{64}$'),
    CONSTRAINT ck_gda_projection_recovery_snapshot_document
        CHECK (
            jsonb_typeof(snapshot_document) = 'object'
            AND snapshot_document ->> 'tenant_id' = tenant_id
            AND snapshot_document ->> 'projection_id' = projection_id
            AND snapshot_document ->> 'target_engine' = target_engine
            AND snapshot_document ->> 'target_ref' = target_ref
            AND snapshot_document ->> 'plan_sha256' = plan_sha256
            AND snapshot_document ->> 'plan_idempotency_key' = plan_idempotency_key
            AND snapshot_document ->> 'snapshot_sha256' = snapshot_sha256
        )
);

CREATE INDEX IF NOT EXISTS idx_gda_projection_recovery_event_plan
    ON gda_control.cross_store_projection_recovery_event_history
        (tenant_id, plan_sha256, event_index);
CREATE INDEX IF NOT EXISTS idx_gda_projection_recovery_snapshot_current
    ON gda_control.cross_store_projection_recovery_snapshot_history
        (tenant_id, plan_sha256, snapshot_version DESC);

CREATE OR REPLACE VIEW gda_control.cross_store_projection_recovery_snapshot_current
WITH (security_invoker = true)
AS
SELECT DISTINCT ON (tenant_id, plan_sha256)
...
ORDER BY tenant_id, plan_sha256, snapshot_version DESC;

CREATE OR REPLACE FUNCTION gda_control.guard_cross_store_projection_recovery_insert()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    IF COALESCE(
        current_setting('gda.cross_store_projection_recovery_write_allowed', true),
        ''
    ) <> '1' THEN
        RAISE EXCEPTION 'use gda_control.record_cross_store_projection_recovery_snapshot()'
            USING ERRCODE = '55000';
    END IF;
...
DROP TRIGGER IF EXISTS trg_gda_projection_recovery_event_insert_guard
    ON gda_control.cross_store_projection_recovery_event_history;
CREATE TRIGGER trg_gda_projection_recovery_event_insert_guard
BEFORE INSERT ON gda_control.cross_store_projection_recovery_event_history
FOR EACH ROW EXECUTE FUNCTION gda_control.guard_cross_store_projection_recovery_insert();

DROP TRIGGER IF EXISTS trg_gda_projection_recovery_snapshot_insert_guard
    ON gda_control.cross_store_projection_recovery_snapshot_history;
CREATE TRIGGER trg_gda_projection_recovery_snapshot_insert_guard
BEFORE INSERT ON gda_control.cross_store_projection_recovery_snapshot_history
FOR EACH ROW EXECUTE FUNCTION gda_control.guard_cross_store_projection_recovery_insert();

DROP TRIGGER IF EXISTS trg_gda_projection_recovery_event_immutable
    ON gda_control.cross_store_projection_recovery_event_history;
CREATE TRIGGER trg_gda_projection_recovery_event_immutable
BEFORE UPDATE OR DELETE ON gda_control.cross_store_projection_recovery_event_history
FOR EACH ROW EXECUTE FUNCTION gda_control.reject_immutable_mutation();

DROP TRIGGER IF EXISTS trg_gda_projection_recovery_snapshot_immutable
    ON gda_control.cross_store_projection_recovery_snapshot_history;
CREATE TRIGGER trg_gda_projection_recovery_snapshot_immutable
BEFORE UPDATE OR DELETE ON gda_control.cross_store_projection_recovery_snapshot_history
FOR EACH ROW EXECUTE FUNCTION gda_control.reject_immutable_mutation();
...
ALTER TABLE gda_control.cross_store_projection_recovery_snapshot_history
    ENABLE ROW LEVEL SECURITY;
ALTER TABLE gda_control.cross_store_projection_recovery_snapshot_history
    FORCE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS tenant_isolation
...
REVOKE ALL ON TABLE gda_control.cross_store_projection_recovery_event_history
    FROM PUBLIC, gda_control_gateway;
REVOKE ALL ON TABLE gda_control.cross_store_projection_recovery_snapshot_history
    FROM PUBLIC, gda_control_gateway;
REVOKE ALL ON TABLE gda_control.cross_store_projection_recovery_snapshot_current
    FROM PUBLIC, gda_control_gateway;
GRANT SELECT ON TABLE gda_control.cross_store_projection_recovery_event_history
    TO gda_control_gateway;
GRANT SELECT ON TABLE gda_control.cross_store_projection_recovery_snapshot_history
    TO gda_control_gateway;
GRANT SELECT ON TABLE gda_control.cross_store_projection_recovery_snapshot_current
    TO gda_control_gateway;

REVOKE ALL ON FUNCTION gda_control.guard_cross_store_projection_recovery_insert()
    FROM PUBLIC;
REVOKE ALL ON FUNCTION gda_control.record_cross_store_projection_recovery_snapshot(
    TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB, TEXT, JSONB, TEXT
) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION gda_control.record_cross_store_projection_recovery_snapshot(
    TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, JSONB, TEXT, JSONB, TEXT
) TO gda_control_gateway;
