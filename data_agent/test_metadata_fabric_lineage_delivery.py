...
from data_agent.platform_contracts import canonical_json_bytes
    ...
    payload.update(
        {
            ...
            "claimed_until": (
                delivery.created_at + timedelta(minutes=1)
            ).isoformat(),
        }
    )
...
def test_delivery_is_deterministic_and_bound_to_authorized_source_plan():
    ...
    assert first == second
    assert str(first.delivery.delivery_id) == (
        "49a54408-b3a8-5843-a27d-6395c080af99"
    )
    ...
    assert first.delivery.actor_subject != (
        first.binding_bundle.record.recorded_by
    )
...
def test_http_emitter_uses_canonical_body_and_stable_idempotency_headers():
    ...
    assert all(
        request.headers["Idempotency-Key"] == delivery.idempotency_key
        for request in requests
    )
    assert receipt.response_status == 200
    assert receipt.response_body_sha256 == hashlib.sha256(
        success_body
    ).hexdigest()
