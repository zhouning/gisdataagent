...
DEFAULT_PROFILE_PATH = REPO_ROOT / "config/metadata-fabric-network-policy-enforcement.local.yaml"
...
def _resource(documents: list[dict[str, Any]], kind: str, name: str) -> dict[str, Any] | None:
    return next(
        (
            ...
            if item.get("kind") == kind and _mapping(item.get("metadata")).get("name") == name
        ),
        None,
    )
...
def _container_security_errors(container: Mapping[str, Any], *, pod_name: str) -> list[str]:
    errors: list[str] = []
...
def _manifest_errors(
    ...
    for document in base_documents:
        ...
        if (
            kind in {"ResourceQuota", "ServiceAccount", "Service"}
            and _mapping(metadata.get("labels")) != RESOURCE_LABELS
        ):
    ...
    quota = _resource(base_documents, "ResourceQuota", "metadata-network-policy-rehearsal") or {}
    ...
    if service_account.get("automountServiceAccountToken") is not False or set(service_account) != {
        "apiVersion",
        "kind",
        "metadata",
    ...
    for pod_name in sorted(PODS):
        ...
        if _mapping(spec.get("nodeSelector")) != {"kubernetes.io/hostname": expected_node}:
        ...
        if pod_name == "probe-server":
            ...
            if (
                ...
                or container.get("volumeMounts") != [{"name": "www", "mountPath": "/www"}]
                or spec.get("volumes") != [{"name": "www", "emptyDir": {"sizeLimit": "1Mi"}}]
            ):
...
def build_network_policy_contract_report(
    *,
    ...
    policy_paths = {name: manifests / filename for name, filename in POLICY_FILES.items()}
    ...
    policy_spec_fingerprints = {
        name: recovery._canonical_sha256(spec) for name, spec in EXPECTED_POLICY_SPECS.items()
    }
    static_inventory = sorted(
        f"{item.get('kind')}/{_mapping(item.get('metadata')).get('name')}" for item in documents
    )
    stable = {
...
def _ready(condition_list: Any) -> bool:
    return any(
        _mapping(item).get("type") == "Ready" and _mapping(item).get("status") == "True"
...
def _cluster_snapshot(runner: recovery._CommandRunner) -> dict[str, Any]:
    ...
    containers = (
        template_spec.get("containers") if isinstance(template_spec.get("containers"), list) else []
    )
    container = next(
        (_mapping(item) for item in containers if _mapping(item).get("name") == "kindnet-cni"),
        {},
    )
...
def _resource_identities(runner: recovery._CommandRunner) -> dict[str, Any]:
    ...
    return {
        ...
        "service": {
            ...
            "ports": [_mapping(port).get("port") for port in service_spec.get("ports") or []],
        },
        "policies": policies,
    }
...
def _runtime_resource_inventory(runner: recovery._CommandRunner) -> list[str]:
    ...
    for kind, resource in queries:
        ...
        resources.extend(
            f"{kind}/{_mapping(_mapping(item).get('metadata')).get('name')}" for item in items
        )
...
def _probe_client(
    ...
    return {
        ...
        "response_sha256": (
            hashlib.sha256(stdout.encode("utf-8")).hexdigest() if transport_connected else None
        ),
...
def _all_probe_pods_ready(runner: recovery._CommandRunner) -> bool:
    ...
    items = payload.get("items") if isinstance(payload.get("items"), list) else []
    return {
        str(_mapping(_mapping(item).get("metadata")).get("name")) for item in items
    } == PODS and all(
        _ready(_mapping(_mapping(item).get("status")).get("conditions")) for item in items
    )
    ...
    return {
        ...
        "runtime_checks": {
            ...
            "cross_node_placement_verified": (
                _mapping(_mapping(resources.get("pods")).get("probe-server")).get("node")
                == SERVER_NODE
                and all(
                    _mapping(_mapping(resources.get("pods")).get(name)).get("node") == CLIENT_NODE
                    for name in CLIENTS.values()
                )
            ),
            "policies_applied": policy_applied,
            "runtime_resource_inventory": runtime_inventory,
            "runtime_resource_inventory_matches": (runtime_inventory == EXPECTED_RUNTIME_RESOURCES),
...
def _resource_errors(resources: Mapping[str, Any]) -> list[str]:
    ...
    for name, spec in EXPECTED_POLICY_SPECS.items():
        ...
        if not policy.get("uid") or policy.get("spec_fingerprint") != recovery._canonical_sha256(
            spec
        ):
...
def _observation_errors(
    ...
    if contract.get("contract_fingerprint") != current_contract.get("contract_fingerprint"):
    ...
    if _mapping(runtime.get("policies_applied")) != {name: True for name in POLICY_FILES}:
    ...
    *,
    ...
    return {
        **stable,
        ...
        "status": ("local_network_policy_enforcement_verified" if verified else "blocked"),
...
def verify_evidence_integrity(report: Mapping[str, Any]) -> list[str]:
    ...
    expected_status = "local_network_policy_enforcement_verified" if verified else "blocked"
