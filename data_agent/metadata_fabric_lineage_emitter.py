...
class LineageEmitterProfile(BaseModel):
    ...
    def _bounded_local_endpoint(self) -> Self:
        ...
        if (
            ...
            raise ValueError(
                "local OpenLineage endpoint must be loopback /api/v1/lineage"
            )
...
class OpenLineageHttpEmitter:
    ...
    def emit(
        self, delivery: MetadataFabricLineageDelivery
    ) -> LineageHttpReceipt:
        if (
            delivery.status != LineageDeliveryStatus.IN_FLIGHT
            or delivery.claimed_by is None
        ):
