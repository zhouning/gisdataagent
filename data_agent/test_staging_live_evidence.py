...
from data_agent import staging_candidate_evidence, staging_live_evidence
...
PLATFORM_FINGERPRINT = _fingerprint(
    {"config": LIVE_CONFIG_FINGERPRINT, "runtime": RUNTIME_FINGERPRINT}
)
...
def test_collector_projects_allowlisted_fields_and_never_reads_secrets():
    ...
    raw_pods = {
        "items": [
            {
                "metadata": {"name": "gis-agent-app-abc", "uid": POD_UID},
                "spec": {
                    "serviceAccountName": "gis-agent-app",
                    "containers": [{"name": "app", "image": IMAGE}],
                },
