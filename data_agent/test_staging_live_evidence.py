...
RUNTIME_PRIVILEGE_CONTRACT_FINGERPRINT = "8" * 64
...
def _runtime_privilege_report() -> dict:
    ...
    stable = {
        ...
        "profile": "staging",
        "contract_fingerprint": RUNTIME_PRIVILEGE_CONTRACT_FINGERPRINT,
...
def _release(candidate: dict) -> dict:
    stable = {
        ...
        "runtime_privilege_fingerprint": candidate[
...
def test_stale_collection_and_missing_golden_slice_fail_closed():
    ...
    assert any("v2 allowlist" in error for error in report["errors"])
...
def test_collector_projects_allowlisted_fields_and_never_reads_secrets():
            ...
            "template": {
                "metadata": {
                    "annotations": {
                        ...
                        staging_live_evidence.RUNTIME_PRIVILEGE_FINGERPRINT_ANNOTATION: (
                            candidate["runtime_privilege_fingerprint"]
                        ),
                    }
                },
                "spec": {
    ...
    responses = [
        ...
        {
            **_runtime_privilege_report(),
            "connection_url": "must-never-appear",
            "observations": [
                {
                    **_runtime_privilege_report()["observations"][0],
    ...
    assert any(
        "data_agent.runtime_privilege_contract" in command
        for command in commands
    )
    privilege_command = next(
        command
        for command in commands
        if "data_agent.runtime_privilege_contract" in command
    )
    assert "--observe-only" in privilege_command
