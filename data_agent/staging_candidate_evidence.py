...
CANDIDATE_SCHEMA = "gda.staging_candidate_evidence.v2"
PLATFORM_TRUTH_SCHEMA = "gda.platform_truth.v1"
RUNTIME_PRIVILEGE_SCHEMA = "gda.runtime_privilege_contract.v1"
...
REQUIRED_LIVE_EVIDENCE = (
    ...
    "live schema/config/runtime/runtime-privilege fingerprints",
...
def _runtime_privilege_stable(report: dict[str, Any]) -> dict[str, Any]:
    return {
...
def _runtime_privilege_errors(report: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if report.get("schema") != RUNTIME_PRIVILEGE_SCHEMA:
        errors.append("candidate runtime privilege contract schema is unsupported")
    if report.get("profile") != "staging":
        errors.append("candidate runtime privilege profile must be staging")
    if report.get("status") != "in_sync":
        errors.append("candidate runtime privilege contract must be in_sync")
    if report.get("admission_allowed") is not True:
        errors.append("candidate runtime privilege admission must be allowed")
    if report.get("read_only") is not True:
        errors.append("candidate runtime privilege observation must be read-only")
    if report.get("self_healed") is not False:
        errors.append("candidate runtime privilege observation must not self-heal")
    if report.get("runtime_role") != "agent_user":
        errors.append("candidate runtime privilege observer must be agent_user")
    if report.get("gateway_role") != "gda_control_gateway":
        errors.append("candidate runtime privilege gateway role is unsupported")
    if report.get("drift") != []:
        errors.append("candidate runtime privilege contract contains drift")
    role = report.get("role_observation")
    if (
        not isinstance(role, dict)
        or role.get("status") != "in_sync"
        ...
        errors.append("candidate runtime privilege role binding must be in_sync")
    expected_attributes = {
        "can_login": False,
        "is_superuser": False,
        "can_create_db": False,
    ...
    if isinstance(role, dict) and (
        role.get("expected_gateway_attributes") != expected_attributes
        or role.get("actual_gateway_attributes") != expected_attributes
    ):
        errors.append("candidate runtime privilege gateway attributes drifted")
    observations = report.get("observations")
    if (
        not isinstance(observations, list)
        or not observations
        or any(
            not isinstance(item, dict) or item.get("status") != "in_sync"
        ...
        errors.append("candidate runtime privilege object observations must be in_sync")
    if not _valid_sha256(report.get("contract_fingerprint")):
        errors.append("candidate runtime privilege contract fingerprint must be sha256")
    evidence_fingerprint = report.get("evidence_fingerprint")
    if not _valid_sha256(evidence_fingerprint):
        errors.append("candidate runtime privilege fingerprint must be sha256")
    elif evidence_fingerprint != _canonical_sha256(
        _runtime_privilege_stable(report)
    ):
        errors.append(
            "candidate runtime privilege fingerprint does not match its content"
        )
    return errors
...
def build_candidate_evidence(
    ...
    runtime_privilege_report: dict[str, Any],
    *,
    source_revision: str,
    image_id: str,
) -> dict[str, Any]:
    ...
    errors.extend(_runtime_privilege_errors(runtime_privilege_report))
    runtime_privilege_fingerprint = runtime_privilege_report.get(
    ...
    stable_evidence = {
        ...
        "runtime_privilege_fingerprint": runtime_privilege_fingerprint,
        "tests": test_summary,
...
def main(argv: list[str] | None = None) -> int:
    ...
    validate.add_argument("--runtime-privilege-report", type=Path, required=True)
    ...
    try:
        report = build_candidate_evidence(
            ...
            _load_json_object(args.runtime_privilege_report),
