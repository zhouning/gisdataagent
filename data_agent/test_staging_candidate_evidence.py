...
def _runtime_privilege_report() -> dict:
    attributes = {
        "can_login": False,
        "is_superuser": False,
        "can_create_db": False,
    ...
    stable = {
        "schema": "gda.runtime_privilege_contract.v1",
        "profile": "staging",
        ...
        "role_observation": {
            ...
            "expected_gateway_attributes": attributes,
            "actual_gateway_attributes": attributes,
            "status": "in_sync",
            "violations": [],
        },
        "observations": [
            {
                "object_id": "schema:gda_control.gda_control",
                "kind": "schema",
                "object_exists": True,
                "expected_privileges": ["USAGE"],
                "actual_privileges": ["USAGE"],
                "missing_privileges": [],
    ...
    return {
        **stable,
        ...
        "evidence_fingerprint": (
            staging_candidate_evidence._canonical_sha256(stable)
        ),
    }
...
def _write_inputs(tmp_path: Path) -> tuple[Path, Path, Path, Path]:
    ...
    runtime_privilege = tmp_path / "runtime-privileges.json"
    runtime_privilege.write_text(
        json.dumps(_runtime_privilege_report()), encoding="utf-8"
    )
    return schema, platform, junit, runtime_privilege
...
def test_valid_candidate_evidence_never_claims_live_staging_or_promotion():
    ...
    assert report["runtime_privilege_fingerprint"] == (
        _runtime_privilege_report()["evidence_fingerprint"]
    )
...
def test_candidate_gate_rejects_runtime_privilege_content_tampering():
    runtime_privilege = _runtime_privilege_report()
    runtime_privilege["role_observation"]["actual_gateway_attributes"][
        "can_login"
    ] = True
    ...
    assert report["candidate_validated"] is False
    assert any(
        "runtime privilege fingerprint does not match" in error
...
def test_candidate_cli_writes_machine_readable_non_promotion_evidence(
    ...
    schema, platform, junit, runtime_privilege = _write_inputs(tmp_path)
    ...
    assert staging_candidate_evidence.main(
        [
            ...
            "--junit",
            str(junit),
            "--runtime-privilege-report",
            str(runtime_privilege),
            "--source-revision",
            SHA,
            "--image-id",
            IMAGE_ID,
...
def test_staging_workflow_is_candidate_validation_not_fake_deployment():
    ...
    assert "data_agent.runtime_privilege_contract" in validation_commands
    assert validation_commands.count("data_agent.runtime_privilege_contract") == 2
    assert "--runtime-role agent_user" in validation_commands
    assert "--runtime-privilege-report" in validation_commands
    ...
    assert uploaded_paths == {
        ...
        "staging-candidate-evidence/runtime-privileges.json",
