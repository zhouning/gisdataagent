...
tables generated by the synthetic major-project artifacts. Neo4j runtime
checks are optional and never required for SQL grounding to proceed.
"""
...
def resolve_major_project_kg_hints(
    ...
    _enrich_with_neo4j_if_enabled(result)
    return result


def _neo4j_backend_enabled() -> bool:
    value = os.environ.get("MAJOR_PROJECT_KG_BACKEND", "")
    return value.strip().lower() in {"neo4j", "graph", "graph_database"}


def _enrich_with_neo4j_if_enabled(result: dict[str, Any]) -> None:
    if not _neo4j_backend_enabled():
        return

    required_edges = list(result.get("required_edges") or [])
    try:
        edge_counts = _query_neo4j_edge_counts(required_edges)
    except Exception as exc:
        result["graph_backend"] = GRAPH_BACKEND
        result["neo4j"] = {
            "status": "error",
            "message": str(exc)[:240],
        }
    ...
    result["graph_backend"] = NEO4J_GRAPH_BACKEND
    result["neo4j"] = {
        "status": "ok",
...
def _query_neo4j_edge_counts(edge_types: list[str]) -> dict[str, int]:
    edge_type_values = [str(edge_type) for edge_type in edge_types if edge_type]
    ...
    database = os.environ.get("NEO4J_DATABASE", "neo4j")
    if not password:
        raise RuntimeError("NEO4J_PASSWORD is required when MAJOR_PROJECT_KG_BACKEND=neo4j")

    driver = GraphDatabase.driver(uri, auth=(user, password))
    try:
        query = """
        UNWIND $edge_types AS edge_type
        OPTIONAL MATCH ()-[r:SYNTHETIC_KG_EDGE]->()
        WHERE r.edge_type = edge_type
        RETURN edge_type, count(r) AS edge_count
        """
        with driver.session(database=database) as session:
            rows = session.run(query, edge_types=edge_type_values)
            return {
                str(row["edge_type"]): int(row["edge_count"] or 0)
                for row in rows
            }
    finally:
        driver.close()
