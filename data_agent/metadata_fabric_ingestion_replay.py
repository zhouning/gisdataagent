...
HISTORICAL_EVIDENCE_CONTRACT_FINGERPRINTS = frozenset(
    {"1deefa60d09b53fce4729de3e75896fe2fb49f8badbc07ca155d28403e451afb"}
)
...
DEFAULT_EVIDENCE_PATH = REPO_ROOT / "docs/evidence/metadata-fabric-ingestion-replay-2026-07-28.json"
...
def load_profile(path: Path = DEFAULT_PROFILE_PATH) -> LocalIngestionProfile:
    try:
        ...
        raise MetadataFabricIngestionReplayError("local ingestion profile is invalid") from exc
...
def build_local_apply_plan(
    ...
    if profile.targets.gravitino.identity != ("gda_lakehouse.iceberg.land_use.land_use_parcels"):
        raise MetadataFabricIngestionReplayError(
...
def build_execution_plan_artifact(
    ...
    *,
    ...
    return Artifact(
        ...
        storage_uri=(f"postgresql://gda-control/execution-plans/{plan.tenant_id}/{artifact_id}"),
...
def validate_apply_authorization(
    ...
    *,
    ...
    approval_matches = (
        ...
        and approval.policy_decision_artifact_id == bundle.policy_decision_artifact.artifact_id
        and approval.policy_decision_sha256 == bundle.policy_decision_artifact.content_sha256
    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            raise ProviderRequestError(f"{self.provider_name} {method} request failed") from exc
        if response.status_code == 404 and allow_not_found:
            return None
        if response.status_code >= 400:
            raise ProviderRequestError(
                f"{self.provider_name} rejected {method} request with status {response.status_code}"
            )
        ...
        try:
            ...
            raise ProviderRequestError(f"{self.provider_name} returned invalid JSON") from exc
        if not isinstance(payload, dict):
            raise ProviderRequestError(f"{self.provider_name} response is not an object")
        return payload
...
class OpenMetadataApplyClient(_ProviderHttpClient):
    ...
    def __init__(
        ...
        *,
        ...
        encoded = base64.b64encode(password.get_secret_value().encode("utf-8")).decode("ascii")
        try:
        ...
        try:
            ...
            raise ProviderRequestError("OpenMetadata login did not return an access token") from exc
        super().__init__(
            base_url=base_url,
            headers={
    ...
    def apply(
        ...
        projection = next(item for item in plan.projections if item.provider == "openmetadata")
        ...
        if table is None:
            table = self._request(
                ...
                json_body={
                    ...
                    "tags": [
                        {
                            "tagFQN": (f"{target.classification}.{target.classification_tag}"),
                    ...
                    "extension": {
                        ...
                        "gdaResourceVersionId": projection.desired_state["resource_version_id"],
...
class GravitinoApplyClient(_ProviderHttpClient):
    ...
    def get_table(self, target: GravitinoTarget) -> dict[str, Any] | None:
        parent_paths = (
            ...
            (f"metalakes/{target.metalake}/catalogs/{target.catalog}/schemas/{target.schema_name}"),
        )
    ...
    def apply(
        ...
        _, schema_created = self._get_or_create(
            ...
            create_path=(f"metalakes/{target.metalake}/catalogs/{target.catalog}/schemas"),
            payload={
                "name": target.schema_name,
        ...
        projection = next(item for item in plan.projections if item.provider == "gravitino")
            ...
            table = self._request(
                ...
                json_body={
                    ...
                    "properties": {
                        ...
                        "gda.resource_version_id": projection.desired_state["resource_version_id"],
                        "gda.content_sha256": projection.desired_state["content_sha256"],
                        "gda.provider_revision": projection.desired_state["provider_revision"],
                    },
                },
            )
            assert table is not None
    ...
    def compensate(self) -> bool:
        ...
        self.created_metalake = None
        if errors:
            raise MetadataFabricIngestionReplayError("Gravitino compensation left created entities")
        return True
...
def _verify_provider_state(
    ...
    *,
    ...
    gravitino_projection = next(item for item in plan.projections if item.provider == "gravitino")
    ...
    if (
        ...
        raise MetadataFabricIngestionReplayError("live OpenMetadata governance projection drift")
    if technical.provider_revision != gravitino_projection.desired_state["provider_revision"]:
        raise MetadataFabricIngestionReplayError("live Gravitino provider revision drift")
    return (
        governance,
        technical,
...
def apply_once(
    ...
    *,
    ...
    if openmetadata_before is None:
        try:
            openmetadata_payload = openmetadata.apply(plan, profile.targets.openmetadata)
...
def build_contract_report(
    ...
    try:
        profile, plan, _run, _target, authorization = _build_contract_inputs(profile_path)
    except (
        AuthorizationEvidenceError,
...
def _kubectl_json(args: list[str]) -> dict[str, Any]:
    ...
    try:
        ...
        raise MetadataFabricIngestionReplayError("kubectl observation is invalid JSON") from exc
    if not isinstance(payload, dict):
...
def _provider_runtime_identity(profile: LocalIngestionProfile) -> dict[str, Any]:
    ...
    for name, provider in (
        ...
        service = _kubectl_json(["-n", NAMESPACE, "get", "service", provider.service, "-o", "json"])
        workload_kind = "deployment" if name == "openmetadata" else "statefulset"
        workload_name = "openmetadata" if name == "openmetadata" else "metadata-gravitino"
...
def build_evidence(observation: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(runtime, Mapping) or runtime.get("all_port_forwards_stopped") is not True:
...
def verify_evidence_integrity(evidence: Mapping[str, Any]) -> list[str]:
    errors: list[str] = []
    stable = {key: value for key, value in evidence.items() if key != "evidence_fingerprint"}
...
def run_live_rehearsal(
    ...
    try:
        ...
        try:
            ...
            password = SecretStr(os.environ[profile.providers.openmetadata.password_env])
    ...
    observation = {
        ...
        "authorization": {
            ...
            "execution_plan_artifact_id": str(authorization.execution_plan_artifact.artifact_id),
            "policy_decision_artifact_id": str(authorization.policy_decision_artifact.artifact_id),
...
def build_validation_report(
    *,
    ...
    evidence_contract_fingerprint: str | None = None
    contract_fingerprint_matches_current = False
    semantic_contract_compatibility_verified = False
    historical_contract_compatibility_verified = False
    try:
        ...
        observed_contract = (
            observation.get("contract") if isinstance(observation, Mapping) else None
        )
        observed_authorization = (
            observation.get("authorization") if isinstance(observation, Mapping) else None
        )
        if isinstance(observed_contract, Mapping):
            candidate = observed_contract.get("contract_fingerprint")
            if isinstance(candidate, str):
                evidence_contract_fingerprint = candidate
            semantic_contract_compatibility_verified = (
                observed_contract.get("source_plan_sha256") == contract["source_plan_sha256"]
                and observed_contract.get("apply_plan_sha256") == contract["apply_plan_sha256"]
                and isinstance(observed_authorization, Mapping)
                and observed_authorization.get("authorization_sha256")
                == contract["authorization_sha256"]
            )
        contract_fingerprint_matches_current = (
            evidence_contract_fingerprint == contract["contract_fingerprint"]
        )
        historical_contract_compatibility_verified = (
            not contract_fingerprint_matches_current
            and evidence_contract_fingerprint in HISTORICAL_EVIDENCE_CONTRACT_FINGERPRINTS
            and semantic_contract_compatibility_verified
        )
        if not (
            contract_fingerprint_matches_current
            or evidence_contract_fingerprint in HISTORICAL_EVIDENCE_CONTRACT_FINGERPRINTS
        ):
            errors.append("local ingestion evidence contract fingerprint is unsupported")
        if not semantic_contract_compatibility_verified:
            errors.append("local ingestion evidence semantic contract drift")
    ...
    return {
        ...
        "contract_fingerprint_matches_current": contract_fingerprint_matches_current,
        "semantic_contract_compatibility_verified": (semantic_contract_compatibility_verified),
        "historical_contract_compatibility_verified": (historical_contract_compatibility_verified),
