...
def test_auto_retry_plan_is_deterministic_and_contains_duplicate_target_rows() -> None:
    first = build_sql_merge_auto_retry_plan(DEFAULT_SOURCE)
    second = build_sql_merge_auto_retry_plan(DEFAULT_SOURCE)
...
def test_auto_retry_source_uses_real_merge_and_automatic_retry_contract() -> None:
    ...
    assert "gda.spark_sql_merge_auto_retry_ready.v1" in source
    assert "def _automatic_retry" in source
    assert "overwritePartitions" not in source


def test_automatic_retry_preserves_baseline_and_flink_revision() -> None:
