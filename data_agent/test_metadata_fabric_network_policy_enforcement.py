        ...
        pods[name] = {
            ...
            "labels": {
                ...
                **(
                    {}
                    if server
                    else {"access": ("allowed" if name == "probe-allowed" else "denied")}
                ),
            },
            "ready": True,
        }
        ...
        stages.append(
            {
                ...
                "attempt_count": 1,
                "clients": {
                    client: _probe(outcome) for client, outcome in expected["expected"].items()
                },
...
def test_static_contract_rejects_manifest_process_storage_and_privilege_drift(
    ...
    server = next(
        ...
        if item.get("kind") == "Pod" and item.get("metadata", {}).get("name") == "probe-server"
    )
...
def test_runtime_policy_normalization_restores_api_omitted_default_deny_rules():
    ingress = deepcopy(enforcement.EXPECTED_POLICY_SPECS["probe-server-default-deny"])
    ingress.pop("ingress")
    egress = deepcopy(enforcement.EXPECTED_POLICY_SPECS["probe-authorized-default-deny"])
    egress.pop("egress")

    assert (
        enforcement._normalized_runtime_policy_spec(ingress)
        == (enforcement.EXPECTED_POLICY_SPECS["probe-server-default-deny"])
    )
    assert (
        enforcement._normalized_runtime_policy_spec(egress)
...
def test_evidence_blocks_stage_cni_server_and_node_drift():
    ...
    report = enforcement.build_network_policy_evidence(observation, now=NOW + timedelta(seconds=10))
...
def test_committed_network_policy_evidence_is_integral_and_current():
    evidence_path = (
        ...
        / "docs/evidence/metadata-fabric-network-policy-enforcement-2026-08-16.json"
    )
    report = json.loads(evidence_path.read_text(encoding="utf-8"))

    assert enforcement.verify_evidence_integrity(report) == []
    assert (
        report["observation"]["contract"]["contract_fingerprint"]
