...
def test_non_livability_demands_are_not_promoted_by_generic_platform_capabilities():
    ...
    assert demands["1"]["implementation_status"] == "implemented_evidence_bounded"
    assert demands["1"]["max_supported_claim"] == "fragile_spatial_scope_admin_unit_registry_and_uwm_identity_readiness"
    assert demands["2"]["implementation_status"] == "implemented_evidence_bounded"
    assert demands["2"]["max_supported_claim"] == "planning_parcel_asset_inventory_version_contract_and_temporal_baseline_readiness"
    assert demands["3"]["implementation_status"] == "implemented_evidence_bounded"
    ...
    assert demands["6"]["implementation_status"] == "implemented_evidence_bounded"
    assert demands["6"]["max_supported_claim"] == "observed_population_evidence_catalog_demographic_contract_and_uwm_population_dynamics_readiness"
    assert all(check["exists"] for check in demands["6"]["evidence_artifact_checks"])
