"""Top-K Targeted DAgger + Ensemble experiment runner.

Runs 5 seeds x 3 DAgger iterations using a 5-member transition-model ensemble.
Compared with the threshold-based targeted DAgger variant, this runner keeps the
Top-K highest-disagreement on-policy transitions per iteration, and records the
best iteration per seed by real-environment reward.
"""
...
RESULTS_DIR = os.getenv("PAPER9_RESULTS_DIR", "D:/adk/results_dual_dreamer_real_topk")
...
def run_iteration(
    ...
    result["selection_method"] = f"topk_{TOPK_ONPOLICY}"
