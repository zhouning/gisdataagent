"""Controlled DAgger + Ensemble experiment runner.

A single configurable runner for paper9 fairness-control experiments.
Supports:
- evidence_mode = blind | threshold | topk
- optional best-checkpoint selection
- optional rollback protection
- optional transition-model early stopping

This avoids maintaining many near-duplicate runners when comparing whether
stability comes from evidence selection or from engineering safeguards.
"""
...
RESULTS_DIR = os.getenv("PAPER9_RESULTS_DIR", "D:/adk/results_dual_dreamer_real_controlled")
...
DISAGREEMENT_FILTER = float(os.getenv("PAPER9_DISAGREEMENT_FILTER", "0.5"))
TOPK_ONPOLICY = int(os.getenv("PAPER9_TOPK", "20"))
MIN_DREAM_STEPS = 20
EVIDENCE_MODE = os.getenv("PAPER9_EVIDENCE_MODE", "threshold").lower()  # blind | threshold | topk
USE_BEST_CHECKPOINT = os.getenv("PAPER9_USE_BEST_CHECKPOINT", "1") == "1"
USE_ROLLBACK = os.getenv("PAPER9_USE_ROLLBACK", "1") == "1"
USE_EARLY_STOP = os.getenv("PAPER9_USE_EARLY_STOP", "1") == "1"
...
def run_iteration(
    ...
    trainer_patience = 6 if USE_EARLY_STOP else 0
    histories = ensemble.train_all(data, epochs=EPOCHS, lr=1e-3, patience=trainer_patience, min_delta=1e-4)
    ...
    if USE_BEST_CHECKPOINT and callback.best_path.exists():
    ...
    if iteration < 2:
        ...
        for _ep in range(10):
            ...
            while not done:
                ...
                candidates.append((r_std, bf, gf, action, reward, nbf, ngf))
                obs = next_obs
                done = term or trunc

        if EVIDENCE_MODE == "blind":
            selected = candidates
        elif EVIDENCE_MODE == "threshold":
            selected = [c for c in candidates if c[0] >= DISAGREEMENT_FILTER]
        else:
            candidates.sort(key=lambda x: x[0], reverse=True)
            selected = candidates[:TOPK_ONPOLICY]

        for r_std, bf, gf, action, reward, nbf, ngf in selected:
    ...
    result["selection_method"] = (
        f"topk_{TOPK_ONPOLICY}" if EVIDENCE_MODE == "topk" else
        f"threshold_{DISAGREEMENT_FILTER}" if EVIDENCE_MODE == "threshold" else
        "blind"
    )
...
def main() -> None:
    ...
    for seed in seeds:
        ...
        for iteration in range(len(seed_results), 3):
            ...
            should_retry = False
            if USE_ROLLBACK and iteration == 1 and seed_results:
            ...
            best_result = max(seed_results, key=lambda r: r["reward"])
            for r in seed_results:
                r["is_best_iteration"] = (r["iter"] == best_result["iter"])
