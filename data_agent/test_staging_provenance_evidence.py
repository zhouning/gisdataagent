...
def _registry() -> dict:
    stable = {
        ...
        "runtime_privilege_fingerprint": "e" * 64,
...
def test_registry_runtime_privilege_drift_blocks_before_attestation():
    registry = _registry()
    del registry["runtime_privilege_fingerprint"]
    ...
    assert report["status"] == "blocked"
    assert "runtime privilege fingerprint must be sha256" in "\n".join(
        report["errors"]
    )
    assert report["production_promotion_allowed"] is False


def test_legacy_registry_v1_artifact_is_rejected():
    registry = _registry()
    registry["schema"] = "gda.staging_registry_evidence.v1"
    ...
    assert report["status"] == "blocked"
    assert "registry evidence schema is unsupported" in "\n".join(
        report["errors"]
    )
    assert report["production_promotion_allowed"] is False
