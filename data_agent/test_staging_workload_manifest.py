...
def test_renderer_emits_only_immutable_workload_resources():
    ...
    assert annotations[
        staging_workload_manifest.RUNTIME_PRIVILEGE_FINGERPRINT_ANNOTATION
    ] == "9" * 64
    ...
    assert "data_agent.runtime_privilege_contract" in init["args"][0]
    assert "--profile staging" in init["args"][0]
    assert init["env"] == [
