...
def _payload(source: dict[str, object]) -> dict[str, object]:
    begin = datetime.fromisoformat(
        str(source["begin_utc"]).replace("Z", "+00:00")
    )
    features = []
...
def test_stage42_acquirer_binds_exact_frozen_plan():
    plan = acquire._load_frozen_plan()

    assert plan["frozen_protocol_artifact"]["sha256"] == (
        acquire.planner.FROZEN_PROTOCOL_SHA256
    )
    assert len(plan["sources"]) == 8


def test_stage42_valid_payload_allows_gaps_without_filling():
    source = _source()
...
def test_stage42_payload_identity_and_values_fail_closed(
...
def test_stage42_payload_rejects_next_page_link():
    source = _source()
    payload = _payload(source)
...
def test_stage42_payload_rejects_duplicate_or_unsorted_times():
    source = _source()
    payload = _payload(source)
    payload["features"][1]["properties"]["time"] = payload["features"][0][
        "properties"
    ]["time"]
...
def test_stage42_payload_rejects_off_grid_or_out_of_window_time():
    source = _source()
    payload = _payload(source)
    payload["features"][0]["properties"]["time"] = "2025-04-14T16:15:00Z"
    with pytest.raises(ValueError, match="feature_invalid"):
        acquire._validate_payload(payload, source)
...
def test_stage42_url_and_output_allowlists_fail_closed(tmp_path):
...
def test_stage42_new_state_has_exact_eight_sources(tmp_path):
...
def test_stage42_fetch_once_enforces_size_before_return():
    class Response:
        status = 200
...
def test_stage42_manifest_bounds_attempts_bytes_and_claims(tmp_path, monkeypatch):
    plan = acquire._load_frozen_plan()
    ...
    assert manifest["actual_request_count"] == 8
    assert manifest["actual_attempt_count"] == 8
    assert manifest["actual_download_bytes"] == 800
    assert manifest["claim_boundary"]["downstream_target_values_acquired"] is True
    assert manifest["claim_boundary"]["empirical_lag_support_sets_compiled"] is False
