from __future__ import annotations

import csv
import hashlib
import io
import json

import pytest

from data_agent.uwm.geospatial_kernel_v2.physical_residual_decay import (
    PHYSICAL_RESIDUAL_DECAY_SCHEMA,
    physical_residual_decay_parameters_from_dict,
)
from scripts import evaluate_geospatial_kernel_action_innovation_cross_system as cross
from scripts import evaluate_geospatial_kernel_physical_residual_decay_transfer as decay


def _paths(tmp_path):
    return {
        "parameter_path": tmp_path / "parameters.json",
        "primary_prediction_path": tmp_path / "primary.csv",
        "replication_prediction_path": tmp_path / "replication.csv",
    }


def test_residual_decay_fits_source_only_and_transfers_one_parameter(tmp_path) -> None:
    bodies, report = decay.compile_physical_residual_decay_transfer_posthoc(**_paths(tmp_path))
    parameters = json.loads(bodies["parameters"])
    primary_rows = list(csv.DictReader(io.StringIO(bodies["primary_predictions"].decode())))
    replication_rows = list(csv.DictReader(io.StringIO(bodies["replication_predictions"].decode())))

    assert parameters["schema"] == PHYSICAL_RESIDUAL_DECAY_SCHEMA
    assert parameters["residual_decay_coefficient"] == pytest.approx(0.9668996582212471)
    assert parameters["training_pair_count"] == 669
    assert parameters["source_system_id"] == "center_hill"
    assert parameters["source_outcomes_used_for_fit"] is True
    assert parameters["target_outcomes_used_for_fit"] is False
    assert parameters["admitted"] is False
    assert report["source_fit_contract"]["free_parameter_count"] == 1
    assert report["source_fit_contract"]["target_system_id"] == "j_percy_priest"
    assert report["source_fit_contract"]["per_target_window_refit_performed"] is False
    assert report["source_fit_contract"]["same_parameter_used_across_target_windows"] is True
    replication = report["replication_source_diagnostic"]
    assert replication["residual_decay_coefficient"] == pytest.approx(0.9636751508935915)
    assert replication["coefficient_minus_frozen_parameter"] == pytest.approx(-0.003224507327655668)
    assert replication["used_for_target_prediction"] is False
    assert report["claim_boundary"]["physical_residual_decay_admitted"] is False
    assert report["claim_boundary"]["physical_residual_decay_promoted"] is False
    assert report["claim_boundary"]["geospatial_kernel_validated"] is False
    assert report["claim_boundary"]["runtime_default_enabled"] is False
    assert report["source_artifacts"]["source_sealed_physical_predictions"]["path"].endswith(
        "predictions/center_hill.csv"
    )
    assert report["promotion_gate"]["accuracy_requirement_passed"] is False
    assert report["promotion_gate"]["physical_residual_decay_promotion_gate_passed"] is False
    assert primary_rows and replication_rows
    assert {row["residual_decay_parameter_sha256"] for row in primary_rows} == {
        row["residual_decay_parameter_sha256"] for row in replication_rows
    }
    assert all(
        row["future_target_observation_used_for_correction"] == "False"
        for row in primary_rows + replication_rows
    )
    assert all(
        int(row["elapsed_from_latest_observation_hours"]) == int(row["horizon_hours"]) + 1
        for row in primary_rows + replication_rows
    )


def test_residual_decay_improves_constant_correction_but_not_raw_physics(
    tmp_path,
) -> None:
    _, report = decay.compile_physical_residual_decay_transfer_posthoc(**_paths(tmp_path))

    for name in ("primary_window", "replication_window"):
        comparison = report[name]["comparison"]
        assert comparison["decay_beats_constant_correction_all_horizons"] is True
        assert comparison["decay_beats_raw_physical_all_horizons"] is False
        assert comparison["decay_beats_raw_physical_horizons_hours"] == []
        assert comparison["decay_beats_arx_all_horizons"] is True
        assert comparison["decay_beats_wwm_all_horizons"] is True
        assert comparison["decay_beats_persistence_all_horizons"] is True
        assert all(
            value["decay_minus_constant_correction_rmse_m3s"] < 0.0
            and value["decay_minus_raw_physical_rmse_m3s"] > 0.0
            for value in comparison["per_horizon"].values()
        )

    primary = report["primary_window"]["metrics_by_horizon"]
    replication = report["replication_window"]["metrics_by_horizon"]
    assert primary["12"]["physical_residual_decay"]["rmse_m3s"] == pytest.approx(31.98836192803558)
    assert replication["12"]["physical_residual_decay"]["rmse_m3s"] == pytest.approx(
        39.90878878324806
    )
    interpretation = report["diagnostic_interpretation"]
    assert interpretation["decay_beats_constant_correction_all_horizons_in_both_windows"] is True
    assert interpretation["decay_beats_raw_physical_all_horizons_in_both_windows"] is False
    assert interpretation["result_may_trigger_refit_on_these_windows"] is False


def test_parameter_body_is_locked_before_target_comparison_rows_load(tmp_path, monkeypatch) -> None:
    events = []
    comparison = json.loads(decay.DEFAULT_COMPARISON_REPORT.read_text())
    target_path = comparison["outputs"]["primary_predictions"]["path"]
    original_read_verified = decay.cross._read_verified
    original_json_body = decay._json_body

    def tracking_read_verified(descriptor):
        if descriptor.get("path") == target_path:
            events.append("target_comparison_rows_load")
        return original_read_verified(descriptor)

    def tracking_json_body(value):
        if value.get("schema") == PHYSICAL_RESIDUAL_DECAY_SCHEMA:
            events.append("parameter_body_compile")
        return original_json_body(value)

    monkeypatch.setattr(decay.cross, "_read_verified", tracking_read_verified)
    monkeypatch.setattr(decay, "_json_body", tracking_json_body)

    decay.compile_physical_residual_decay_transfer_posthoc(**_paths(tmp_path))

    assert events == ["parameter_body_compile", "target_comparison_rows_load"]


def test_future_target_outcome_cannot_change_decay_forecasts(tmp_path) -> None:
    bodies, _ = decay.compile_physical_residual_decay_transfer_posthoc(**_paths(tmp_path))
    parameters = physical_residual_decay_parameters_from_dict(json.loads(bodies["parameters"]))
    comparison = json.loads(decay.DEFAULT_COMPARISON_REPORT.read_text())
    source = cross._read_verified(comparison["outputs"]["primary_predictions"])
    parameter_sha256 = hashlib.sha256(bodies["parameters"]).hexdigest()

    original_body, _ = decay._compile_window(
        source_body=source,
        parameters=parameters,
        parameter_sha256=parameter_sha256,
    )
    mutated_body, _ = decay._compile_window(
        source_body=_mutate_target_outcomes(source),
        parameters=parameters,
        parameter_sha256=parameter_sha256,
    )
    original_rows = list(csv.DictReader(io.StringIO(original_body.decode())))
    mutated_rows = list(csv.DictReader(io.StringIO(mutated_body.decode())))
    forecast_columns = (
        "physical_residual_decay_m3s",
        "residual_decay_weight",
        "elapsed_from_latest_observation_hours",
        "residual_decay_clipped",
    )

    assert [row["observed_discharge_m3s"] for row in original_rows] != [
        row["observed_discharge_m3s"] for row in mutated_rows
    ]
    assert [tuple(row[column] for column in forecast_columns) for row in original_rows] == [
        tuple(row[column] for column in forecast_columns) for row in mutated_rows
    ]


def test_residual_decay_outputs_are_deterministic_and_bound(tmp_path) -> None:
    paths = _paths(tmp_path)
    first_bodies, first_report = decay.compile_physical_residual_decay_transfer_posthoc(**paths)
    second_bodies, second_report = decay.compile_physical_residual_decay_transfer_posthoc(**paths)

    assert first_bodies == second_bodies
    assert first_report["parameter_lock"] == second_report["parameter_lock"]
    assert first_report["primary_window"] == second_report["primary_window"]
    assert first_report["replication_window"] == second_report["replication_window"]
    for name, body in first_bodies.items():
        descriptor = first_report["outputs"][name]
        assert descriptor["sha256"] == hashlib.sha256(body).hexdigest()
        assert descriptor["size_bytes"] == len(body)


def test_residual_decay_rejects_tampered_source_physical_descriptor(
    tmp_path,
) -> None:
    payload = json.loads(decay.DEFAULT_SOURCE_PHYSICAL_REPORT.read_text(encoding="utf-8"))
    payload["systems"][decay.SOURCE_SYSTEM_ID]["prediction_artifact"]["sha256"] = "0" * 64
    tampered = tmp_path / "source_physical.json"
    tampered.write_text(json.dumps(payload), encoding="utf-8")

    with pytest.raises(ValueError, match="cross_system_posthoc_artifact_identity_mismatch"):
        decay.compile_physical_residual_decay_transfer_posthoc(
            source_physical_report_path=tampered,
            **_paths(tmp_path),
        )


def _mutate_target_outcomes(body: bytes) -> bytes:
    rows = list(csv.DictReader(io.StringIO(body.decode("utf-8"))))
    for row in rows:
        if row["observed_discharge_m3s"]:
            row["observed_discharge_m3s"] = str(float(row["observed_discharge_m3s"]) + 10_000.0)
    return cross._encode_rows(rows)
