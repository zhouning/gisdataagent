...
from data_agent.agentops_temporal_reconciler_fencing_postgres_rehearsal import (
...
def test_fencing_rehearsal_report_writer_preserves_hash(tmp_path) -> None:
    payload = {
        ...
        "checks": {"stale_worker_is_fenced_before_checkpoint_write": True},
        "passed": True,
        "failure_reasons": [],
    ...
    report = AgentOpsTemporalReconcilerFencingPostgresRehearsalReport.model_validate(
        payload
    )

    target = write_agentops_temporal_reconciler_fencing_postgres_rehearsal_report(
        report,
        tmp_path / "agentops-reconciler-fencing.json",
    )
...
def test_real_postgres_fencing_rehearsal() -> None:
