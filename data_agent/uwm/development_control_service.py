...
class DevelopmentControlService:
    def __init__(self, root: Path):
        ...
        if len(bundle_ids) != 1 or None in bundle_ids:
            raise ValueError("development_control_bundle_mismatch")

    def overview(self): return deepcopy(self._payloads["overview"])
    def rule_assets(self): return deepcopy(self._payloads["rule_assets"])
    def dcr_channels(self): return deepcopy(self._payloads["dcr_channels"])
    def data_contracts(self): return deepcopy(self._payloads["data_contracts"])
    def execution_gate(self): return deepcopy(self._payloads["execution_gate"])
