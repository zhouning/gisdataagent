...
def _complete_profile() -> dict:
    ...
    profile["policy"].update(
        {
            "label_admission_policy_reference": (
                "policy://production/workload-identity-labels-v1"
            ),
    ...
    profile["traffic_matrix"] = {
        ...
        "entries": [
            ...
            for flow_id, (source, destination, ports, purpose) in (
                gate.EXPECTED_FLOWS.items()
            )
        ],
    }
    profile["operations"] = {
...
def _attestation(profile_path: Path, **changes: object) -> dict:
    ...
    payload = {
        ...
        "evidence_uri": (
            "https://evidence.gda.internal/metadata-network-policy/run-42"
        ),
...
def test_checked_in_profile_is_valid_but_production_readiness_is_blocked():
    ...
    assert (
        "traffic_matrix.entries.gda-control-to-openmetadata-api"
        in report["profile_blockers"]
    )
...
def test_attestation_rejects_workload_identity_selector_and_admission_drift(
    ...
    if drift == "identity":
        ...
        profile["workloads"]["openmetadata"]["selector_labels"][
...
def test_traffic_matrix_rejects_missing_extra_and_changed_flows(tmp_path, drift):
    ...
    if drift == "missing":
        assert report["profile_valid"] is True
        assert any(
            blocker.startswith("traffic_matrix.entries.")
            for blocker in report["profile_blockers"]
        )
    else:
...
def test_report_integrity_rejects_tampering_and_production_overclaim():
    ...
    assert (
        "NetworkPolicy gate result is inconsistent: tenant_isolation_verified" in errors
    )
