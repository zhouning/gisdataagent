...
def test_restores_chainlit_cookie_oauth_openapi_model():
    class CookieOAuthWithoutModel(SecurityBase):
        def __init__(self):
            self.tokenUrl = "/login"
            self.scheme_name = "CookieOAuth"
            self.auto_error = False

        async def __call__(self, request: Request):
            return None

    security = CookieOAuthWithoutModel()
    app = FastAPI()

    @app.get("/secure")
    async def secure(_token: str | None = Depends(security)):
        return {"ok": True}

    assert ensure_chainlit_oauth_openapi_model(security)
    assert not ensure_chainlit_oauth_openapi_model(security)

    schema = app.openapi()
    oauth = schema["components"]["securitySchemes"]["CookieOAuth"]
    assert oauth["type"] == "oauth2"
    assert oauth["flows"]["password"]["tokenUrl"] == "/login"
