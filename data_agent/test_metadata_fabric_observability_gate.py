...
def test_attestation_source_revision_must_match_verification_target(tmp_path):
    profile_path = _write_profile(tmp_path, _complete_profile())
    attestation = _attestation(profile_path)
    ...
    assert report["production_observability_gate_passed"] is False
    assert any(
        "source revision does not match" in error
        for error in report["attestation_errors"]
    )
