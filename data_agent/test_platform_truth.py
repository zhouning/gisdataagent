...
def test_snapshot_comparison_reports_config_and_platform_drift():
    unchanged = {
        ...
        "environment_access": {"fingerprint": "environment-a"},
...
def test_platform_snapshot_binds_environment_access_baseline(tmp_path):
    source_root = tmp_path / "data_agent"
    source_root.mkdir()
    (source_root / "worker.py").write_text(
        'import os\nVALUE = os.environ.get("GDA_TEST_VALUE")\n',
        encoding="utf-8",
    )

    report = platform_truth.build_platform_snapshot(
        {}, profile="development", source_root=source_root
    )

    assert report["environment_access"]["accesses"] == {
        "GDA_TEST_VALUE": ["data_agent/worker.py"]
    }
    assert report["environment_access"]["matches_baseline"] is False
    assert len(report["platform_fingerprint"]) == 64
