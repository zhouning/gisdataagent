...
DEFAULT_SOURCE_EVIDENCE = REPO_ROOT / "docs/evidence/metadata-fabric-binding-ledger-2026-07-28.json"
DEFAULT_EVIDENCE_PATH = (
    REPO_ROOT / "docs/evidence/metadata-fabric-openlineage-delivery-2026-07-28.json"
)
DEFAULT_WRAPPER_PATH = REPO_ROOT / "scripts/metadata-fabric-openlineage-delivery.sh"
MIGRATIONS = tuple(
...
def _load_json_object(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise MetadataFabricLineageDeliveryError(f"{path.name} must contain an object")
    return payload
...
def build_lineage_delivery_bundle(
    ...
    if (
        source_evidence.get("binding_id") != str(binding_bundle.record.binding_id)
        or source_evidence.get("record_sha256") != binding_bundle.record.record_sha256
    ):
        raise MetadataFabricLineageDeliveryError(
            "M3-3 evidence does not match the deterministic binding"
        )
    source_plan = _build_source_plan()
    apply_plan = parse_metadata_fabric_execution_plan_artifact(binding_bundle.artifacts[0])
...
def build_contract_report(
    *,
    ...
    stable = {
        ...
        "source_evidence_sha256": (bundle.source_evidence_sha256 if bundle is not None else None),
        "delivery_id": (str(bundle.delivery.delivery_id) if bundle is not None else None),
        "event_sha256": (bundle.delivery.event_sha256 if bundle is not None else None),
        "idempotency_key": (bundle.delivery.idempotency_key if bundle is not None else None),
        "files": files,
        "errors": errors,
    }
    return {
...
def run_local_rehearsal(
    ...
    *,
    ...
    expected_body_sha = hashlib.sha256(
        canonical_json_bytes(bundle.delivery.event.model_dump(mode="json", by_alias=True))
    ).hexdigest()
    ...
    stable = {
        ...
        "status": ("local_wire_openlineage_delivery_verified" if verified else "blocked"),
        ...
        "stable_idempotency_header_verified": all(
            item["idempotency_key"] == bundle.delivery.idempotency_key for item in state.requests
        ),
        "receiver_commit_then_failed_ack_recovered": (
            first_batch.retry_pending == 1 and second_batch.delivered == 1 and duplicate_count == 1
        ),
...
def validate_rehearsal_evidence(
    ...
    *,
    ...
    try:
        bundle = build_lineage_delivery_bundle(source_evidence_path)
        success_body_sha256 = hashlib.sha256(
            canonical_json_bytes({"accepted": True, "duplicate": True})
        ).hexdigest()
        expected_values = {
            ...
            "idempotency_key": bundle.delivery.idempotency_key,
            "target_name": bundle.delivery.target_name,
            "first_enqueue_created": True,
            "replay_enqueue_created": False,
            "first_attempt_response_status": 503,
            "first_attempt_retry_pending": True,
            "final_response_status": 200,
            "final_attempt_count": 2,
            "receipt_sha256": openlineage_receipt_sha256(
                bundle.delivery,
                response_status=200,
                response_body_sha256=success_body_sha256,
            ),
            "wire_request_count": 2,
            "receiver_unique_accept_count": 1,
            "receiver_duplicate_count": 1,
            "wire_body_sha256": bundle.delivery.event_sha256,
            "local_loopback_receiver": True,
            "errors": [],
        }
        for field, expected in expected_values.items():
            if evidence.get(field) != expected:
                errors.append(f"lineage delivery evidence {field} does not match")
    except (KeyError, MetadataFabricLineageDeliveryError, OSError, ValueError):
        errors.append("lineage delivery source evidence is invalid")
    for claim in (
    ...
    if evidence.get("transport_semantics") != ("at_least_once_with_receiver_idempotency"):
...
def main(argv: list[str] | None = None) -> int:
    ...
    validate.add_argument("--source-evidence", type=Path, default=DEFAULT_SOURCE_EVIDENCE)
    ...
    rehearse.add_argument("--source-evidence", type=Path, default=DEFAULT_SOURCE_EVIDENCE)
    ...
    if args.command == "validate":
        ...
        except (MetadataFabricLineageDeliveryError, OSError, ValueError) as exc:
            report["errors"].append(f"lineage delivery evidence is invalid: {type(exc).__name__}")
