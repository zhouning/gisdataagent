...
EXPECTED_PLAN_SHA256 = (
    "a5c8ef636c03a38d0c6edaacff7d1edeba9c4b8a7f1491c493e9308257c5a94d"
)
EXPECTED_OPENLINEAGE_SHA256 = (
    "4929e51c4126e09415a9fc1578c9401077c5d7c374294e70deeebd29c8216dd2"
)
EXPECTED_REPLAY_SHA256 = (
    "c33857b2ae75f1106ed7d59e8e53296a3f76f4b90ef386238159b328d47c57ca"
)
...
def test_plan_binds_terminal_evidence_projection_ownership_and_openlineage():
    ...
    assert plan.projections[0].authority_fields == (
        ingestion.OPENMETADATA_AUTHORITY_FIELDS
    )
    assert plan.projections[1].authority_fields == (
        ingestion.GRAVITINO_AUTHORITY_FIELDS
    )
...
def test_openmetadata_projection_drift_blocks_replay(field):
    values = _inputs()
    plan = _plan(values)
    values["openmetadata"] = values["openmetadata"].model_copy(
        update={field: ("changed:value",)}
    )
    ...
    assert any(
        "projection_state_drift:openmetadata" in item for item in replay.blockers
    )
...
def test_changed_expected_fingerprint_fails_closed(tmp_path):
    expected = json.loads(
        ingestion.DEFAULT_EXPECTED_FIXTURE.read_text(encoding="utf-8")
    )
