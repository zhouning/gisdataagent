...
class LineageDeliveryStatus(str, Enum):
    PENDING = "pending"
    IN_FLIGHT = "in_flight"
    DELIVERED = "delivered"
    FAILED = "failed"
...
class MetadataFabricLineageDelivery(_FrozenModel):
    delivery_schema: Literal[
        "gda.metadata_fabric_openlineage_delivery.v1"
    ] = Field(default=DELIVERY_SCHEMA, alias="schema")
    tenant_id: TenantId
    delivery_id: UUID
    binding_id: UUID
    ...
    def _content_bound_delivery(self) -> Self:
        ...
        if (
            self.response_body_sha256 is not None
...
def validate_delivery_source(
    *,
    ...
    if (
        ...
        or apply_plan.resource_version_id
        != binding.binding.resource_version_id
