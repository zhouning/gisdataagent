...
class OpenLineageRunEvent(_FrozenModel):
    ...
    producer: Literal[
        "https://github.com/zhouning/gisdataagent/"
        "data_agent/metadata_fabric_ingestion.py"
    ] = OPENLINEAGE_PRODUCER
    run: OpenLineageRun
    job: OpenLineageJob
...
class MetadataFabricIngestionPlan(_FrozenModel):
    ...
    def _content_bound_plan(self) -> Self:
        ...
        if (
            ...
            or not self.openlineage_event.outputs[0].name.endswith(
                f"@{self.resource_version_id}"
            )
        ):
...
def _dataset_identity(version: ResourceVersion) -> OpenLineageDataset:
    ...
    return OpenLineageDataset(
        ...
        name=(
            f"{parsed['resource_kind']}/{parsed['resource_id']}"
            f"@{version.resource_version_id}"
        ),
    )
...
def _validate_terminal_evidence(
    *,
    ...
    if source.resource_version_id not in {
    ...
    *,
    ...
    if reconciliation.status != bridge.ReconciliationStatus.VERIFIED:
        raise MetadataFabricIngestionError(
            "provider observations do not match binding: "
            + ", ".join(reconciliation.blockers)
        )
    projections = (
        _projection(
    ...
    values["idempotency_key"] = canonical_json_fingerprint(
        _plan_idempotency_payload(values)
    )
    stable = {
...
def evaluate_replay(
    ...
    *,
    ...
    expected_targets = {
        (projection.provider, projection.target_identity)
    ...
    for projection in plan.projections:
        ...
        if _projection_state_sha256(state) != projection.desired_state_sha256:
            blockers.append(
                ...
                f"{projection.provider}:{projection.target_identity}"
            )
...
def _load_contract_inputs(
    ...
    if platform_report["status"] != "valid":
        raise MetadataFabricIngestionError(
            "platform golden fixture is invalid: "
            + ", ".join(platform_report["errors"])
        )
    ...
    metadata_target = ResourceVersion.model_validate(
        metadata_payload["resource_version"]
    )
    ...
    gravitino_refs = tuple(
        bridge.GravitinoTableRef.model_validate(item)
        for item in metadata_payload["gravitino_refs"]
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    validate.add_argument(
        "--platform-fixture", type=Path, default=DEFAULT_PLATFORM_FIXTURE
    )
    validate.add_argument(
        "--metadata-fixture", type=Path, default=DEFAULT_METADATA_FIXTURE
    )
