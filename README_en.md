...
- **Standards Platform Wave 1 → Wave 7**: Data standards upgraded from Word documents to **fully governable lifecycle assets** — collection / drafting / review / publication / derivation / data modeling. 16 `std_*` tables + standalone Outbox worker + ltree + pgvector(768) + StandardsTab dual sub-tabs + StandardsEditorAgent; 5 of 6 derivation strategies shipped (SemanticHint / ValueDomain / Synonym / **QcRule / DefectTaxonomy**), **derivation coverage 83% (5/6)**, shifting from scenario-driven to **standard-driven**. New rollback + impact-graph API (`POST /api/std/derive/rollback/{vid}` + `GET /api/std/impact/{kind}/{id}`). Migrations 067-084 all applied; 264 standards_platform tests passing.
...
| DB Migrations | 84 SQL migrations (latest 084 — `agent_defect_code_bindings`) |
| Standards Platform | 16 `std_*` tables + standalone Outbox worker + ltree + pgvector(768) + 6 derivation strategies (5/6 shipped: SemanticHint / ValueDomain / Synonym / QcRule / DefectTaxonomy) + rollback + impact graph + StandardsEditorAgent |
