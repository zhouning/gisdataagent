...
\begin{abstract}
% =============================================================================
Natural language interfaces to PostGIS must handle challenges that mainstream text-to-SQL benchmarks rarely exercise: geometry-aware schema grounding, OGC topological and metric predicates, coordinate reference systems, \texttt{::geography} casting, KNN operators, and safe refusal of destructive or unbounded queries. We present \textbf{NL2Semantic2SQL}, a bilingual semantic-grounding and safe-execution framework that couples a semantic layer (aliases, geometry types, SRIDs, spatial operator rules) with an intent-conditioned router, schema-aware prompting, a SQL postprocessor enforcing read-only safety and \texttt{LIMIT} injection on large-table previews, and an execution-time self-correction loop. We evaluate on a 125-question Chongqing PostGIS benchmark (85 Spatial + 40 Robustness across six safety categories). The 40-question Robustness suite is the headline primary result: Full reaches $0.975$ vs.\ baseline $0.450$ (paired McNemar $b=0$, $c=21$, $p<10^{-4}$), with bounded-answer compliance on OOM Prevention rising from $1/8$ to $7/8$. On Spatial we pool three independent Full runs, mean~$\pm$~SD EX$=0.663 \pm 0.048$ vs.\ baseline $0.529$; majority-vote $p=0.052$ (marginal). A secondary BIRD warehouse track shows design-set $+0.093$ ($p=0.0213$, in-sample tuning) but held-out $+0.033$ (n.s., $p=0.3833$); we report this conservatively. A bilingual human re-audit of a 50-question Chinese BIRD probe yields paired $p=1.00$, ruling out translation artefacts as the cause of the cross-lingual gap.
\end{abstract}
...
Natural language access to spatial databases is a long-standing GIScience problem~\citep{goodchild1992gis,egenhofer1991topological,clementini1993small}. Recent large language models have pushed general text-to-SQL forward on benchmarks such as Spider~\citep{yu2018spider} and BIRD~\citep{li2023bird}, yet these benchmarks deliberately abstract away the execution machinery that defines geospatial querying: the OGC Simple Feature Access predicates~\citep{ogcsfs}, coordinate reference system transformations, the \texttt{geometry}/\texttt{geography} type distinction, spatial indexes, KNN operators, and the safety constraints that any production GIS must enforce against destructive or unbounded queries. Natural language to executable PostGIS~\citep{postgis} (NL2GeoSQL) is therefore not an instance of text-to-SQL with a GIS dataset but a distinct problem shaped by spatial-database semantics.
...
encodes the executable subset of OGC Simple Feature Access~\citep{ogcsfs}
...
\item $I$ --- intent tags: \texttt{attribute\_filter}, \texttt{aggre\-gation},
...
\texttt{information\_schema.\allowbreak{}key\_column\_\allowbreak{}usage}),
...
Python module that the runtime agent consumes before each grounding call.\par}

\noindent\textbf{Executable subset, not a complete ontology.} $G$ is not a
complete geospatial ontology and is not a GeoSPARQL~\citep{ogcgeosparql}
...
\caption{Alignment of semantic-layer rules to OGC SFA (06-104r4), ISO 19107 clauses, and PostGIS functions. Rule-level mapping; each enforced rule traces to a specific clause.}
\label{tab:alignment}
...
We additionally compare against \textbf{DIN-SQL}~\citep{pourreza2023dinsql} as an external prompting baseline on both tracks. All configurations share the same underlying language model (Gemini~2.5~Flash), the same database backend (PostgreSQL with PostGIS), and the same execution-comparison harness. Experimental configurations are detailed in Supplement~S5.
...
\begin{table}[t]
\centering
\caption{GIS Spatial EX on 85 questions. Three independent Full runs, pooled mean~$\pm$~SD, and majority vote. Paired McNemar vs.\ baseline (two-sided exact binomial). ``Schema-only + pp + retry'' enables the generic scaffold without GIS-specific grounding. 95\% Wilson CIs in brackets.}
...
\begin{table}[t]
\centering
\caption{GIS Robustness on 40 safety/refusal questions, six categories. Paired McNemar $b=0$, $c=21$, $p<10^{-4}$. OOM Prevention reports bounded-answer compliance (``must contain \texttt{LIMIT}'').}
...
Table~\ref{tab:dinsql} compares NL2Semantic2SQL against DIN-SQL~\citep{pourreza2023dinsql} as an external prompting baseline on both tracks. DIN-SQL was re-implemented for PostgreSQL/PostGIS against the same execution harness and evaluated on the same GIS 85q Spatial and 15q Robustness pilot splits; the older BIRD 495q single-pass run is retained for token-cost continuity but is not directly comparable to the round-2 108q/150q results. Two paired DIN-SQL runs on the v5 evaluation track --- BIRD 150q held-out and the 40q Robustness suite --- are added at the end of this subsection and reported in Supplement~S4.

\begin{table}[t]
\centering\small
\caption{Three-way comparison: Baseline vs.\ DIN-SQL vs.\ Full. DIN-SQL on GIS re-implemented for PostgreSQL/PostGIS. The BIRD row is the older 495q comparison only; not directly comparable to the 108q/150q results. The GIS Spatial Full EX ($0.682$) is Sample~1 from the three-run protocol (cf.\ Section~\ref{sec:gis-results}); it is not the paper-wide primary headline.}
\label{tab:dinsql}
...
\midrule
GIS Spatial        & 85  & 0.529 & 0.565 & \textbf{0.682} (S1) & $+$0.118 ($p=0.021$) \\
GIS Robustness (pilot) & 15  & 0.333 & 0.267 & \textbf{0.800} & $+$0.533 ($p=0.008$) \\
...
On BIRD, the older 495q single-pass comparison clusters all three methods between $0.47$ and $0.51$; the full-vs-DIN-SQL difference is $+0.019$ and not statistically significant ($p=0.382$). On the v5 round-2 150-question held-out set (paired DIN-SQL vs.\ Full pipeline, same configuration as Section~\ref{sec:bird-results}), the Full pipeline reaches EX $0.507$ vs.\ DIN-SQL $0.440$, $b=18$, $c=8$, paired McNemar two-sided exact $p=0.0755$ (marginal); see the next paragraph and Supplement~S4 for the full table.

In summary, the full pipeline obtains statistically significant gains over both the baseline and DIN-SQL on the GIS Spatial split and the 15q Robustness pilot, while on the older 495q BIRD comparison and on the round-2 150q held-out comparison the directional gain is small and either non-significant or marginal. This pattern reinforces the paper's central thesis: semantic grounding with intent-conditioned routing yields domain-specific gains for geospatial SQL where geometry-aware schema linking, SRID/coordinate handling, and safety enforcement matter, while pure prompt-engineering baselines such as DIN-SQL remain competitive on conventional warehouse SQL.

\paragraph{DIN-SQL on the 150-question held-out and the 40-question Robustness suite.} Supplement~S4 reports two paired DIN-SQL comparisons run with the same external baseline implementation, the same Gemini~2.5~Flash backbone, and the same execution-comparison harness. On BIRD 150q held-out, the Full pipeline reaches EX $0.507$ vs.\ DIN-SQL $0.440$, with discordant pairs $b=18$ vs.\ $c=8$ and paired McNemar two-sided exact $p=0.0755$ (marginal at $\alpha=0.05$). On the 40-question Robustness suite, the Full pipeline reaches EX $0.975$ vs.\ DIN-SQL $0.275$, with $b=28$ vs.\ $c=0$ and paired McNemar two-sided exact $p=7.45\times10^{-9}$; the gap concentrates on safety/refusal categories (Security Rejection, Anti-Illusion, OOM Prevention) where DIN-SQL has no built-in handling.
...
\begin{table}[t]
\centering
\caption{Error breakdown for the full pipeline on BIRD (108q design + 150q held-out). Validity $\geq 0.99$ on both sets; dominant failure mode is wrong-result valid SQL.}
\label{tab:errors}
\begin{tabular}{lcc}
\toprule
...
\begin{table}[t]
\centering
\caption{Agent-loop-native ablation on GIS Spatial 85q. Each row disables one component of Full Sample~1 (EX~$=0.706$). Paired McNemar vs.\ Full Sample~1 (two-sided exact binomial).}
\label{tab:ablation}
\begin{tabular}{lccccc}
\toprule
...
\paragraph{Cross-model-family check.} Supplement~S3 reports a $2{\times}2$ cross-family factorial on the full 85-question Spatial set (Gemini~2.5~Flash vs.\ DeepSeek-V3, $N{=}3$ per cell, unified model gateway). Schema-only baseline parity between the two families ($0.529$ EX each, $b{=}c{=}0$, $p{=}1.000$) rules out an a-priori capability-gap confound. Within-family Gemini reproduces the main-text headline ($+0.129$ EX, $p=0.052$); the same full-pipeline stack does not transfer to DeepSeek (within-family $-0.047$, $p=0.597$, null; cross-family full $+0.176$ in Gemini's favour, $p=0.008$). The grounding rules were error-attributed against Gemini traces, so we report this as an honest negative portability finding and scope family-adapted grounding rules as future work.
...
\centering
\footnotesize
\caption{Paired McNemar tests (two-sided exact binomial). $b$ = base-OK/full-ERR, $c$ = base-ERR/full-OK. P = primary claim; D = development-set tuning; E = exploratory.}
\label{tab:mcnemar}
...
DIN-SQL vs.\ Full, BIRD 150q held-out & 150 & 18 & 8 & 0.0755 & E & Paired external baseline; marginal \\
DIN-SQL vs.\ Full, GIS Robustness (40q) & 40  & 28 & 0  & \textbf{$7.45\times10^{-9}$} & E & Paired external baseline; highly significant \\
\bottomrule
\end{tabular}
\end{table}
...
\end{tikzpicture}
\caption{Cross-track EX summary. GIS Spatial 85q: full $0.682$ (Sample~1, stochastic; primary headline is majority-vote $0.659$, cf.\ Section~\ref{sec:gis-results}) vs.\ DIN-SQL $0.565$ vs.\ baseline $0.529$ ($p=0.0072$ full vs.\ baseline; $p=0.0213$ full vs.\ DIN-SQL). BIRD design (108q): $0.593$ vs.\ $0.500$ ($p=0.0213$). BIRD held-out (150q): $0.507$ vs.\ $0.473$ ($p=0.3833$, NS).}
\label{fig:cross}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Cross-lingual evaluation. BIRD Chinese: 50 QIDs from the 108-question design set, translated to Chinese. GIS Chinese: native Chinese questions.}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{Cross-model-family generalisation.} The $2{\times}2$ cross-family factorial (Supplement~S3) shows that the Gemini-tuned grounding stack does not transfer to DeepSeek-V3 at the same prompt budget; family-adapted grounding rules, and extending the comparison to additional open- and closed-weight model families, are the natural next step.
\end{enumerate}
...
% =============================================================================
The 125-question GIS benchmark (85 Spatial + 40 Robustness), the 108q BIRD design set and 150q held-out qid lists, the cross-lingual 100q pairs, the semantic-layer YAML, the SQL postprocessor, the DIN-SQL runner, the evaluation harness, and per-question predictions are released in an anonymised companion repository (Supplement~S5).
