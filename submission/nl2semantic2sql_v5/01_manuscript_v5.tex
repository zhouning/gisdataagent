...
\paragraph{Cross-model-family check.} To test whether the ablation signal is specific to Gemini~2.5~Flash, Supplement~S3 reports a baseline-only cross-family ablation on a $30$-question stratified subset of GIS Spatial. Within-family, grounding plus intent-conditioned routing lift Gemini EX from $0.600$ to $0.800$ ($b=0$, $c=6$, paired exact $p=0.0312$), directionally consistent with the agent-loop component ablation in Table~\ref{tab:ablation}. Cross-family at the schema-only floor, DeepSeek-V4-Flash matches Gemini~2.5~Flash at $0.600$ EX on the same 30 items ($b=c=2$, paired exact $p=1.000$), so the within-family Gemini gain is not explained by an a-priori capability gap between the two families on this benchmark. The fully crossed DeepSeek-full row is not reported here: the full pipeline relies on ADK's \texttt{LlmAgent} agent loop, which is currently Gemini-coupled, and a model-agnostic agent loop together with a larger cross-family comparison across additional open- and closed-weight families is scheduled as future work.
