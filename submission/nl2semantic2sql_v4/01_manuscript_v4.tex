...
The cross-lingual evaluation (Section~\ref{sec:cross-lingual}) reports a $10$-point EX drop from English to Chinese BIRD ($0.660 \to 0.560$) on paired QIDs and strong $0.820$ EX on 50 Chinese GIS questions. Validity remains $0.980$, so failures are row-set mismatches from lexical drift rather than malformed SQL. A bilingual human re-audit of all 50 Chinese BIRD translations (18~\textsc{ok}, 32~\textsc{fix}, 0~\textsc{drop}) and a paired re-run of the v4 pipeline on the reviewed set yielded $0.540$ post-review vs.\ $0.560$ pre-review (paired McNemar $p=1.00$; see Table~\ref{tab:crosslingual-rereview}), which rules out LLM-translation artefacts as the explanation for the English$\to$Chinese gap. The residual degradation therefore reflects genuine cross-lingual schema-linking and reasoning difficulty, and is the scope for a future bilingual grounding round rather than a translation-pipeline issue.
