...
The sole use we make of this table is the domain-scoping observation: all six single-pass configurations lie within $\pm 0.016$ on Overall EX, and removing either the round-2 rules or the multi-hop warehouse join hints slightly \emph{improves} the single-pass GIS score. These warehouse-tuned rules do not help GIS accuracy and should be selectively disabled in the spatial path. We make no stronger claim; in particular we do not attribute any specific fraction of the agent-loop $+0.153$ Spatial gain to individual components. An agent-loop-native ablation is left to future work.
...
On the BIRD Chinese track, the $0.100$ EX degradation concentrates on moderate/challenging items where domain terminology (e.g., ``APS diagnosis'', ``urea nitrogen'') translates into less specific Chinese phrases, weakening schema linking. Validity remains high ($0.980$); failures are primarily row-set mismatches from value-translation drift. On the GIS Chinese track, EX$=0.820$ exceeds the English-evaluated GIS 85-question Spatial result from Section~\ref{sec:gis-results}, confirming the framework does not require English input to achieve strong GIS performance. Limitations: the Chinese BIRD questions were produced by LLM translation, so translation artefacts cannot be ruled out; translation-loss and cross-lingual-reasoning effects are not isolated. Future work should add no-alias and translate-back-to-English baselines and human-verified gold questions. Task~15 of the accompanying revision refreshes these numbers against a human-reviewed subset.
...
The expanded benchmark confirms what the pilot could not substantiate: semantic grounding with intent-conditioned routing provides a reliable advantage for geospatial SQL (Spatial EX $+0.153$, $p=0.0072$). Three mechanisms drive it. \emph{Geometry-aware type injection} ensures the model knows which columns carry geometry, their SRID, and when geography casting is required; without this the baseline omits \texttt{::geography} casts and returns area/distance in degrees rather than metres (this drives the $+0.305$ Medium-difficulty gain). \emph{Intent-conditioned operator routing} gates domain-specific rules (KNN \texttt{<->}, \texttt{LIMIT} injection) to the queries where they apply. \emph{Safety and robustness enforcement} through SQL postprocessing catches dangerous operations and refusal cases the baseline LLM does not guard against. The transition from $p=0.125$ on a 20-question pilot to $p=0.0072$ on 85 Spatial questions reflects a power issue, not a new effect. We do not make an analogous statistical claim on the 40-question Robustness expansion because baseline and DIN-SQL were not paired-evaluated on the expanded categories (see Table~\ref{tab:gis-robust}). We also do \emph{not} claim that any particular component explains a specific fraction of the $+0.153$ Spatial gain: Table~\ref{tab:ablation} runs under the single-pass configuration.
...
The cross-lingual evaluation (Section~\ref{sec:cross-lingual}) reports a $10$-point EX drop from English to Chinese BIRD ($0.660 \to 0.560$) on paired QIDs and strong $0.820$ EX on 50 Chinese GIS questions. Validity remains $0.980$, so failures are row-set mismatches from lexical drift rather than malformed SQL. Because the Chinese BIRD questions were LLM-translated, translation artefacts are not excluded; human-verified Chinese BIRD questions would be needed to isolate translation-loss from cross-lingual-reasoning effects.
