#!/usr/bin/env python3
"""Run a bounded Spark SQL MERGE with a complex AND/OR/IN match predicate."""
...
def _baseline(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if spark.catalog.tableExists(args.table):
        raise RuntimeError("isolated complex-predicate SQL MERGE table already exists")
    ...
    (
        ...
        .tableProperty("gda.acceptance", "spark-sql-merge-complex-predicate")
...
def _merge(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(before) != 2 or before[-1]["snapshot_id"] != args.flink_snapshot_id:
        raise RuntimeError("complex-predicate SQL MERGE did not start from Flink snapshot")
    if list(current) != plan["after_flink_rows"]:
        raise RuntimeError("complex-predicate SQL MERGE did not read exact Flink state")
    ...
    checks = {
        ...
        "guard_token_absent": not any(
            row["commit_token"] == guard["commit_token"] for row in actual
        ),
...
def main() -> int:
    ...
    try:
        ...
        spark.stop()
    report = {
        "schema": "gda.spark_flink_iceberg_sql_merge_complex_predicate_phase.v1",
