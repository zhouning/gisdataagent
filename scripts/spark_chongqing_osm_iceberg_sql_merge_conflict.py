#!/usr/bin/env python3
"""Certify snapshot-bound Spark SQL MERGE conflict rejection and fresh retry."""
...
BARRIER_RE = re.compile(
    ...
    r"flink_iceberg_sql_merge_conflict_[0-9a-f]{10}/spark-ready\.json$"
)
...
def _validate_barriers(args: argparse.Namespace) -> None:
    if not args.ready_marker or not args.release_marker:
        raise ValueError("concurrent SQL MERGE requires both barrier paths")
    if not BARRIER_RE.fullmatch(args.ready_marker.as_posix()):
        raise ValueError("unsafe SQL MERGE barrier path")
    ...
    if args.release_marker.as_posix() != args.ready_marker.as_posix().replace(
        ...
        raise ValueError("SQL MERGE release marker must share the acceptance work directory")
...
def _baseline(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if spark.catalog.tableExists(args.table):
        raise RuntimeError("isolated SQL MERGE table already exists")
    ...
    (
        ...
        .tableProperty("gda.acceptance", "spark-sql-merge-conflict")
...
def _wait_partition(rows, *, marker: Path, release: Path, payload: dict[str, Any], timeout: int):
    ...
    if not release.is_file():
        raise RuntimeError("SQL MERGE barrier timed out")
    yield from values


def _merge_source(spark, args: argparse.Namespace, source: dict[str, Any], *, wait: bool) -> None:
    frame = spark.createDataFrame([source], schema=_merge_schema()).repartition(1)
    if wait:
...
def _run_merge(
    spark, args: argparse.Namespace, source: dict[str, Any], *, wait: bool
) -> dict[str, Any]:
    _merge_source(spark, args, source, wait=wait)
...
def _concurrent_merge(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    result = _run_merge(spark, args, plan["merge_source_stale"], wait=True)
    ...
    checks = {
        **result["checks"],
        ...
        == [1, 2],
        "release_marker_exact": release.get("schema") == "gda.spark_sql_merge_conflict_release.v1"
...
def main() -> int:
    ...
    try:
        ...
        spark.stop()
    report = {
        "schema": "gda.spark_flink_iceberg_sql_merge_conflict_phase.v1",
