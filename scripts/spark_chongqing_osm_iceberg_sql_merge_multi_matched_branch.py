#!/usr/bin/env python3
"""Run a bounded Spark SQL MERGE with delete and update matched branches."""
...
def _baseline(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    (
        ...
        .tableProperty("gda.acceptance", "spark-sql-merge-multi-matched-branch")
...
def _merge(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    spark.createDataFrame(source, schema=_merge_schema()).createOrReplaceTempView(
        "gda_sql_merge_multi_matched_branch_source"
    )
    spark.sql(
        f"""
        MERGE INTO {args.table} AS target
        USING gda_sql_merge_multi_matched_branch_source AS source
...
def main() -> int:
    ...
    try:
        ...
        spark.stop()
    report = {
        "schema": "gda.spark_flink_iceberg_sql_merge_multi_matched_branch_phase.v1",
