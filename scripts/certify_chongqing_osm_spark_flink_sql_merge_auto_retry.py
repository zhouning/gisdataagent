#!/usr/bin/env python3
"""Certify automatic fresh-state retry after a duplicate-source-row SQL MERGE rejection."""
...
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_sql_merge_auto_retry.py"
SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_sql_merge_auto_retry"
...
DEFAULT_REPORT = (
    ...
    "chongqing-osm-spark-flink-sql-merge-auto-retry-report.json"
)


def build_sql_merge_auto_retry_plan(source_path: Path) -> dict[str, Any]:
    ...
    stale_a_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "merge-duplicate-source-a",
    ...
    stale_b_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "merge-duplicate-source-b",
            "target_road_id": base["target_road_id"],
            "source_sha256": base["source"]["source_parquet_sha256"],
            "baseline_content_sha256": base["baseline_content_sha256"],
        }
    ...
    fresh_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "merge-deduplicated-fresh-flink-revision",
            "target_road_id": base["target_road_id"],
    ...
    stale_rows = [
        {
            **stale_common,
            "new_revision": 2,
            "writer_engine": "spark-sql-merge-duplicate-a",
            "commit_token": stale_a_token,
            "source_row_id": "stale-source-a",
        },
        {
            **stale_common,
            "new_revision": 99,
            "writer_engine": "spark-sql-merge-duplicate-b",
            "commit_token": stale_b_token,
            "source_row_id": "stale-source-b",
        },
    ]
    ...
    fresh = {
        ...
        "writer_engine": "spark-sql-merge",
        "commit_token": fresh_token,
        "source_row_id": "fresh-source-deduplicated",
    }
    fresh_row = {
        "road_id": flink["road_id"],
        "revision": 3,
    ...
    return {
        **base,
        "schema": "gda.chongqing_osm_spark_flink_sql_merge_auto_retry_plan.v1",
        "merge_source_stale_rows": stale_rows,
        "stale_source_row_ids": [row["source_row_id"] for row in stale_rows],
        "merge_source_fresh": fresh,
        "final_merge_rows": final_rows,
        "final_merge_content_sha256": _canonical_sha256(final_rows),
        "sql_merge_stale_token": _canonical_sha256([stale_a_token, stale_b_token]),
        "sql_merge_stale_tokens": [stale_a_token, stale_b_token],
        "sql_merge_fresh_token": fresh_token,
    }
...
def _spark_command(
    ...
    *,
    ...
    if commit_marker:
        command.extend(
            ("--commit-marker", f"/workspace/{commit_marker.relative_to(REPO_ROOT).as_posix()}")
        )
    return command
...
def _spark_phase(args: argparse.Namespace, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        _run_command(
            ...
            stage=f"Spark SQL MERGE {kwargs['phase']}",
    ...
    if not report_path.exists():
        if command_error is not None:
            raise command_error
        raise RuntimeError(f"Spark SQL MERGE {kwargs['phase']} did not write evidence")
    report = json.loads(report_path.read_text(encoding="utf-8"))
    if report.get("status") != "passed" or report.get("phase") != kwargs["phase"]:
        raise RuntimeError(
            f"Spark SQL MERGE {kwargs['phase']} returned failed evidence: "
            f"checks={report.get('checks')}, "
            f"first_retry_rows={report.get('first_retry_rows')}, "
            f"expected_first_retry_rows={report.get('expected_first_retry_rows')}, "
            f"command_error={command_error}"
        )
    return report
...
def _remove_keys(client: Any, keys: set[str], *, prefix: str) -> dict[str, Any]:
    ordered = sorted(keys)
    for key in ordered:
        client.delete_object(Bucket=BUCKET, Key=key)
    return {"keys_removed": len(ordered), "remaining_keys": len(_keys(client, prefix))}
...
def _cleanup_prefix_safe(client: Any, prefix: str) -> dict[str, Any]:
    ...
    return {
        ...
        "object_prefix_empty": not remaining,
    }
...
def main() -> int:
    ...
    parser.add_argument(
        "--budget-only",
        action="store_true",
        help="run the bounded retry-budget fail-closed phase instead of a successful fresh retry",
    )
    ...
    work_dir = (
        ...
        / f"flink_iceberg_sql_merge_auto_retry_{token}"
    )
    plan_path = work_dir / "plan.json"
    baseline_path = work_dir / "spark-baseline.json"
    budget_path = work_dir / "spark-budget.json"
    auto_retry_path = work_dir / "spark-auto-retry.json"
    verify_path = work_dir / "spark-verify.json"
    ready_marker = work_dir / "spark-ready.json"
    release_marker = work_dir / "spark-release.json"
    spark_container = f"gda-iceberg-spark-sql-merge-auto-retry-{token}"
    client = boto3.client(
        "s3",
    ...
    try:
        ...
        phase = "budget-exhausted" if args.budget_only else "auto-retry"
        phase_path = budget_path if args.budget_only else auto_retry_path
        coordination_rows = (
            plan["retry_budget_rows"] if args.budget_only else plan["merge_source_stale_rows"]
        )
        ...
        spark_process = subprocess.Popen(
            spark_command, cwd=REPO_ROOT, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
        )
            ...
            json.dumps(
                {
                    ...
                    "source_row_count": len(coordination_rows),
                    "source_row_ids": [row["source_row_id"] for row in coordination_rows],
                    "expected_revision": coordination_rows[0]["expected_revision"],
                },
                sort_keys=True,
                separators=(",", ":"),
            )
        ...
        phase_report = json.loads(phase_path.read_text(encoding="utf-8"))
        if spark_process.returncode != 0 or phase_report.get("status") != "passed":
            raise RuntimeError(
                "automatic SQL MERGE retry did not converge: "
                f"returncode={spark_process.returncode}, "
                f"checks={phase_report.get('checks')}, "
        ...
        if args.budget_only:
            auto_retry = phase_report
            verify = {
                "phase": "budget-verify",
                "status": "passed",
                "checks": {
                    "budget_phase_passed": all(auto_retry["checks"].values()),
                    "current_snapshot_is_flink_child": final_metadata["current_snapshot"][
                        "snapshot_id"
                    ]
                    == flink_snapshot_id,
                },
        ...
        checks = {
            ...
            == "true",
            "spark_sql_merge_auto_retry_ready_marker_exact": ready
            == {
                "schema": "gda.spark_sql_merge_auto_retry_ready.v1",
                "baseline_snapshot_id": baseline["baseline_snapshot_id"],
                "source_row_count": len(coordination_rows),
                "source_row_ids": [row["source_row_id"] for row in coordination_rows],
                "target_road_id": coordination_rows[0]["road_id"],
                "expected_revision": coordination_rows[0]["expected_revision"],
            },
            ...
            "automatic_cardinality_rejection_and_retry_passed": all(auto_retry["checks"].values()),
            "retry_budget_fail_closed_passed": not args.budget_only
            or auto_retry["phase"] == "budget-exhausted",
            "independent_multi_source_sql_merge_verify_passed": all(verify["checks"].values()),
            "final_snapshot_child_of_flink": (
                final_metadata["current_snapshot"]["parent_id"]
                == auto_retry["snapshots"][-2]["snapshot_id"]
                if not args.budget_only
                else final_metadata["current_snapshot"]["snapshot_id"] == flink_snapshot_id
            ),
            "sql_merge_auto_retry_object_graph_materialized": inventory["metadata_json_count"]
            >= (2 if args.budget_only else 3)
            and inventory["manifest_avro_count"] >= 4
            and inventory["data_parquet_count"] >= (4 if args.budget_only else 5),
        }
        report = {
            "schema": "gda.chongqing_osm_spark_flink_sql_merge_auto_retry.acceptance.v1",
            ...
            "source": {
                **plan["source"],
                ...
                "stale_tokens": plan["sql_merge_stale_tokens"],
                "fresh_token": plan["sql_merge_fresh_token"],
            },
            "runtime": {
            ...
            "control_plane": {
                ...
                "retry_mode": (
                    "bounded_retry_budget_fail_closed"
                    if args.budget_only
                    else "automatic_fresh_state_after_cardinality_rejection"
                ),
            },
            "not_claimed": [
                "SQL UPDATE semantics beyond this single-key bounded MERGE",
                (
                    "MERGE with deletes, inserts, partition evolution, "
                    "equality/position delete or MOR"
                ),
                (
                    "streaming checkpoint writers, REST or Gravitino catalog, HA, Kubernetes, "
                    "production SLO/RPO/RTO, or production recovery-controller semantics"
                ),
            ],
        }
        if report["status"] != "passed":
            raise RuntimeError(f"automatic SQL MERGE retry checks failed: {checks}")
    except Exception as exc:
