#!/usr/bin/env python3
"""Certify stale Spark overwrite rejection and a fresh retry after Flink append."""
...
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_overwrite_conflict.py"
SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_overwrite_conflict"
DEFAULT_REPORT = (
    REPO_ROOT / ".tmp/source-sync-certification/"
    "chongqing-osm-spark-flink-overwrite-conflict-report.json"
)


def build_overwrite_conflict_plan(source_path: Path) -> dict[str, Any]:
    append = build_concurrent_append_plan(source_path)
    baseline = [dict(row) for row in append["baseline_rows"]]
    after_flink = [dict(row) for row in append["after_flink_rows"]]
    target_id = baseline[0]["road_id"]
    target_revision = baseline[0]["revision"] + 1
    spark_token = _canonical_sha256(
        {
            "engine": "spark-3.5",
            "operation": "full-table-overwrite",
    ...
    def apply_update(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
        updated = []
        for row in rows:
            if row["road_id"] == target_id:
                updated.append(
                    {
                        **row,
                        "revision": target_revision,
                        "writer_engine": "spark-3.5-overwrite",
                        "commit_token": spark_token,
                    }
                )
            else:
                updated.append(dict(row))
        return sorted(updated, key=lambda row: row["road_id"])

    stale_overwrite = apply_update(baseline)
    final = apply_update(after_flink)
    if len(stale_overwrite) != 3 or len(final) != 4:
        raise RuntimeError("overwrite conflict plan has invalid bounds")
    if not any(row["commit_token"] == append["flink_commit_token"] for row in final):
        raise RuntimeError("fresh overwrite retry would lose the Flink row")
    return {
        "schema": "gda.chongqing_osm_spark_flink_overwrite_conflict_plan.v1",
        "source": append["source"],
        "baseline_rows": baseline,
        "flink_row": dict(append["flink_row"]),
        "after_flink_rows": after_flink,
        "stale_overwrite_rows": stale_overwrite,
        "final_rows": final,
        "target_road_id": target_id,
        ...
        "stale_overwrite_content_sha256": _canonical_sha256(stale_overwrite),
...
def _spark_phase(args: argparse.Namespace, **kwargs: Any) -> dict[str, Any]:
    ...
    _run_command(
        ...
        stage=f"Spark overwrite conflict {phase}",
        timeout=args.timeout_seconds,
    )
    report = json.loads(report_path.read_text(encoding="utf-8"))
    if report.get("status") != "passed" or report.get("phase") != phase:
        raise RuntimeError(f"Spark overwrite conflict {phase} returned failed evidence")
    return report
...
def main() -> int:
    ...
    work_dir = (
        REPO_ROOT / ".tmp/source-sync-certification" / f"flink_iceberg_overwrite_conflict_{token}"
    )
    ...
    try:
        ...
        plan = build_overwrite_conflict_plan(args.source)
        ...
        if not conflict_path.is_file():
            ...
            raise RuntimeError(f"Spark overwrite conflict emitted no evidence: {output}")
        conflict = json.loads(conflict_path.read_text(encoding="utf-8"))
        if spark_process.returncode != 0 or conflict.get("status") != "passed":
            raise RuntimeError(
                "Spark stale overwrite was not rejected safely: "
        ...
        checks = {
            ...
            "spark_overwrite_in_flight_before_flink_commit": (
                ready_exact
            ...
            "stale_spark_overwrite_failed_closed": all(conflict["checks"].values()),
            ...
            "fresh_state_retry_preserved_flink_write": all(retry["checks"].values()),
            ...
            "conflict_object_graph_materialized": (
        ...
        report = {
            "schema": "gda.chongqing_osm_spark_flink_overwrite_conflict.acceptance.v1",
            ...
            "source": {
                **plan["source"],
                ...
                "stale_overwrite_content_sha256": plan["stale_overwrite_content_sha256"],
            ...
            "not_claimed": [
                "concurrent delete, row-level update, or merge conflict isolation",
        ...
        if report["status"] != "passed":
            raise RuntimeError(f"overwrite conflict checks failed: {checks}")
