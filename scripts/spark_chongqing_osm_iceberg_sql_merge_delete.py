#!/usr/bin/env python3
"""Run a bounded Spark SQL MERGE with a matched-delete branch."""
...
def _baseline(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if spark.catalog.tableExists(args.table):
        raise RuntimeError("isolated SQL MERGE delete table already exists")
    ...
    (
        ...
        .tableProperty("gda.acceptance", "spark-sql-merge-delete")
...
def _merge(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(before) != 2 or before[-1]["snapshot_id"] != args.flink_snapshot_id:
        raise RuntimeError("SQL MERGE delete did not start from Flink snapshot")
    if list(current) != plan["after_flink_rows"]:
        raise RuntimeError("SQL MERGE delete did not read exact Flink state")
    source = [plan["delete_source_row"]]
    spark.createDataFrame(source, schema=_merge_schema()).createOrReplaceTempView(
        "gda_sql_merge_delete_source"
    )
    spark.sql(
        f"""
        MERGE INTO {args.table} AS target
        USING gda_sql_merge_delete_source AS source
    ...
    actual = _rows(spark.table(args.table))
    snapshots = _snapshots(spark, args.table)
    deleted = plan["delete_source_row"]
    checks = {
        "merge_committed": True,
        "matched_target_revision_two_removed": not any(
            row["road_id"] == deleted["road_id"] and row["revision"] == deleted["expected_revision"]
            for row in actual
        ),
        "baseline_revision_one_preserved": any(
            row["road_id"] == deleted["road_id"] and row["revision"] == 1 for row in actual
        ),
        ...
        and snapshots[-1]["operation"] in {"overwrite", "delete"},
    }
    return {
        "phase": "merge",
        "status": "passed" if all(checks.values()) else "failed",
        ...
        "content_sha256": _canonical_sha256(actual),
        "snapshots": snapshots,
        "deleted_target_road_id": deleted["road_id"],
...
def _verify(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "deleted_revision_two_absent": not any(
            row["road_id"] == deleted["road_id"] and row["revision"] == deleted["expected_revision"]
            for row in actual
        ),
        "baseline_revision_one_survives": any(
            row["road_id"] == deleted["road_id"] and row["revision"] == 1 for row in actual
        ),
        ...
        "final_row_count_is_three": len(actual) == 3,
    }
    return {
        "phase": "verify",
        "status": "passed" if all(checks.values()) else "failed",
...
def main() -> int:
    ...
    try:
        ...
        spark.stop()
    report = {
        "schema": "gda.spark_flink_iceberg_sql_merge_delete_phase.v1",
