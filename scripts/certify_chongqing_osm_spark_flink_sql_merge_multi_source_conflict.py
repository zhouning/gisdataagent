#!/usr/bin/env python3
"""Certify duplicate-source-row Spark SQL MERGE isolation against a Flink append."""
...
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_sql_merge_multi_source_conflict.py"
SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_sql_merge_multi_source_conflict"
...
DEFAULT_REPORT = (
    ...
    "chongqing-osm-spark-flink-sql-merge-multi-source-conflict-report.json"
)


def build_sql_merge_multi_source_conflict_plan(source_path: Path) -> dict[str, Any]:
    ...
    return {
        **base,
        "schema": "gda.chongqing_osm_spark_flink_sql_merge_multi_source_conflict_plan.v1",
...
def _spark_phase(args: argparse.Namespace, **kwargs: Any) -> dict[str, Any]:
    ...
    if report.get("status") != "passed" or report.get("phase") != kwargs["phase"]:
        raise RuntimeError(f"Spark SQL MERGE {kwargs['phase']} returned failed evidence")
    return report
...
def main() -> int:
    ...
    work_dir = (
        ...
        / f"flink_iceberg_sql_merge_multi_source_conflict_{token}"
    )
    ...
    spark_container = f"gda-iceberg-spark-sql-merge-multi-source-{token}"
    client = boto3.client(
        "s3",
    ...
    try:
        ...
        plan = build_sql_merge_multi_source_conflict_plan(args.source)
        ...
        release_marker.write_text(
            json.dumps(
                {
                    "schema": "gda.spark_sql_merge_multi_source_conflict_release.v1",
        ...
        if spark_process.returncode != 0 or conflict.get("status") != "passed":
            raise RuntimeError(
                "SQL MERGE conflict was not rejected: "
                f"returncode={spark_process.returncode}, "
                f"checks={conflict.get('checks')}, "
        ...
        after_conflict_keys = _keys(client, prefix)
        orphan_keys = after_conflict_keys - after_flink_keys
        ...
        checks = {
            ...
            == "true",
            "spark_sql_merge_multi_source_ready_marker_exact": ready
            == {
            ...
            "stale_multi_source_sql_merge_rejected": all(conflict["checks"].values()),
            "catalog_unchanged_after_stale_merge": after_conflict_location == after_flink_location
            and after_conflict_metadata == after_flink_metadata,
            "stale_multi_source_merge_orphans_explicitly_cleaned": orphan_cleanup["keys_removed"]
            == len(orphan_keys)
            and orphan_cleanup["remaining_keys"] == len(after_flink_keys),
            "fresh_deduplicated_sql_merge_retry_passed": all(retry["checks"].values()),
            "independent_multi_source_sql_merge_verify_passed": all(verify["checks"].values()),
            "final_snapshot_child_of_flink": final_metadata["current_snapshot"]["parent_id"]
            == flink_snapshot_id,
            "sql_merge_multi_source_object_graph_materialized": inventory["metadata_json_count"]
            >= 3
            and inventory["manifest_avro_count"] >= 4
            and inventory["data_parquet_count"] >= 5,
        }
        report = {
            "schema": "gda.chongqing_osm_spark_flink_sql_merge_multi_source_conflict.acceptance.v1",
            ...
            "table": {
                ...
                "stale_multi_source_sql_merge": conflict,
            ...
            "control_plane": {
                ...
                "retry_mode": "explicit_deduplicated_fresh_state_sql_merge",
            },
            "not_claimed": [
        ...
        if report["status"] != "passed":
            raise RuntimeError(f"multi-source SQL MERGE checks failed: {checks}")
