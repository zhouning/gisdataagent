#!/usr/bin/env python3
"""Certify a real Spark SQL MERGE with delete and update matched branches."""
...
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_sql_merge_multi_matched_branch.py"
SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_sql_merge_multi_matched_branch"
...
DEFAULT_REPORT = (
    ...
    "chongqing-osm-spark-flink-sql-merge-multi-matched-branch-report.json"
)
...
def build_sql_merge_multi_matched_branch_plan(source_path: Path) -> dict[str, Any]:
    ...
    delete_token = _canonical_sha256(
        {
            ...
            "operation": "merge-matched-delete-branch",
    ...
    update_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "merge-matched-update-branch",
    ...
    matched = [delete_source, update_source]
    final_rows = [
        row
    ...
    return {
        **base,
        "schema": "gda.chongqing_osm_spark_flink_sql_merge_multi_matched_branch_plan.v1",
...
def main() -> int:
    ...
    work_dir = (
        ...
        / f"flink_iceberg_sql_merge_multi_matched_branch_{token}"
    )
    ...
    spark_container = f"gda-iceberg-spark-sql-merge-multi-matched-branch-{token}"
    client = boto3.client(
        "s3",
    ...
    try:
        ...
        plan = build_sql_merge_multi_matched_branch_plan(args.source)
        ...
        checks = {
            ...
            "sql_merge_multi_matched_branches_passed": all(merge["checks"].values()),
            ...
            "multi_matched_branch_object_graph_materialized": inventory["metadata_json_count"] >= 3
            and inventory["manifest_avro_count"] >= 4
            and inventory["data_parquet_count"] >= 5,
        }
        report = {
            "schema": "gda.chongqing_osm_spark_flink_sql_merge_multi_matched_branch.acceptance.v1",
            ...
            "control_plane": {
                ...
                "merge_branches": ["matched_delete_when_action_delete", "matched_update_default"],
