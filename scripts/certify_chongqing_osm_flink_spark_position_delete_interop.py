...
CONFLICT_RE = re.compile(
    r"GDA_POSITION_DELETE_CONFLICT_REJECTED baseline=(\d+) current=(\d+) "
    r"target_road_id=(\d+) token=([0-9a-f]{64}) orphan_cleanup=(true|false)"
)
...
def build_flink_position_delete_plan(source_path: Path) -> dict[str, Any]:
    ...
    stale_token = _canonical_sha256(
        {
            "engine": "flink-1.19.3-iceberg-1.7.2",
            "operation": "stale-position-delete-conflict-probe",
            "target_road_id": plan["target_road_id"],
            "baseline_content_sha256": plan["baseline_content_sha256"],
            "source_sha256": plan["source"]["source_parquet_sha256"],
        }
    ...
    return {
        **plan,
        ...
        "stale_flink_commit_token": stale_token,
    }
...
def _run_flink_position_delete(
    ...
    *,
    ...
    token = commit_token or plan["flink_commit_token"]
    command = [
        "docker",
        "exec",
        flink.container,
    ...
    completed = _run_command(
        command,
        stage=(
            "run stale Flink position delete conflict probe"
            if expect_conflict
            else "run isolated Flink position delete writer"
        ),
        timeout=timeout,
    )
    if expect_conflict:
        marker = CONFLICT_RE.search(completed.stdout)
        checks = {
            "conflict_marker_observed": marker is not None,
            "baseline_snapshot_bound": bool(
                marker and marker.group(1) == baseline["baseline_snapshot_id"]
            ),
            "current_snapshot_is_first_delete": bool(
                marker and marker.group(2) != marker.group(1)
            ),
            "target_road_id_exact": bool(
                marker and int(marker.group(3)) == plan["target_road_id"]
            ),
            "stale_commit_token_exact": bool(marker and marker.group(4) == token),
            "uncommitted_delete_file_cleaned": bool(
                marker and marker.group(5) == "true"
            ),
        }
        return {
            "status": "passed" if all(checks.values()) else "failed",
            "checks": checks,
            "baseline_snapshot_id": marker.group(1) if marker else None,
            "current_snapshot_id": marker.group(2) if marker else None,
            "target_road_id": int(marker.group(3)) if marker else None,
            "commit_token": marker.group(4) if marker else None,
            "orphan_delete_file_cleaned": (
                marker.group(5) == "true" if marker else None
            ),
            "conflict_type": "iceberg_validation_exception" if marker else None,
        }
...
def main() -> int:
    ...
    parser.add_argument(
        "--conflict",
        action="store_true",
        help="run a stale Flink position-delete validation probe after the first commit",
    )
    ...
    try:
        ...
        if args.conflict:
            conflict_probe = _run_flink_position_delete(
                flink,
                jar_path=jar_path,
                plan=plan,
                baseline=baseline,
                ...
                commit_token=plan["stale_flink_commit_token"],
            )
            if conflict_probe["status"] != "passed":
                raise RuntimeError(
                    f"stale Flink position delete was not rejected: {conflict_probe}"
                )
            conflict_metadata = _metadata_evidence(
                client,
                _catalog_metadata_location(
                    catalog,
                    namespace=namespace,
        ...
        if args.conflict:
            checks.update(
                {
                    "stale_position_delete_conflict_rejected": (
                        conflict_probe is not None
                        and all(conflict_probe["checks"].values())
                    ),
                    "stale_position_delete_left_catalog_unchanged": (
                        conflict_metadata == delete_metadata
                    ),
                    "stale_position_delete_created_no_snapshot": (
                        delete_metadata["snapshot_count"] == 2
                        and delete_metadata["current_snapshot"]["snapshot_id"]
                        == flink_commit["snapshot_id"]
                    ),
                }
        ...
        report = {
            "schema": (
                "gda.chongqing_osm_flink_spark_position_delete_conflict.acceptance.v1"
                if args.conflict
                else "gda.chongqing_osm_flink_spark_position_delete.acceptance.v1"
            ),
            "status": "passed" if all(checks.values()) else "failed",
            "generated_at": datetime.now(UTC).isoformat(),
            "checks": checks,
            "mode": "stale-position-delete-conflict" if args.conflict else "single-write",
            "source": {
                **plan["source"],
                "baseline_content_sha256": plan["baseline_content_sha256"],
            ...
            "table": {
                ...
                "stale_flink_position_delete": conflict_probe,
                "catalog_after_delete": delete_metadata,
                "catalog_after_stale_probe": conflict_metadata,
                "spark_verify": verify,
            ...
            "not_claimed": [
                ...
                *(
                    []
                    if args.conflict
                    else ["concurrent position-delete conflict isolation"]
                ),
