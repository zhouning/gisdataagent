...
class FlinkIcebergSandbox:
    ...
    def rest_url(self) -> str | None:
        return (
            f"http://127.0.0.1:{self.rest_host_port}"
            if self.rest_host_port is not None
            else None
        )
    ...
    def start(self) -> dict[str, int]:
        ...
        mapped = subprocess.run(
            ["docker", "port", self.container, "8081/tcp"],
            capture_output=True,
            text=True,
            timeout=15,
            check=False,
        )
        if mapped.returncode != 0 or not mapped.stdout.strip():
            raise RuntimeError("isolated Flink REST port was not published")
        try:
            self.rest_host_port = int(mapped.stdout.strip().rsplit(":", 1)[-1])
        except ValueError as exc:
            raise RuntimeError("isolated Flink REST port is invalid") from exc
        for _ in range(120):
            completed = subprocess.run(
                [
                    "docker",
    ...
    def run(
        ...
        *,
        ...
        catalog_uri: str,
        catalog_user: str,
        catalog_mode: str = "jdbc",
        catalog_prefix: str = "default_catalog",
    ) -> dict[str, Any]:
        row = plan["append_row"]
        completed = _run_command(
            [
