#!/usr/bin/env python3
"""Verify the governed Chongqing schedule-window acceptance evidence."""
...
def audit(
    *,
    ...
    try:
        ...
        if len(matches) != 1:
            raise ValueError(
                "schedule Run must correlate to exactly one provider instance"
            )
    ...
    checks = {
        ...
        "provider_uses_correlated_start": (
            instance.command_type == "START_PROCESS"
            and instance.schedule_time is None
        ),
        "provider_correlation_complete": all(
            variables.get(key) == value
            for key, value in expected_correlation.items()
        ),
        "provider_gda_correlation_surface_complete": len(gda_variables) >= 14,
        ...
        "native_online_schedule_disabled": not submission[
            "native_online_schedule_enabled"
        ],
    }
    report = {
        "schema": "gda.chongqing_jqdltb_schedule_window_acceptance.v1",
        ...
        "scheduled_for": invocation.schedule_times[0].isoformat(),
        ...
        "recovery_lag_seconds": submission["recovery_lag_seconds"],
        "missed_window_recovery": submission["missed_window_recovery"],
