...
def verify_s2_fulu(*, product_dir: Path, requested_at: str) -> dict[str, Any]:
    ...
    for area_id in ("fulu_heping", "fulu_banzhu"):
        ...
        cases[area_id] = {
            ...
            "same_snapshot_for_baseline_and_intervention": rollout["baseline"]["t0_snapshot_digest"] == rollout["intervention"]["t0_snapshot_digest"] == run["execution_scope"]["rollout_snapshot_digest"],
            "source_snapshot_digest": run["execution_scope"]["source_snapshot_digest"],
            "rollout_snapshot_digest": run["execution_scope"]["rollout_snapshot_digest"],
            "rollout_node_count": run["execution_scope"]["rollout_node_count"],
            "rollout_edge_count": run["execution_scope"]["rollout_edge_count"],
