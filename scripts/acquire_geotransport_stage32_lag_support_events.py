"""Acquire blind events for the Stage 32 empirical lag-support set test."""
...
from scripts import (
    acquire_geotransport_stage31_identifiable_response_events as stage31,
)
...
OBSERVATION_SCHEMA = "gwm.geotransport.stage32_observation_acquisition.v1"
RELEASE_OPERATOR_PATH = REPO_ROOT / (
    "data_agent/uwm/geospatial_kernel_v2/"
    "release_excitation_identifiability.py"
)
LAG_SUPPORT_OPERATOR_PATH = REPO_ROOT / (
...
EXCLUDED_EVENT_TIMES = stage31.EXCLUDED_EVENT_TIMES + (
    "2025-06-06T16:00:00Z",
    "2021-03-22T12:00:00Z",
    "2022-06-13T13:00:00Z",
    "2024-02-03T13:00:00Z",
)
STAGE28_DEVELOPMENT_START = stage31.STAGE28_DEVELOPMENT_START
STAGE28_DEVELOPMENT_END = stage31.STAGE28_DEVELOPMENT_END
...
def compile_selection_plan(*, values_mode: bool = False) -> dict[str, Any]:
    ...
    return {
        ...
        "purpose": (
            "select blind release-supported events before applying a frozen "
            "empirical lag-support set operator to downstream observations"
        ),
        "frozen_operator_artifacts": {
            "release_excitation_identifiability": stage29._artifact(
                RELEASE_OPERATOR_PATH
            ),
            "empirical_lag_support": stage29._artifact(
                LAG_SUPPORT_OPERATOR_PATH
            ),
        },
        "development_evidence": {
            "source_stage": 31,
            "event_support_sets_hours": [[5, 6], [5, 6], [6], [5, 6]],
            "common_support_hours": [6],
            "stage32_outcomes_used": False,
        },
        "frozen_release_support_gate": stage31.compile_selection_plan()[
            "frozen_release_support_gate"
        ],
        "frozen_empirical_lag_support": {
        ...
        "release_candidate_pool": stage31.compile_selection_plan()[
        ...
        "predeclared_event_selection": {
            ...
            "stage28_through_stage31_exclusion_radius_days": (
            ...
            "direction_or_flow_quota": None,
            "selection_data": "cwms_release_values_only",
            "selected_role": "blind_lag_support",
        },
        "predeclared_lag_support_test": {
            "per_event_support_set_must_be_nonempty": True,
            "all_events_must_have_detectable_response": True,
            "cross_event_common_support_requirement": (
                "intersection_of_all_event_support_sets_is_nonempty"
            ),
            "common_support_is_not_physical_travel_time": True,
        ...
        "claim_boundary": {
            ...
            "lag_support_sets_compiled": False,
            "common_empirical_support_admitted": False,
            "physical_travel_time_admitted": False,
...
def _selection_sources() -> list[dict[str, Any]]:
    source = dict(stage31._selection_sources()[0])
    source["role"] = "release_only_lag_support_candidate_pool"
    return [source]
...
def compile_observation_plan(
    ...
    if (
        ...
        raise ValueError("stage32_observation_request_boundary_exceeded")
    return {
        "schema": OBSERVATION_SCHEMA,
        "mode": "observation_plan",
        "purpose": (
            "acquire observations after release events and both support "
            "operators are hash frozen"
        ),
        "frozen_event_selection_manifest": stage29._artifact(path),
        "selected_events": selection["selected_events"],
        "frozen_operator_artifacts": selection[
            "frozen_operator_artifacts"
        ],
        "frozen_empirical_lag_support": selection[
            "frozen_empirical_lag_support"
        ],
        "predeclared_lag_support_test": selection[
            "predeclared_lag_support_test"
        ],
        "request_boundary": {
            ...
            "lag_support_operator_may_be_retuned": False,
        },
        "sources": sources,
        "claim_boundary": {
            "events_and_operators_frozen_before_observation_values": True,
            "lag_support_sets_compiled": False,
            "physical_travel_time_admitted": False,
            "runtime_operator_admitted": False,
        },
...
def _observation_sources(
    ...
    sources = [dict(value) for value in stage31._observation_sources(events)]
    for value in sources:
        value["role"] = (
            f"blind_lag_support_{value['site_role']}_values"
        )
    return sources
...
def main() -> int:
    ...
    if args.timeout_seconds <= 0.0 or args.retries <= 0:
        raise ValueError("stage32_positive_request_limits_required")
...
def _acquire_release_selection(
    ...
    stage29._write_json(
        candidate_path,
        {
            "schema": "gwm.geotransport.stage32_release_candidates.v1",
    ...
    manifest = {
        **values_plan,
        "status": "lag_support_events_frozen_before_observations",
        ...
        "claim_boundary_after_release_selection": {
            "operators_and_events_frozen": True,
...
def _acquire_observations(
    ...
    manifest = {
        **frozen_plan,
        ...
        "status": "lag_support_observations_acquired",
        "retrieved_at": datetime.now(timezone.utc).isoformat(),
        ...
        "claim_boundary_after_observations": {
            "operators_and_events_frozen_before_observation_values": True,
...
def _select_events(
    ...
    for index in range(EVENT_BEFORE_STEP_HOURS, len(series) - EVENT_AFTER_STEP_HOURS):
        ...
        if window_range < MINIMUM_WINDOW_RANGE_M3S or any(
            ...
            continue
        release_support = excitation.compile_release_excitation_identifiability(
            tuple(value[1] for value in window)
        )
        if not release_support.blind_response_test_admissible:
            continue
        candidates.append(
            {
                ...
                "release_direction": (
                    "increase" if signed_step > 0.0 else "decrease"
                ),
    ...
    for candidate in candidates:
        ...
        if len(selected) == EVENT_COUNT:
            break
    if len(selected) != EVENT_COUNT:
        raise ValueError("stage32_four_release_supported_events_unavailable")
    return candidates, selected


def _candidate_rank(value: dict[str, Any]) -> tuple[object, ...]:
    support = value["release_excitation_identifiability"]
    return (
        -int(support["excursion_support_hours"]),
        -float(support["normalized_excitation_volume_step_hours"]),
        float(support["lag_design_condition_number"]),
...
def _validate_selection_manifest_shape(value: dict[str, Any]) -> None:
    ...
    if (
        ...
        or any(
            event.get("role") != "blind_lag_support"
            or event.get("selected_without_observation_values") is not True
            or event.get("operators_frozen_without_observation_values")
            is not True
        ...
        or after.get("operators_and_events_frozen") is not True
        or after.get("downstream_values_acquired") is not False
    ):
        raise ValueError("stage32_event_selection_manifest_invalid")


def _load_exact_plan(path: Path, expected: dict[str, Any]) -> dict[str, Any]:
    if not path.is_file():
        raise ValueError("stage32_plan_must_be_frozen_before_values")
    value = stage29._read_json(path)
    if value != expected:
        raise ValueError("stage32_frozen_plan_mismatch")
    return value
