#!/usr/bin/env python3
"""Certify real Spark SQL MERGE conflict isolation against a Flink append."""
...
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_sql_merge_conflict.py"
SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_sql_merge_conflict"
...
def build_sql_merge_conflict_plan(source_path: Path) -> dict[str, Any]:
    ...
    stale_token = _canonical_sha256(
        {
            ...
            "operation": "merge-stale-baseline-revision",
    ...
    fresh_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "merge-fresh-flink-revision",
            "target_road_id": base["target_road_id"],
    ...
    return {
        **base,
        "schema": "gda.chongqing_osm_spark_flink_sql_merge_conflict_plan.v1",
        "merge_source_stale": stale,
        "merge_source_fresh": fresh,
        "final_merge_rows": final_rows,
        "final_merge_content_sha256": _canonical_sha256(final_rows),
        "sql_merge_stale_token": stale_token,
...
def main() -> int:
    ...
    work_dir = (
        REPO_ROOT / ".tmp/source-sync-certification" / f"flink_iceberg_sql_merge_conflict_{token}"
    )
    ...
    spark_container = f"gda-iceberg-spark-sql-merge-{token}"
    client = boto3.client(
        "s3",
    ...
    try:
        ...
        plan = build_sql_merge_conflict_plan(args.source)
        ...
        release_marker.write_text(
            json.dumps(
                {
                    "schema": "gda.spark_sql_merge_conflict_release.v1",
        ...
        checks = {
            ...
            == "true",
            "spark_sql_merge_ready_marker_exact": ready
            == {
                "schema": "gda.spark_sql_merge_conflict_ready.v1",
                "baseline_snapshot_id": baseline["baseline_snapshot_id"],
                "commit_token": plan["sql_merge_stale_token"],
            ...
            "stale_sql_merge_rejected": all(conflict["checks"].values()),
            "catalog_unchanged_after_stale_merge": after_conflict_location == after_flink_location
            and after_conflict_metadata == after_flink_metadata,
            "stale_merge_orphans_explicitly_cleaned": orphan_cleanup["keys_removed"]
            == len(orphan_keys)
            and orphan_cleanup["remaining_keys"] == len(after_flink_keys),
            "fresh_sql_merge_retry_passed": all(retry["checks"].values()),
            "independent_sql_merge_verify_passed": all(verify["checks"].values()),
            "final_snapshot_child_of_flink": final_metadata["current_snapshot"]["parent_id"]
            == flink_snapshot_id,
            "sql_merge_object_graph_materialized": inventory["metadata_json_count"] >= 3
            and inventory["manifest_avro_count"] >= 4
            and inventory["data_parquet_count"] >= 5,
        }
        report = {
            "schema": "gda.chongqing_osm_spark_flink_sql_merge_conflict.acceptance.v1",
            ...
            "source": {
                **plan["source"],
                ...
                "stale_token": plan["sql_merge_stale_token"],
            ...
            "control_plane": {
                ...
                "retry_mode": "explicit_fresh_state_sql_merge",
            },
            "not_claimed": [
                "SQL UPDATE semantics beyond this single-key bounded MERGE",
                (
                    "MERGE with deletes, inserts, multiple source rows, partition evolution, "
        ...
        if report["status"] != "passed":
            raise RuntimeError(f"SQL MERGE checks failed: {checks}")
