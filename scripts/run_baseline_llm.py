...
def run():
    ...
    print("=" * 70)
    print("🚀 GIS NL2SQL Benchmark - Zero-Shot Baseline Evaluation")
    ...
    for i, item in enumerate(dataset):
        ...
        try:
            ...
            gen_sql = resp.text.strip().replace("```sql", "").replace("```", "").strip()
        except Exception as e:
            gen_sql = str(e)
        ...
        if target == "Execution Accuracy" and item['golden_sql']:
            if engine:
        ...
        report_file = os.path.join(os.path.dirname(__file__), "..", "benchmarks", "baseline_gemini_3.1_pro_report.json")
