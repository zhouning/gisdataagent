#!/usr/bin/env python3
"""Certify multi-row Spark SQL UPDATE snapshot guarding against two Flink appends."""
...
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_sql_update_multi_conflict.py"
SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_sql_update_multi_conflict"
...
DEFAULT_REPORT = (
    ...
    "chongqing-osm-spark-flink-sql-update-multi-conflict-report.json"
)
...
def build_sql_update_multi_conflict_plan(source_path: Path) -> dict[str, Any]:
    base = build_update_conflict_plan(source_path)
    baseline = [dict(row) for row in base["baseline_rows"]]
    targets = sorted(baseline, key=lambda row: int(row["road_id"]))[:2]
    target_ids = [int(row["road_id"]) for row in targets]
    flink_rows: list[dict[str, Any]] = []
    flink_tokens: list[str] = []
    for target in targets:
        label = base64.b64decode(target["road_name_base64"]).decode("utf-8")
        payload = {
            "road_id": int(target["road_id"]),
            "revision": 2,
            "road_name_base64": base64.b64encode(f"{label} [Flink revision 2]".encode()).decode(
                "ascii"
            ),
        ...
        token = _canonical_sha256(
            {
                "engine": "flink-1.19.3",
                "operation": "multi-target-revision-append",
                "row": payload,
                "source_sha256": base["source"]["source_parquet_sha256"],
            }
        )
        flink_rows.append({**payload, "commit_token": token})
        flink_tokens.append(token)
    stale_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "multi-row-update-stale-baseline-revision",
            "target_road_ids": target_ids,
            "source_sha256": base["source"]["source_parquet_sha256"],
    ...
    fresh_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "multi-row-update-fresh-flink-revision",
            "target_road_ids": target_ids,
            "flink_tokens": flink_tokens,
            "source_sha256": base["source"]["source_parquet_sha256"],
        }
    )
    stale = {
        ...
        "expected_revision": 1,
        "new_revision": 2,
        "writer_engine": "spark-sql-update-multi",
    ...
    after_flink = sorted([*baseline, *flink_rows], key=_row_order)
    final_rows = sorted(
        [row for row in after_flink if row["road_id"] not in target_ids or row["revision"] == 1]
        + [
            {
                **row,
                "revision": 3,
            ...
            for row in flink_rows
        ],
        key=_row_order,
    )
    return {
        **base,
        "schema": "gda.chongqing_osm_spark_flink_sql_update_multi_conflict_plan.v1",
        "target_road_ids": target_ids,
        "flink_rows": flink_rows,
        "flink_commit_tokens": flink_tokens,
...
def _spark_phase(args: argparse.Namespace, **kwargs: Any) -> dict[str, Any]:
    ...
    _run_command(
        ...
        stage=f"Spark multi-row SQL UPDATE {kwargs['phase']}",
        timeout=args.timeout_seconds,
    )
    report = json.loads(report_path.read_text(encoding="utf-8"))
    if report.get("status") != "passed" or report.get("phase") != kwargs["phase"]:
        raise RuntimeError(f"Spark multi-row SQL UPDATE {kwargs['phase']} returned failed evidence")
    return report
...
def main() -> int:
    ...
    work_dir = (
        REPO_ROOT
        / ".tmp/source-sync-certification"
        / f"flink_iceberg_sql_update_multi_conflict_{token}"
    )
    paths = {
        name: work_dir / f"spark-{name}.json"
        for name in ("baseline", "conflict", "retry", "verify")
    }
    ready_marker, release_marker = work_dir / "spark-ready.json", work_dir / "spark-release.json"
    plan_path = work_dir / "plan.json"
    spark_container = f"gda-iceberg-spark-sql-update-multi-{token}"
    client = boto3.client(
        "s3",
    ...
    try:
        ...
        plan = build_sql_update_multi_conflict_plan(args.source)
        ...
        baseline = _spark_phase(
            args, phase="baseline", plan_path=plan_path, report_path=paths["baseline"], **common
        )
        ...
        flink_results = []
        for row, commit_token in zip(
            plan["flink_rows"], plan["flink_commit_tokens"], strict=True
        ):
            flink_results.append(
                _run_flink_partition_append(
                    flink,
                    jar_path=jar_path,
                    plan={"flink_row": row, "flink_commit_token": commit_token},
                    warehouse_uri=warehouse_uri,
                    table=table,
                    catalog_uri=catalog.jdbc_uri,
                    catalog_user=catalog.user,
        ...
        release_marker.write_text(
            json.dumps(
                {
                    "schema": "gda.spark_sql_update_multi_conflict_release.v1",
                    "flink_snapshot_id": flink_snapshot_id,
                    "target_road_ids": plan["target_road_ids"],
                    "flink_commit_tokens": plan["flink_commit_tokens"],
                },
                sort_keys=True,
                separators=(",", ":"),
            )
        ...
        conflict = json.loads(paths["conflict"].read_text(encoding="utf-8"))
        if spark_process.returncode != 0 or conflict.get("status") != "passed":
            raise RuntimeError(
                "multi-row SQL UPDATE conflict was not rejected: "
        ...
        orphan_cleanup.update(
            {
                ...
                "catalog_unchanged_before_cleanup": after_conflict_location == after_flink_location
        ...
        checks = {
            ...
            == "true",
            "spark_multi_update_ready_marker_exact": ready
            == {
            ...
            "two_flink_appends_passed": len(flink_results) == 2
            and all(all(item["checks"].values()) for item in flink_results),
            "catalog_advanced_through_two_flink_children": after_flink_metadata["snapshot_count"]
            == 3,
            "stale_multi_row_update_rejected": all(conflict["checks"].values()),
            "catalog_unchanged_after_stale_update": after_conflict_location == after_flink_location
            and after_conflict_metadata == after_flink_metadata,
            "stale_update_orphans_explicitly_cleaned": orphan_cleanup["keys_removed"]
            == len(orphan_keys)
            and orphan_cleanup["remaining_keys"] == len(after_flink_keys),
            "fresh_multi_row_update_retry_passed": all(retry["checks"].values()),
            "independent_multi_row_verify_passed": all(verify["checks"].values()),
            "final_snapshot_child_of_flink": (
                final_metadata["current_snapshot"]["parent_id"] == flink_snapshot_id
                if not plan.get("capability_probe")
                else final_metadata["current_snapshot"]["snapshot_id"] == flink_snapshot_id
            ),
            "multi_update_object_graph_materialized": (
                inventory["metadata_json_count"] >= 4
                and inventory["manifest_avro_count"] >= 6
                and inventory["data_parquet_count"] >= 7
                if not plan.get("capability_probe")
                else inventory["metadata_json_count"] >= 3
                and inventory["manifest_avro_count"] >= 6
                and inventory["data_parquet_count"] >= 5
            ),
        }
        report = {
            "schema": "gda.chongqing_osm_spark_flink_sql_update_multi_conflict.acceptance.v1",
            ...
            "control_plane": {
                ...
                "retry_mode": "explicit_fresh_state_multi_row_sql_update",
            },
            "not_claimed": [
                "SQL UPDATE joins/subqueries or complex predicates",
                "SQL MERGE multi-branch/multi-source-row semantics",
        ...
        if report["status"] != "passed":
            raise RuntimeError(f"multi-row SQL UPDATE checks failed: {checks}")
    except Exception as exc:
        safe = f"{type(exc).__name__}: {exc}"
        for value in (access_key, secret_key, catalog.password if catalog is not None else ""):
            if value:
