#!/usr/bin/env python3
"""Run a bounded Spark SQL MERGE with multiple not-matched insert branches."""
...
def _baseline(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if spark.catalog.tableExists(args.table):
        raise RuntimeError("isolated multi-branch SQL MERGE table already exists")
    ...
    (
        ...
        .tableProperty("gda.acceptance", "spark-sql-merge-multi-not-matched-branch")
...
def _merge(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    spark.createDataFrame(source, schema=_merge_schema()).createOrReplaceTempView(
        "gda_sql_merge_multi_not_matched_branch_source"
    )
    spark.sql(
        f"""
        MERGE INTO {args.table} AS target
        USING gda_sql_merge_multi_not_matched_branch_source AS source
    ...
    actual = _rows(spark.table(args.table))
    snapshots = _snapshots(spark, args.table)
    priority = plan["priority_insert_source_row"]
    default = plan["default_insert_source_row"]
    checks = {
        "merge_committed": True,
        "priority_insert_token_once": sum(
            row["commit_token"] == priority["commit_token"] for row in actual
        )
        == 1,
        "default_insert_token_once": sum(
            row["commit_token"] == default["commit_token"] for row in actual
        )
        == 1,
    ...
    return {
        ...
        "content_sha256": _canonical_sha256(actual),
        "snapshots": snapshots,
        "priority_insert_token_count": sum(
            row["commit_token"] == priority["commit_token"] for row in actual
        ),
        "default_insert_token_count": sum(
            row["commit_token"] == default["commit_token"] for row in actual
        ),
...
def _verify(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "priority_not_matched_inserted_once": sum(
            row["road_id"] == priority["road_id"]
            and row["commit_token"] == priority["commit_token"]
            for row in actual
        ) == 1,
        "default_not_matched_inserted_once": sum(
            row["road_id"] == default["road_id"]
            and row["commit_token"] == default["commit_token"]
            for row in actual
        ) == 1,
        ...
        "final_row_count_is_six": len(actual) == 6,
    }
    return {
        "phase": "verify",
        "status": "passed" if all(checks.values()) else "failed",
...
def main() -> int:
    ...
    try:
        ...
        spark.stop()
    report = {
        "schema": "gda.spark_flink_iceberg_sql_merge_multi_not_matched_branch_phase.v1",
