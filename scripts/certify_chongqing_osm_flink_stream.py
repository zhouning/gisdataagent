...
def verify_flink_output(
    *,
    ...
    return {
        ...
        "accepted_files": accepted_inventory,
        "rejected_files": rejected_inventory,
        "final_state_sha256": _canonical_sha256(final_rows),
        "final_state_records": final_rows,
        "rejected_records": sorted(
            rejected,
            key=lambda item: (item["reason"], item["event_id"]),
        ),
...
def _sync_definition(
    ...
    values: dict[str, Any] = {
        ...
        "governance_contract": {
            ...
            "target_layer": "silver",
            "data_kind": "vector",
            "capture_kind": "event_stream",
            "source_adapter": {
                "adapter_id": "flink-filesystem",
            ...
            "quality_rule_version_refs": ["quality:event-integrity-v1"],
            ...
            "event_time_field": "event_time_ms",
            "watermark_delay_seconds": OUT_OF_ORDERNESS_MS // 1000,
        },
    }
...
def _commit_from_verification(
    *,
    ...
    values: dict[str, Any] = {
        ...
        "target_content_sha256": verification["target_content_sha256"],
        "records_read": 10,
...
def _certify(
    ...
    *,
    ...
    output_path = evidence_dir / "silver-output-manifest.json"
    quality_path = evidence_dir / "quality-manifest.json"
    quarantine_path = evidence_dir / "quarantine-manifest.json"

    output_document = {
        "schema": "gda.source_sync_output.v1",
        "accepted_files": verification["accepted_files"],
        "accepted_manifest_sha256": verification["accepted_manifest_sha256"],
        "final_state_sha256": verification["final_state_sha256"],
        "final_state_records": verification["final_state_records"],
        "records_output": verification["final_state_rows"],
        "source_slice_sha256": source_slice_sha256,
    }
    output_path.write_bytes(_canonical_bytes(output_document))
    target_content_sha256 = _sha256_file(output_path)
    verification["target_content_sha256"] = target_content_sha256

    quality_rule_ref = "quality:event-integrity-v1"
    quality_document = {
        "schema": "gda.source_sync_quality.v1",
        "checks": verification["checks"],
        "quality_rule_version_ref": quality_rule_ref,
        "records_accepted": verification["accepted_event_count"],
        "records_read": len(events),
        "records_rejected": verification["rejected_event_count"],
        "source_slice_sha256": source_slice_sha256,
        "target_content_sha256": target_content_sha256,
    }
    quality_path.write_bytes(_canonical_bytes(quality_document))
    quality_content_sha256 = _sha256_file(quality_path)

    reason_counts = {"duplicate": 1, "late": 1}
    quarantine_provider_document = {
        "schema": "gda.flink_rejected_output.v1",
        "rejected_files": verification["rejected_files"],
        "rejected_records": verification["rejected_records"],
    }
    quarantine_path.write_bytes(_canonical_bytes(quarantine_provider_document))
    ...
    if quarantine_urn is None:
        raise RuntimeError("Silver stream definition lacks a quarantine Resource")
    _register_resource_version(
        gateway,
        tenant_id="local-dev",
    ...
    quality_metrics = {
        "accepted": verification["accepted_event_count"],
        "checked": len(events),
        "rejected": verification["rejected_event_count"],
        "violations": 0,
    }
    ...
    quarantine_record = SourceSyncQuarantineRecorder(
        SourceSyncQuarantineContract(
            ...
            authority_system="flink-filesystem",
    ...
    quarantine_artifact = quarantine_record.artifact
    quarantine_evidence = quarantine_record.evidence
    commit_write = authority.commit(
        commit, governance_evidence, quarantine_evidence
    )
    ...
    checks = {
        ...
        "real_provider_rejections_bound": (
            verification["rejected_event_count"] == 2
            and {
                (item["reason"], item["event_id"])
                for item in verification["rejected_records"]
            }
            == {("duplicate", "cq-osm-e05"), ("late", "cq-osm-e07")}
            and quarantine_evidence.records_rejected == 2
            and quarantine_evidence.reason_counts == reason_counts
        ),
        "physical_governance_artifacts_hashed": (
            _sha256_file(output_path) == output_artifact.content_sha256
            and _sha256_file(quality_path) == quality_artifact.content_sha256
            and _sha256_file(quarantine_path)
            == quarantine_artifact.content_sha256
        ),
        "source_sync_commit_advanced_once": (
            ...
            and commit_write.governance_evidence == governance_evidence
            and commit_write.quarantine_evidence == quarantine_evidence
        ),
        "same_id_replay_requires_identical_evidence": (
            not same_id_replay.created
            and same_id_replay.commit == commit
            and same_id_replay.governance_evidence == governance_evidence
            and same_id_replay.quarantine_evidence == quarantine_evidence
        ),
        ...
        "cross_run_replay_recovered_commit": (
            ...
            and replay_write.quarantine_evidence == quarantine_evidence
        ),
    ...
    return {
        ...
        "authority": {
            ...
            "quarantine_evidence": quarantine_evidence.model_dump(mode="json"),
            "physical_artifacts": {
                "output": output_artifact.model_dump(mode="json"),
                "quality": quality_artifact.model_dump(mode="json"),
                "quarantine": quarantine_artifact.model_dump(mode="json"),
            },
