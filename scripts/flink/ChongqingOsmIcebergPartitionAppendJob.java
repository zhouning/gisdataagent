...
/** Append one road revision without reusing a classloader across query jobs. */
public final class ChongqingOsmIcebergPartitionAppendJob {
    private ChongqingOsmIcebergPartitionAppendJob() {}
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.printf(
                "GDA_PARTITION_FLINK_STARTED road_id=%d revision=%d token=%s%n",
                options.roadId,
                options.revision,
                options.commitToken);
        ...
        insertion.await();
        System.out.printf(
                "GDA_PARTITION_FLINK_COMMITTED road_id=%d revision=%d token=%s%n",
                options.roadId,
                options.revision,
                options.commitToken);
    }
    ...
    private static final class JobArguments {
        ...
        static JobArguments parse(String[] args) {
            ...
            if (roadId <= 0 || revision != 2) {
                throw new IllegalArgumentException("unexpected partition append bounds");
            }
