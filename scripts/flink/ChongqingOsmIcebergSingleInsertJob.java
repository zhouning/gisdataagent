...
/** Insert one road into an Iceberg table using one provider operation. */
public final class ChongqingOsmIcebergSingleInsertJob {
    private ChongqingOsmIcebergSingleInsertJob() {}
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.printf(
                "GDA_SINGLE_INSERT_FLINK_STARTED road_id=%d token=%s%n",
                options.roadId,
                options.commitToken);
        TableResult insertion = tableEnvironment.executeSql(insertSql(options));
        insertion.await();
        System.out.printf(
                "GDA_SINGLE_INSERT_FLINK_COMMITTED road_id=%d token=%s%n",
                options.roadId,
                options.commitToken);
    }
    ...
    private static final class JobArguments {
        ...
        static JobArguments parse(String[] args) {
            ...
            if (roadId <= 0 || revision != 1) {
                throw new IllegalArgumentException("unexpected single insert bounds");
            }
