#!/usr/bin/env python3
"""Certify stale partition delete rejection and a fresh same-key retry."""
...
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_partition_delete_conflict.py"
SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_partition_delete_conflict"
DEFAULT_REPORT = (
    REPO_ROOT / ".tmp/source-sync-certification/"
    "chongqing-osm-spark-flink-partition-delete-conflict-report.json"
)


def build_partition_delete_conflict_plan(source_path: Path) -> dict[str, Any]:
    update = build_update_conflict_plan(source_path)
    baseline = [dict(row) for row in update["baseline_rows"]]
    flink_row = dict(update["flink_row"])
    target_id = int(update["target_road_id"])
    non_target = [row for row in baseline if row["road_id"] != target_id]
    delete_token = _canonical_sha256(
        {
            "engine": "spark-3.5",
            "operation": "partition-delete-by-road-id",
            "target_road_id": target_id,
            "baseline_content_sha256": update["baseline_content_sha256"],
            "source_sha256": update["source"]["source_parquet_sha256"],
        }
    )
    return {
        "schema": "gda.chongqing_osm_spark_flink_partition_delete_conflict_plan.v1",
        "source": update["source"],
        "baseline_rows": baseline,
        "flink_row": flink_row,
        "after_flink_rows": update["after_flink_rows"],
        "final_rows": non_target,
        "target_road_id": target_id,
        "flink_commit_token": update["flink_commit_token"],
        "spark_delete_token": delete_token,
        "baseline_content_sha256": update["baseline_content_sha256"],
        "after_flink_content_sha256": update["after_flink_content_sha256"],
        "final_content_sha256": _canonical_sha256(non_target),
    }
...
def _spark_phase(args: argparse.Namespace, **kwargs: Any) -> dict[str, Any]:
    ...
    _run_command(
        ...
        stage=f"Spark partition delete conflict {phase}",
        timeout=args.timeout_seconds,
    )
    report = json.loads(report_path.read_text(encoding="utf-8"))
    if report.get("status") != "passed" or report.get("phase") != phase:
        raise RuntimeError(f"Spark partition delete {phase} returned failed evidence")
    return report
...
def main() -> int:
    ...
    work_dir = (
        ...
        / f"flink_iceberg_partition_delete_conflict_{token}"
    )
    ...
    try:
        ...
        plan = build_partition_delete_conflict_plan(args.source)
        ...
        if not conflict_path.is_file():
            ...
            raise RuntimeError(f"Spark partition delete emitted no evidence: {output}")
        conflict = json.loads(conflict_path.read_text(encoding="utf-8"))
        if spark_process.returncode != 0 or conflict.get("status") != "passed":
            raise RuntimeError(
                "Spark stale partition delete was not rejected safely: "
        ...
        checks = {
            ...
            "spark_delete_intent_in_flight_before_flink_commit": (
                ready_exact
            ...
            "stale_spark_partition_delete_failed_closed": all(conflict["checks"].values()),
            ...
            "fresh_state_retry_removed_target_partition_only": all(retry["checks"].values()),
            ...
            "partition_delete_object_graph_materialized": (
                inventory["metadata_json_count"] >= 3
                and inventory["manifest_avro_count"] >= 4
                and inventory["data_parquet_count"] >= 4
            ),
        }
        report = {
            "schema": "gda.chongqing_osm_spark_flink_partition_delete_conflict.acceptance.v1",
            "status": "passed" if all(checks.values()) else "failed",
            "generated_at": datetime.now(UTC).isoformat(),
            "checks": checks,
            "source": {
                **plan["source"],
                "selected_road_ids": [row["road_id"] for row in plan["baseline_rows"]],
                "target_road_id": plan["target_road_id"],
                "flink_commit_token": plan["flink_commit_token"],
            ...
            "table": {
                ...
                "delete_mode": "copy-on-write data-file delete",
                "baseline": baseline,
            ...
            "control_plane": {
                ...
                "delete_semantics": "remove all rows in one identity road_id partition",
            },
            "not_claimed": [
                "equality-delete, position-delete, or merge-on-read delete files",
        ...
        if report["status"] != "passed":
            raise RuntimeError(f"partition delete conflict checks failed: {checks}")
