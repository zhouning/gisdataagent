#!/usr/bin/env python3
"""Certify a real Spark SQL MERGE with a complex AND/OR/IN predicate."""
...
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_sql_merge_complex_predicate.py"
SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_sql_merge_complex_predicate"
...
DEFAULT_REPORT = (
    ...
    "chongqing-osm-spark-flink-sql-merge-complex-predicate-report.json"
)
...
def build_sql_merge_complex_predicate_plan(source_path: Path) -> dict[str, Any]:
    base = build_update_conflict_plan(source_path)
    target_a = base["flink_row"]
    target_b = next(row for row in base["baseline_rows"] if row["road_id"] == 102262020)
    guard_target = next(row for row in base["baseline_rows"] if row["road_id"] == 102262024)
    matched = []
    matched_targets = ((target_a, "promote"), (target_b, "refresh"))
    for index, (target, action) in enumerate(matched_targets, start=1):
        ...
        token = _canonical_sha256(
            {
                ...
                "operation": "merge-complex-predicate",
                "action": action,
        ...
        matched.append(
            {
                ...
                "writer_engine": "spark-sql-merge-complex-predicate",
                "commit_token": token,
                "source_row_id": f"matched-update-source-{index}",
                "action": action,
            }
        )
    guard_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "merge-complex-predicate-guard",
            "target_road_id": guard_target["road_id"],
            "source_sha256": base["source"]["source_parquet_sha256"],
        }
    )
    guard = {
        "road_id": guard_target["road_id"],
        "expected_revision": 1,
        "result_revision": 9,
        "road_name_base64": guard_target["road_name_base64"],
        "geometry_sha256": guard_target["geometry_sha256"],
        "writer_engine": "spark-sql-merge-complex-predicate-guard",
        "commit_token": guard_token,
        "source_row_id": "complex-predicate-guard-source",
        "action": "ignore",
    }
    final_rows = [
        row
    ...
    return {
        **base,
        "schema": "gda.chongqing_osm_spark_flink_sql_merge_complex_predicate_plan.v1",
        "matched_source_rows": matched,
        "guard_source_row": guard,
        "merge_source_rows": [*matched, guard],
...
def main() -> int:
    ...
    work_dir = (
        ...
        / f"flink_iceberg_sql_merge_complex_predicate_{token}"
    )
    ...
    try:
        ...
        plan = build_sql_merge_complex_predicate_plan(args.source)
        ...
        checks = {
            ...
            "sql_merge_complex_predicate_passed": all(merge["checks"].values()),
            ...
            "complex_predicate_object_graph_materialized": inventory["metadata_json_count"] >= 3
            and inventory["manifest_avro_count"] >= 4
            and inventory["data_parquet_count"] >= 5,
        }
        report = {
            "schema": "gda.chongqing_osm_spark_flink_sql_merge_complex_predicate.acceptance.v1",
            ...
            "source": {
                **plan["source"],
                ...
                "guard_token": plan["guard_source_row"]["commit_token"],
            },
            "runtime": {
            ...
            "control_plane": {
                ...
                "merge_branches": ["matched_update_complex_predicate"],
                "writer_recovery_mode": "explicit_fresh_state",
            },
            "not_claimed": [
                (
                    "MERGE delete branches, multiple matched branches, cross-partition "
                    "multi-file writes, "
                    "or automatic retry"
                ),
                (
