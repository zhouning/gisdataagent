#!/usr/bin/env python3
"""Run a bounded Spark SQL MERGE with matched-update and not-matched-insert branches."""
...
def _baseline(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    (
        ...
        .tableProperty("gda.acceptance", "spark-sql-merge-multi-branch")
...
def _merge(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    spark.createDataFrame(source, schema=_merge_schema()).createOrReplaceTempView(
        "gda_sql_merge_multi_branch_source"
    )
    spark.sql(
        f"""
        MERGE INTO {args.table} AS target
        USING gda_sql_merge_multi_branch_source AS source
    ...
    actual = _rows(spark.table(args.table))
    snapshots = _snapshots(spark, args.table)
    matched = plan["matched_source_row"]
    inserted = plan["insert_source_row"]
    checks = {
        "merge_committed": True,
        "matched_update_token_once": sum(
            row["commit_token"] == matched["commit_token"] for row in actual
        )
        == 1,
        "insert_token_once": sum(
            row["commit_token"] == inserted["commit_token"] for row in actual
        )
        == 1,
    ...
    return {
        ...
        "content_sha256": _canonical_sha256(actual),
        "snapshots": snapshots,
        "matched_update_token_count": sum(
            row["commit_token"] == matched["commit_token"] for row in actual
        ),
        "insert_token_count": sum(
            row["commit_token"] == inserted["commit_token"] for row in actual
        ),
...
def _verify(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "matched_target_revision_three": any(
            row["road_id"] == matched["road_id"]
            and row["revision"] == matched["result_revision"]
            and row["commit_token"] == matched["commit_token"]
            for row in actual
        ),
        "unmatched_target_inserted_once": sum(
            row["road_id"] == inserted["road_id"]
            and row["commit_token"] == inserted["commit_token"]
            for row in actual
        )
        == 1,
...
def main() -> int:
    ...
    try:
        ...
        spark.stop()
    report = {
        "schema": "gda.spark_flink_iceberg_sql_merge_multi_branch_phase.v1",
