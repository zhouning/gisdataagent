#!/usr/bin/env python3
"""Certify a bounded Spark SQL UPDATE driven by an uncorrelated scope subquery."""
...
DEFAULT_REPORT = (
    REPO_ROOT
    / "docs/reports/chongqing_osm_spark_flink_sql_update_subquery_2026-08-24.json"
)
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_sql_update_subquery.py"
...
def build_sql_update_subquery_plan(source_path: Path) -> dict:
    ...
    plan.update(
        {
            "schema": "gda.chongqing_osm_spark_flink_sql_update_subquery_plan.v1",
            "subquery_scope_rows": [
...
def main() -> int:
    base.build_sql_update_multi_conflict_plan = build_sql_update_subquery_plan
    base.SPARK_SOURCE = SPARK_SOURCE
    base.SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_sql_update_subquery"
    ...
    if report_path.exists():
        ...
        plan = build_sql_update_subquery_plan(source_path)
        report.setdefault("source", {})["guard_road_id"] = plan["guard_road_id"]
        report["schema"] = "gda.chongqing_osm_spark_flink_sql_update_subquery.acceptance.v1"
        ...
        report["not_claimed"] = [
            "correlated UPDATE subqueries, UPDATE joins or multi-table writes",
