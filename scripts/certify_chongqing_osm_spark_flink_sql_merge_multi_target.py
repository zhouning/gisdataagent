#!/usr/bin/env python3
"""Certify a real Spark SQL MERGE that updates two distinct target rows."""
...
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_sql_merge_multi_target.py"
SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_sql_merge_multi_target"
...
DEFAULT_REPORT = (
    ...
    "chongqing-osm-spark-flink-sql-merge-multi-target-report.json"
)
...
def build_sql_merge_multi_target_plan(
    source_path: Path, *, file_scope_contract: bool = False
) -> dict[str, Any]:
    base = build_update_conflict_plan(source_path)
    target_a = base["flink_row"]
    target_b = next(row for row in base["baseline_rows"] if row["road_id"] != target_a["road_id"])
    matched = []
    for index, target in enumerate((target_a, target_b), start=1):
        expected_revision = 2 if target is target_a else 1
        result_revision = expected_revision + 1
        token = _canonical_sha256(
            {
                "engine": "spark-3.5-sql",
                "operation": "merge-multi-target-update",
        ...
        matched.append(
            {
                ...
                "writer_engine": "spark-sql-merge-multi-target",
                "commit_token": token,
                "source_row_id": f"matched-update-source-{index}",
            }
        )
    final_rows = [
        row
    ...
    plan = {
        **base,
        "schema": "gda.chongqing_osm_spark_flink_sql_merge_multi_target_plan.v1",
        "matched_source_rows": matched,
        "merge_source_rows": matched,
        "final_merge_rows": final_rows,
        "final_merge_content_sha256": _canonical_sha256(final_rows),
        "target_road_ids": [source["road_id"] for source in matched],
    }
    if file_scope_contract:
        guard_road_id = next(
            row["road_id"]
            for row in base["baseline_rows"]
            if row["road_id"] not in plan["target_road_ids"]
        )
        plan.update(
            {
                "file_scope_contract": True,
                "expected_unchanged_partition_ids": [guard_road_id],
                "file_scope_policy": "target_partitions_only_copy_on_write",
            }
        )
    return plan
...
def _spark_phase(args: argparse.Namespace, **kwargs: Any) -> dict[str, Any]:
    ...
    if completed.returncode != 0 and report is None:
        raise RuntimeError(
            f"Spark SQL MERGE multi-target {kwargs['phase']} failed: "
...
def main() -> int:
    ...
    parser.add_argument(
        "--file-scope-contract",
        action="store_true",
        help="Enable Iceberg table.files physical partition-range accounting.",
    )
    parser.add_argument("--report", type=Path, default=DEFAULT_REPORT)
    args = parser.parse_args()
    if args.file_scope_contract and args.report == DEFAULT_REPORT:
        args.report = (
            REPO_ROOT
            / ".tmp/source-sync-certification/"
            "chongqing-osm-spark-flink-sql-merge-partition-file-scope-report.json"
        )
    ...
    work_dir = (
        REPO_ROOT
        / ".tmp/source-sync-certification"
        / f"flink_iceberg_sql_merge_multi_target_{token}"
    )
    ...
    spark_container = f"gda-iceberg-spark-sql-merge-multi-target-{token}"
    client = boto3.client(
        "s3",
    ...
    try:
        ...
        plan = build_sql_merge_multi_target_plan(
            args.source, file_scope_contract=args.file_scope_contract
        )
        ...
        checks = {
            ...
            "sql_merge_multi_target_update_passed": all(merge["checks"].values()),
            "physical_file_scope_contract_passed": (
                not args.file_scope_contract
                or (
                    merge.get("file_scope") is not None
                    and all(merge["file_scope"]["checks"].values())
                )
            ),
            ...
            "multi_target_object_graph_materialized": inventory["metadata_json_count"] >= 3
            and inventory["manifest_avro_count"] >= 4
            and inventory["data_parquet_count"] >= 5,
        }
        report = {
            "schema": (
                "gda.chongqing_osm_spark_flink_sql_merge_partition_file_scope.acceptance.v1"
                if args.file_scope_contract
                else "gda.chongqing_osm_spark_flink_sql_merge_multi_target.acceptance.v1"
            ),
            ...
            "control_plane": {
                ...
                "merge_branches": ["matched_update"],
                "writer_recovery_mode": "explicit_fresh_state",
                "file_scope_contract": args.file_scope_contract,
            },
            "not_claimed": [
                (
        ...
        if report["status"] != "passed":
            raise RuntimeError(f"multi-target SQL MERGE checks failed: {checks}")
