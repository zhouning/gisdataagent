#!/usr/bin/env python3
"""Reject a stale key delete, retry from fresh state, and verify Iceberg."""
...
from scripts.spark_chongqing_osm_iceberg_concurrent_append import _rows, _schema
...
BARRIER_RE = re.compile(
    ...
    r"flink_iceberg_delete_conflict_[0-9a-f]{10}/spark-(?:ready|release)\.json$"
)
...
def _snapshots(spark, table: str) -> list[dict[str, Any]]:
    return [
        ...
        for row in spark.sql(
            ...
            f"summary['gda.commit-token'] AS commit_token FROM {table}.snapshots "
            "ORDER BY committed_at, snapshot_id"
        ).collect()
    ]


def _key_expression(spark, road_id: int):
    java_id = spark._jvm.java.lang.Long.valueOf(str(road_id))  # noqa: SLF001
    return spark._jvm.org.apache.iceberg.expressions.Expressions.equal(  # noqa: SLF001
        "road_id",
        java_id,
    )


def _iceberg_table(spark, table: str):
    return spark._jvm.org.apache.iceberg.spark.Spark3Util.loadIcebergTable(  # noqa: SLF001
...
def _baseline(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if spark.catalog.tableExists(args.table):
        raise RuntimeError("isolated delete conflict table already exists")
    frame = spark.createDataFrame(plan["baseline_rows"], schema=_schema())
    (
        frame.writeTo(args.table)
        .using("iceberg")
        .tableProperty("format-version", "2")
        .tableProperty("gda.acceptance", "spark-flink-delete-conflict")
    ...
    checks = {
        ...
        "delete_target_absent_from_baseline": all(
...
def _validate_barriers(args: argparse.Namespace) -> None:
    if not args.ready_marker or not args.release_marker:
        raise ValueError("concurrent delete requires both barrier paths")
    for path in (args.ready_marker, args.release_marker):
        if not BARRIER_RE.fullmatch(path.as_posix()):
            raise ValueError("unsafe delete conflict barrier path")
...
def _concurrent_delete(
    ...
    if (
        ...
        raise RuntimeError("Spark delete did not start from the exact baseline")
    ...
    if provider_baseline != args.baseline_snapshot_id:
        raise RuntimeError("Iceberg delete transaction did not bind the baseline")
    ...
    delete_intent = (
        ...
        .set("gda.operation", "delete-by-road-id")
    )
    ready_payload = {
    ...
    if not args.release_marker.is_file():
        raise RuntimeError("Spark delete conflict barrier timed out")
    ...
    delete_token_snapshots = sum(
        item["commit_token"] == plan["spark_delete_token"] for item in snapshots
    )
    checks = {
        ...
        "stale_delete_rejected": not delete_committed,
        ...
        "flink_target_visible_once_after_conflict": len(target_rows) == 1
        and target_rows[0]["commit_token"] == plan["flink_commit_token"],
        "delete_commit_token_absent_after_conflict": delete_token_snapshots == 0,
    ...
    return {
        ...
        "delete_commit_token_snapshot_count": delete_token_snapshots,
    }
...
def _retry(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if not fresh_state_exact:
        raise RuntimeError("explicit delete retry did not read the exact Flink state")
    iceberg_table = _iceberg_table(spark, args.table)
    iceberg_table.refresh()
    if str(iceberg_table.currentSnapshot().snapshotId()) != args.flink_snapshot_id:
        raise RuntimeError("delete retry provider state drifted before commit")
    (
    ...
    checks = {
        ...
        "target_road_removed": all(row["road_id"] != plan["target_road_id"] for row in actual),
        "non_target_rows_preserved": list(actual) == plan["final_rows"],
...
def _verify(spark, args: argparse.Namespace, plan: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {
        ...
        "target_road_absent": all(row["road_id"] != plan["target_road_id"] for row in actual),
...
def main() -> int:
    ...
    try:
        ...
        spark.stop()
    report = {
        "schema": "gda.spark_flink_iceberg_delete_conflict_phase.v1",
