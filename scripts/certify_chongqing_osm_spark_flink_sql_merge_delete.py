#!/usr/bin/env python3
"""Certify a real Spark SQL MERGE matched-delete branch."""
...
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_sql_merge_delete.py"
SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_sql_merge_delete"
...
DEFAULT_REPORT = (
    ...
    "chongqing-osm-spark-flink-sql-merge-delete-report.json"
)
...
def build_sql_merge_delete_plan(source_path: Path) -> dict[str, Any]:
    ...
    deleted = {
        ...
        "writer_engine": "spark-sql-merge-delete",
    ...
    return {
        **base,
        "schema": "gda.chongqing_osm_spark_flink_sql_merge_delete_plan.v1",
        "delete_source_row": deleted,
        "merge_source_rows": [deleted],
        "final_merge_rows": final_rows,
        "final_merge_content_sha256": _canonical_sha256(final_rows),
        "sql_merge_delete_token": delete_token,
    }
...
def _spark_phase(args: argparse.Namespace, **kwargs: Any) -> dict[str, Any]:
    ...
    if completed.returncode != 0 and report is None:
        raise RuntimeError(
            f"Spark SQL MERGE delete {kwargs['phase']} failed: "
...
def main() -> int:
    ...
    work_dir = (
        ...
        / f"flink_iceberg_sql_merge_delete_{token}"
    )
    ...
    spark_container = f"gda-iceberg-spark-sql-merge-delete-{token}"
    client = boto3.client(
        "s3",
    ...
    try:
        ...
        plan = build_sql_merge_delete_plan(args.source)
        ...
        checks = {
            ...
            "sql_merge_matched_delete_passed": all(merge["checks"].values()),
            ...
            "merge_delete_object_graph_materialized": inventory["metadata_json_count"] >= 3
            and inventory["manifest_avro_count"] >= 4
            and inventory["data_parquet_count"] >= 4,
        }
        report = {
            "schema": "gda.chongqing_osm_spark_flink_sql_merge_delete.acceptance.v1",
            "status": "passed" if all(checks.values()) else "failed",
            "generated_at": datetime.now(UTC).isoformat(),
            "checks": checks,
            "source": {
                **plan["source"],
                "deleted_target_road_id": plan["target_road_id"],
                ...
                "delete_token": plan["sql_merge_delete_token"],
            },
            "runtime": {
            ...
            "control_plane": {
                ...
                "merge_branches": ["matched_delete"],
                "writer_recovery_mode": "explicit_fresh_state",
            },
            "not_claimed": [
                (
                    "MERGE multiple matched branches, multiple target rows, "
        ...
        if report["status"] != "passed":
            raise RuntimeError(f"SQL MERGE delete checks failed: {checks}")
