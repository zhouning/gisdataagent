#!/usr/bin/env python3
"""Run the real step-bound AgentOps HITL acceptance rehearsal."""
...
from data_agent.agentops_temporal_approval_rehearsal import (
    run_rehearsal,
    write_report,
)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--task-queue",
        default="agentops-step-hitl-rehearsal",
    )
    parser.add_argument(
        "--admin-database-url",
    ...
    if not args.admin_database_url:
        parser.error(
            "--admin-database-url or GDA_REHEARSAL_ADMIN_DATABASE_URL is required"
        )
    return args
