#!/usr/bin/env python3
"""Certify a real Spark SQL MERGE with mixed matched and not-matched branches."""
...
SPARK_SOURCE = REPO_ROOT / "scripts/spark_chongqing_osm_iceberg_sql_merge_mixed_branches.py"
SPARK_MODULE = "scripts.spark_chongqing_osm_iceberg_sql_merge_mixed_branches"
...
DEFAULT_REPORT = (
    ...
    "chongqing-osm-spark-flink-sql-merge-mixed-branches-report.json"
)
...
def build_sql_merge_mixed_branches_plan(source_path: Path) -> dict[str, Any]:
    base = build_update_conflict_plan(source_path)
    append = build_concurrent_append_plan(source_path)
    target_delete = base["flink_row"]
    target_update = next(
        row for row in base["baseline_rows"] if row["road_id"] != target_delete["road_id"]
    )
    delete_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "merge-mixed-branches",
            "branch": "matched-delete",
            "target_road_id": target_delete["road_id"],
            "expected_revision": 2,
            "flink_token": base["flink_commit_token"],
            "source_sha256": base["source"]["source_parquet_sha256"],
        }
    )
    update_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "merge-mixed-branches",
            "branch": "matched-update",
            "target_road_id": target_update["road_id"],
            "expected_revision": 1,
            "flink_token": base["flink_commit_token"],
            "source_sha256": base["source"]["source_parquet_sha256"],
        }
    ...
    delete_source = {
        "road_id": target_delete["road_id"],
        "expected_revision": 2,
        "result_revision": -1,
        "road_name_base64": target_delete["road_name_base64"],
        "geometry_sha256": target_delete["geometry_sha256"],
        "writer_engine": "spark-sql-merge-delete-branch",
        "commit_token": delete_token,
        "source_row_id": "matched-delete-source",
        "action": "delete",
    }
    update_source = {
        "road_id": target_update["road_id"],
        "expected_revision": 1,
        "result_revision": 2,
        "road_name_base64": target_update["road_name_base64"],
        "geometry_sha256": target_update["geometry_sha256"],
        "writer_engine": "spark-sql-merge-update-branch",
        "commit_token": update_token,
    ...
    priority_base = append["spark_row"]
    default_base = _road_row(source_table.slice(5, 1).to_pylist()[0])
    existing_ids = {row["road_id"] for row in base["after_flink_rows"]}
    if priority_base["road_id"] in existing_ids or default_base["road_id"] in existing_ids:
        raise RuntimeError("mixed-branch insert roads must not exist in Flink target state")
    if priority_base["road_id"] == default_base["road_id"]:
        raise RuntimeError("mixed-branch insert roads must be distinct")
    priority_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "merge-mixed-branches",
            "branch": "not-matched-priority-insert",
            "insert_road_id": priority_base["road_id"],
            "source_sha256": base["source"]["source_parquet_sha256"],
        }
    )
    default_token = _canonical_sha256(
        {
            "engine": "spark-3.5-sql",
            "operation": "merge-mixed-branches",
            "branch": "not-matched-default-insert",
            "insert_road_id": default_base["road_id"],
            "source_sha256": base["source"]["source_parquet_sha256"],
        }
    )
    priority_source = {
    ...
    mixed_sources = [delete_source, update_source, priority_source, default_source]
    final_rows = [
        row
        ...
        if not any(
            ...
            if source["action"] in {"delete", "update"}
        )
    ]
    final_rows.extend(
        {
    ...
    final_rows.extend(
        ...
        for source in mixed_sources
        if source["action"] in {"insert_priority", "insert_default"}
    )
    final_rows = sorted(
        final_rows,
        key=_row_order,
    ...
    return {
        **base,
        "schema": "gda.chongqing_osm_spark_flink_sql_merge_mixed_branches_plan.v1",
        "delete_source_row": delete_source,
        "update_source_row": update_source,
        "priority_insert_source_row": priority_source,
        "default_insert_source_row": default_source,
        "matched_source_rows": [delete_source, update_source],
        "merge_source_rows": mixed_sources,
        "final_merge_rows": final_rows,
        "final_merge_content_sha256": _canonical_sha256(final_rows),
        "target_road_ids": [delete_source["road_id"], update_source["road_id"]],
        "inserted_road_ids": [priority_source["road_id"], default_source["road_id"]],
    }
...
def main() -> int:
    ...
    work_dir = (
        ...
        / f"flink_iceberg_sql_merge_mixed_branches_{token}"
    )
    ...
    spark_container = f"gda-iceberg-spark-sql-merge-mixed-branches-{token}"
    client = boto3.client(
        "s3",
    ...
    try:
        ...
        plan = build_sql_merge_mixed_branches_plan(args.source)
        ...
        checks = {
            ...
            "sql_merge_mixed_branches_passed": all(merge["checks"].values()),
            ...
            "mixed_branch_object_graph_materialized": inventory["metadata_json_count"] >= 3
            and inventory["manifest_avro_count"] >= 4
            and inventory["data_parquet_count"] >= 5,
        }
        report = {
            "schema": "gda.chongqing_osm_spark_flink_sql_merge_mixed_branches.acceptance.v1",
            ...
            "source": {
                **plan["source"],
                ...
                "inserted_road_ids": plan["inserted_road_ids"],
                ...
                "matched_tokens": [
                    source["commit_token"] for source in plan["matched_source_rows"]
                ],
                "insert_tokens": [
                    plan["priority_insert_source_row"]["commit_token"],
                    plan["default_insert_source_row"]["commit_token"],
                ],
            },
            "runtime": {
            ...
            "control_plane": {
                ...
                "merge_branches": [
                    "matched_delete_when_action_delete",
                    "matched_update_default",
                    "not_matched_priority_insert_when_action_insert_priority",
                    "not_matched_default_insert",
                ],
                "writer_recovery_mode": "explicit_fresh_state",
            },
            "not_claimed": [
                (
                    "MERGE additional branch combinations, multiple target rows, "
