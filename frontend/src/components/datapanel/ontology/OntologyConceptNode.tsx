...
export default function OntologyConceptNode({ data, selected }: NodeProps) {
  ...
  return (
    <div className={`ontology-node ontology-node-${value.kind || 'concept'} ${selected ? 'selected' : ''}`}>
      ...
      <div className="ontology-node-foot">
        ...
        <span>{value.kind === 'ReferenceConcept' || value.kind === 'ValueDomainMember' ? '代码项' : value.kind?.endsWith('Class') ? '语义类' : `${value.propertyCount || 0} 字段`}</span>
      </div>
