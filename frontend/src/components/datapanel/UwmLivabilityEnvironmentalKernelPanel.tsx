...
import { AlertTriangle, CloudSun, Map, RefreshCw, Shield } from 'lucide-react';
...
export default function UwmLivabilityEnvironmentalKernelPanel() {
  ...
  const load = async () => {
    ...
    try {
      const [sceneResponse, gateResponse, mapResponse] = await Promise.all([
      ...
      const [sceneData, gateData, mapData] = await Promise.all([sceneResponse.json(), gateResponse.json(), mapResponse.json()]);
      if (!sceneResponse.ok || !gateResponse.ok || !mapResponse.ok) throw new Error(sceneData.error || gateData.error || mapData.error || '环境 Kernel 产品不可用');
  ...
  return <div className="uwm-livability-panel">
    ...
    <p>not_a_causal_effect_estimate 固定为 true；证据不足时接口返回冲突，不生成降温或降污染数字。</p>
  </div>;
}
