...
import { Download, FileUp, RefreshCw, Search, ShieldAlert, Upload } from 'lucide-react';
...
export default function OfflineIngestTab() {
  ...
  return (
    <div style={{ padding: 14, color: '#0f172a', fontSize: 13, overflowY: 'auto', height: '100%' }}>
      ...
      <section style={{ borderTop: '1px solid #e2e8f0', paddingTop: 12 }}><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}><div style={{ fontWeight: 600 }}>标准合同证据</div><span style={{ color: '#64748b', fontSize: 11 }}>{contracts?.authority || 'not_configured'} · {contractEntries.length} 个合同</span></div>{contracts?.authority !== 'ea_standard' && <div style={{ display: 'flex', gap: 6, alignItems: 'center', color: '#a16207', background: '#fffbeb', padding: 8, borderRadius: 4, marginBottom: 8 }}><ShieldAlert size={15} />当前工作簿/截图只能做候选匹配，不能自动发布。</div>}<div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>{contractEntries.slice(0, 18).map((contract) => <span key={contract.code} style={{ border: '1px solid #e2e8f0', borderRadius: 4, padding: '4px 7px', background: '#fff' }}>{contract.code} <span style={{ color: '#64748b' }}>{contract.fields?.length || 0} 字段</span></span>)}</div></section>
    </div>
  );
}
