...
The dedicated worktree has local-only, ignored links for deterministic re-execution:

- `data_agent/uploads/data_products` -> primary worktree real product data;
- `.tmp/connector-cache` -> primary worktree frozen Flink/Iceberg/JDBC artifacts.

Neither link nor any referenced payload is staged or pushed.

## Branch Integration Boundary

- The continuation branch is based on the pushed AR-2 real-source baseline and preserves that
  branch's metadata-fabric migrations `097-101` byte-for-byte.
- Only new migrations `102_source_schema_drift_ledger`,
  `103_unified_approval_case_authority`, and `104_source_sync_checkpoint_authority` are included.
- A parallel uncommitted line in the primary worktree also uses migration numbers `100/101` for
  DataProduct registry/promotion. Those files, the dependent full DataOps executor, and its DEM HTTP
  wiring test are deliberately excluded until they receive new forward-only migration identities.
- DEM source adapter, native-raster ODS materialization, staging logic, and historical ADR evidence
  remain included; do not restore the excluded migrations by overwriting the metadata-fabric files.
- ApprovalCase's four authenticated API operations were merged onto the existing Starlette route
  boundary without removing the branch's active-metadata gateway behavior.
- Clean-worktree verification completed with real Chongqing input and frozen connector artifacts:
  `171 passed, 1 skipped`; all new Python files pass Ruff, all changed Python files pass Ruff's
  fatal-error rules, `compileall` passes, and `git diff --check` is clean.
...
```bash
cd /Users/zhouning/gisdataagent/.worktrees/ar2-geospatial-lakehouse-runtime
git fetch origin
git status --short
...
```bash
PYTHONPATH=$PWD /Users/zhouning/gisdataagent/.venv/bin/pytest -q \
