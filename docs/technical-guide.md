...
**Version**: v23.0 &nbsp;|&nbsp; **Framework**: Google ADK v1.27.2 &nbsp;|&nbsp; **Date**: 2026-04-10
...
29. [上下文工程与反馈飞轮](#29-上下文工程与反馈飞轮)
30. [矢量切片与大数据渲染](#30-矢量切片与大数据渲染)
31. [统一模型网关](#31-统一模型网关)
32. [遥感智能体](#32-遥感智能体)
33. [离线模式与具身执行](#33-离线模式与具身执行)
34. [分析意图消歧与 NL2Workflow](#34-分析意图消歧与-nl2workflow)
35. [v16.0 → v23.0 演进总结](#35-v160--v230-演进总结)
...
│   Service Worker (sw.js) — 离线缓存地图瓦片/静态资源/用户数据   │
├─────────────────────────────────────────────────────────────────┤
│                    REST API Layer (276+ endpoints, 22 route modules)    │
│   Starlette Routes → mount before Chainlit catch-all           │
│   api/ sub-package: causal, world_model, quality, metadata...  │
│   RateLimitMiddleware + CircuitBreakerMiddleware (Starlette)    │
├─────────────────────────────────────────────────────────────────┤
│                    Chainlit Application Layer                   │
│   Auth Callbacks │ Session Mgmt │ File Upload │ Intent Router   │
│   Intent Disambiguation v2 (subtask decompose + wave execution)│
├─────────────────────────────────────────────────────────────────┤
...
│   41 BaseToolset (240+ tools) │ 26 ADK Skills │ MCP Hub        │
│   UserToolset │ SemanticOperators │ ToolEvolution               │
├─────────────────────────────────────────────────────────────────┤
│                    Context & Intelligence Layer                 │
│ ContextEngine (6 providers) │ FeedbackLoop │ ReferenceQueryStore│
│ MetricFlow 语义模型 │ PromptOptimizer │ EvaluatorRegistry      │
├─────────────────────────────────────────────────────────────────┤
│                    Data & Infrastructure                        │
...
│ Remote Sensing │ Vector Tiles │ Annotation WS │ Embodied Exec  │
└─────────────────────────────────────────────────────────────────┘
```
...
```
data_agent/
├── app.py              # 入口 (3948 行): Chainlit UI, RBAC, 文件上传, 管线调度
├── agent.py            # Agent 定义 (853 行): 管线组装, 工厂函数, 13 子 Agent
├── intent_router.py    # 语义路由 (290 行): classify_intent, 多语言检测, 意图消歧
├── pipeline_helpers.py # 管线辅助 (365 行): 工具说明, 进度渲染, 错误分类
├── pipeline_runner.py  # 无头执行器 (360 行): run_pipeline_headless
├── frontend_api.py     # REST API (3438 行): 127 个端点 + api/ 子包 153 端点
├── auth.py             # 认证 (492 行): 密码/OAuth/暴力破解防护(5次/15分钟锁定)
├── user_context.py     # 上下文传播 (37 行): 6 个 ContextVar
├── db_engine.py        # 数据库单例 (102 行): 连接池 (pool_size=20)
├── db_engine_async.py  # 异步数据库引擎 (154 行): asyncpg 连接池 + fetch/execute
...
├── context_engine.py   # 统一上下文引擎 (583 行): 6 provider + Token 预算 (v19.0)
├── feedback.py         # 反馈飞轮 (368 行): 👍👎 + 失败分析 + 自动入库 (v19.0)
├── reference_queries.py # 参考查询库 (395 行): embedding 搜索 + NL2SQL few-shot (v19.0)
├── semantic_model.py   # 语义模型 (338 行): MetricFlow GIS 扩展 YAML (v19.0)
├── model_gateway.py    # 统一模型网关 (449 行): 在线(Gemini)+离线(LM Studio/Gemma)+LiteLLM
├── remote_sensing.py   # 遥感智能体 (974 行): 变化检测+时序分析+空间约束验证 (v22.0)
├── annotation_ws.py    # 标注协同 WebSocket (87 行): 单实例广播 (v23.0)
├── embodied.py         # 具身执行接口 (234 行): BaseExecutor + MockUAV/Satellite (v23.0)
├── lite_mode.py        # Lite 模式 (206 行): DuckDB 轻量部署 (v22.0)
├── nl2workflow.py      # NL2Workflow (309 行): 自然语言→可执行工作流 DAG (v23.0)
├── prompt_optimizer.py # 提示词优化 (436 行): bad case 收集 + 失败分析 (v23.0)
├── evaluator_registry.py # 评估器注册表 (665 行): 15 内置评估器 (v23.0)
├── otel_tracing.py     # OTel 埋点 (167 行): pipeline/agent/tool/llm 4 级 span (v23.0)
├── tile_server.py      # 矢量切片 (267 行): MVT + ST_AsMVT + 过期清理 (v17.1)
├── circuit_breaker.py  # 熔断器 (150 行): 故障容错
├── prompts/            # 3 个 YAML: optimization, planner, general
├── toolsets/            # 41 BaseToolset 子类 (.py 文件)
├── skills/             # 26 个 ADK Skill 目录
├── api/                # 22 个路由子模块 (causal, world_model, quality, lineage, feedback 等)
├── fusion/             # 20 模块: 多模态数据融合 v2.0
├── connectors/         # 10 个连接器: BaseConnector + Registry + WFS/STAC/OGC/ArcGIS...
├── standards/          # 9 个 YAML 标准文件 (缺陷分类法, QC 模板, GB/T 标准)
├── migrations/         # 64 个 SQL 迁移文件
├── evals/              # 4 管线评测集
├── semantic_layer.py   # 语义目录 (1847 行)
├── data_catalog.py     # 数据资产目录 (1547 行)
├── knowledge_graph.py  # 地理知识图谱 (705 行)
├── drl_engine.py       # DRL 优化引擎 (1017 行): 7 场景 + NSGA-II + 约束建模
├── workflow_engine.py  # 工作流引擎 (1938 行): 顺序 + DAG + SLA + 节点重试
├── mcp_hub.py          # MCP Hub 管理器 (920 行): ToolRuleEngine
├── multimodal.py       # 多模态输入 (186 行)
├── observability.py    # 可观测性 (602 行): JSON 日志 + Prometheus + AlertEngine
├── health.py           # 健康检查 (283 行)
...
├── context_manager.py  # 上下文管理 (59 行): 可插拔 Provider + Token 预算
...
├── semantic_operators.py # 语义算子 (703 行): Clean/Integrate/Analyze/Visualize
├── guardrails.py       # 护栏引擎 (398 行): YAML 驱动工具访问控制
├── plan_refiner.py     # 计划修正 (206 行): 5 种恢复策略
├── tool_evolution.py   # 工具演化 (694 行): 统一元数据 + 失败驱动发现
├── failure_learning.py # 失败学习 (158 行)
├── graph_rag.py        # GraphRAG (507 行): 图谱增强检索
├── a2a_server.py       # A2A 协议 (264 行): Agent Card + Task 生命周期
├── agent_registry.py   # Agent 注册 (176 行): 服务发现 + 心跳
├── analysis_chains.py  # 条件分析链 (222 行): 自动后续分析
├── plugin_registry.py  # 插件注册 (113 行): DataPanel Tab 动态注册
└── test_*.py           # 171 个测试文件 (3586+ 测试)
```
...
def mount_frontend_api(app: Starlette):
    ...
    # frontend_api.py 定义 127 个端点
    # api/ 子包定义 153 个端点 (22 个路由模块)
    routes = [
...
| MCP Hub | 14+ | mcp_routes | `/api/mcp/servers` CRUD + toggle + reconnect + test + share |
| 工作流 | 15+ | workflow_routes | `/api/workflows` CRUD + execute + history + SLA |
| 分析 | 5 | frontend_api | `/api/analytics/latency`, `/api/analytics/tool-success` |
| 自定义技能 | 15+ | skills_routes | `/api/skills` CRUD + generate (AI 辅助) + AI Workflow |
| 技能包 | 6 | bundle_routes | `/api/bundles` CRUD |
| 知识库 | 10+ | kb_routes | `/api/kb/*` 检索 + 案例库 |
| 用户工具 | 6 | frontend_api | `/api/user-tools` CRUD |
| 虚拟数据源 | 10+ | virtual_routes | `/api/virtual-sources` CRUD + 预览 |
| 因果推断 | 10+ | causal_routes | `/api/causal/*` (A/B/C 三角度) |
| World Model | 8+ | world_model_routes / causal_world_model_routes | `/api/world-model/*` |
| 质检 | 22+ | quality_routes | `/api/qc/reviews` + 工作流 + 缺陷分类 |
| 元数据 | 4+ | metadata_routes | `/api/metadata/*` |
| 消息总线 | 3+ | messaging_routes | `/api/messages/*` |
| 拓扑 | 3+ | topology_routes | `/api/topology/*` |
| 分发 | 9+ | distribution_routes | `/api/distribution/*` 审批 + 打包 |
| 文件 | 9+ | file_routes | `/api/files/*` |
| 矢量切片 | 5 | tile_routes | `/api/tiles/*` MVT 切片 + 资产编码 |
| 融合 v2 | 5 | fusion_v2_routes | `/api/fusion-v2/*` 时序 + 冲突解决 |
| 血缘追踪 | 5 | lineage_routes | `/api/lineage/*` 跨系统血缘 |
| 反馈 | 5 | feedback_routes | `/api/feedback/*` 👍👎 + 分析 |
| 参考查询 | 6 | reference_query_routes | `/api/reference-queries/*` |
| 语义模型 | 5 | semantic_model_routes | `/api/semantic-model/*` MetricFlow |
| 上下文 | 2 | context_routes | `/api/context/preview` |
...
**不足**：
- `frontend_api.py` 仍保留 127 个核心端点（3438 行），进一步拆分空间依然存在
...
`drl_engine.py`（1017 行）实现 `LandUseOptEnv`，支持 7 个优化场景：
...
| **优化场景** | 耕地优化, 城市绿地, 设施选址, 交通网络, 公共设施布局, 综合规划, 道路网络 (7 个) |
| **约束建模** | 硬约束 min_retention_rate (action mask) + 软约束 budget_cap/max_area_cap (reward penalty) |
| **DRL 动画** | GIF 优化过程回放 + 前后对比 PNG (v22.0) |
...
26 个细粒度场景技能，位于 `data_agent/skills/` 下，每个技能是一个 kebab-case 目录：
...
### 17.1 结构化日志 (`observability.py`, 602 行)
...
v15.0+ 增加：
- **OpenTelemetry 追踪** (`otel_tracing.py`, 167 行): pipeline/agent/tool/llm 4 级 span + graceful degradation
- **AlertEngine**: 可配置阈值告警规则 + Webhook 推送（`alert_rules` 表）
- **决策追踪** (`agent_decision_tracer.py`): Agent 决策路径记录

v23.0 增加：
- **OTel 埋点全覆盖**: 4 级 span (pipeline → agent → tool → llm)，优雅降级（OTel 不可用时静默跳过）
- **API 安全中间件**: `RateLimitMiddleware` + `CircuitBreakerMiddleware` (Starlette 层)
...
**同步引擎** (`db_engine.py`, 102 行): 单例 SQLAlchemy Engine，全局共享连接池：
    ...
    if _engine is None:
        ...
        if url:
            _engine = create_engine(url,
                pool_size=20,         # v18.0: 5→20 持久连接
                max_overflow=10,      # 高峰期额外 10 个 (总计 max 30)
...
**异步引擎** (`db_engine_async.py`, 154 行, v18.0): asyncpg 连接池单例 + fetch/execute 便利函数：

```python
async def get_async_pool():
    """asyncpg 连接池单例，支持读写分离预埋"""
    # pool_size=20, max_size=30
    # 提供 fetch_rows(), execute_query() 便利函数
```

**Prometheus 连接池监控** (v18.0): 活跃连接数、等待队列长度、连接创建/回收计数。

### 18.2 数据库表清单

系统使用 50+ 张业务表（64 个迁移文件）：
...
| `agent_feedback` | 用户反馈 👍👎 (v19.0) |
| `agent_reference_queries` | 参考查询库 (v19.0) |
| `agent_semantic_models` | 语义模型定义 (v19.0) |
| `agent_lineage_edges` | 跨系统血缘边 (v21.0) |
| `agent_external_assets` | 外部资产注册 (v21.0) |
| `agent_model_configs` | 模型配置持久化 (v23.0) |

### 18.3 迁移框架

系统使用 **64 个 SQL 迁移文件**（`data_agent/migrations/`）+ `migration_runner.py` 自动执行：

- 迁移文件按编号顺序执行（001-064）
- `migration_runner.py` 在应用启动时检查并执行未运行的迁移
- 仍有部分表通过 `ensure_*_table()` 函数创建（33 个 ensure 函数）
- 迁移覆盖：虚拟数据源、技能包、工作流模板、任务队列、数据分类分级、分发反馈、版本快照、DRL 历史、SLA、MCP 规则、告警规则、案例库、QC 复核、元数据系统、Prompt 注册、模型网关、评测场景、反馈表、参考查询、语义模型、跨系统血缘、模型配置等

### 18.4 架构评价

**优势**：
- 64 个 SQL 迁移文件 + migration_runner.py 提供了版本化的 schema 迁移能力
- v18.0 连接池优化 (5→20) + asyncpg 异步引擎显著提升并发性能
- 迁移覆盖了 v14.5 以来的全部新功能表

**不足**：
- **连接池已优化**：v18.0 将 `pool_size` 从 5 提升到 20，并增加 asyncpg 异步引擎
...
- **171 个测试文件** / **3586+ 测试函数**（v23.0）
- 覆盖：管线端到端、工具单元测试、API 端点、认证、融合、因果推断、World Model、QC 工作流、上下文引擎、反馈飞轮、遥感、DRL 约束等
- 可插拔评估器注册表 (`evaluator_registry.py`, 665 行): 15 内置评估器（质量/安全/性能/准确性）
...
### 25.3 Model Gateway (`model_gateway.py`, 449 行)

v23.0 大幅扩展为统一模型网关：

- **在线模型**: Gemini 2.0 Flash / 2.5 Flash / 2.5 Pro (Vertex AI)
- **离线模型**: LM Studio / Gemma 4 31B (vLLM 双路径)
- **LiteLLM 适配**: 统一 API 接口，支持动态注册新模型
- **任务感知路由**: 基于 task_type, context_tokens, quality_requirement, budget 自动选择模型
- **ModelConfigManager**: DB 持久化管理员模型配置，前端交互式切换
- **成本追踪**: scenario/project_id 归因到 `agent_token_usage` 表

### 25.4 Context Manager (`context_manager.py`, 59 行)

可插拔上下文提供者 + Token 预算执行。`prepare()` 方法收集语义层、知识库、标准库等多源上下文。

> 注：v19.0 引入了更完整的 `ContextEngine` (583 行)，见第 29 章。
...
| 编号 | 问题 | 严重性 | 状态 |
|------|------|--------|------|
| S-1 | `app.py` 职责过重 | 高 | 部分缓解：已拆分 `intent_router.py` + `pipeline_helpers.py`，但 app.py 仍有 3948 行 |
| S-2 | 模块级全局可变状态 10+ 处 | 高 | 大部分已修复：主要全局状态均已加锁保护 |
| S-3 | 数据库迁移管理 | 中 | ✅ 已有 64 个 SQL 迁移文件 + `migration_runner.py`，但非 Alembic |
| S-4 | `frontend_api.py` 端点过多 | 中 | ✅ 已拆分 `api/` 子包（22 个路由模块），但主文件仍保留 127 端点 / 3438 行 |
...
| T-5 | 测试覆盖 | ✅ 171 文件 / 3586+ 测试函数 |

### 28.8 架构优势总结

v23.0 系统在以下方面展现了成熟且持续演进的架构设计：

1. **语义路由 + 多管线分发**：Gemini Flash 低成本路由 → 高能力模型推理；多语言检测（zh/en/ja）；意图消歧 v2（子任务分解 + wave 按序执行）
2. **ContextVar 多租户隔离**：零侵入式用户上下文传播，6 个 ContextVar
3. **质量保证循环**：Generator + Critic 的 LoopAgent 模式，3 条管线均有自动质量自检
4. **虚拟数据层**：10 个连接器 + Fernet 加密 + 语义 schema 映射
5. **MCP Server v2.0**：36+ 工具暴露 + ToolRuleEngine 自动选择
6. **数据融合包**：20 模块 / 10 种策略 + PostGIS 下推 + 时序融合 + 冲突解决
7. **懒加载工具注册表**：`_RegistryProxy` 避免导入 41 重型 toolset 类
8. **用户自扩展生态**：Custom Skills（版本/评分/克隆/审批/AI 生成/AI Workflow）+ User Tools（4 模板）+ Workflow Templates + Plugin Registry
9. **多 Agent 编排**：DAG 工作流 + 节点级重试 + SLA + A2A 协议 + Agent Registry
10. **DRL 优化深度**：7 场景 + NSGA-II + MaskablePPO + 硬/软约束建模 + World Model Dreamer + GIF 动画
...
14. **BCG 平台能力**：Prompt Registry + 统一模型网关 (Gemini+Gemma+LiteLLM) + Context Manager + Eval Scenario
15. **L3 自主能力**：4 语义算子 + 计划修正 + 错误恢复 + 护栏引擎 + 工具演化
16. **上下文工程**：ContextEngine 6 provider + FeedbackLoop 反馈飞轮 + ReferenceQueryStore + MetricFlow 语义模型
17. **遥感智能体**：变化检测 (3 方法) + Mann-Kendall 时序 + 空间约束 fact-check + 多 Agent Debate
18. **离线与具身**：Service Worker 离线缓存 + 具身执行接口 (UAV/Satellite) + Lite 模式 (DuckDB)
19. **端到端评测**：4 管线独立评测 + 场景化评测 + 15 内置评估器 + CI 集成
20. **失败学习机制**：失败提取 → 提示注入 → 自动恢复闭环
21. **无头管线执行器**：`pipeline_runner.py` 零 UI 依赖
22. **API 安全**：RateLimitMiddleware + CircuitBreakerMiddleware + OTel 4 级 span
23. **标注协同**：WebSocket 实时广播 + 跨图层关联高亮

### 28.9 当前优先修复建议

v23.0 中大部分历史缺陷已修复，剩余待修复项：
...
### 29.1 概述 (v19.0 — Datus.ai 对标)

从"一次性回答"转向"越用越准的上下文飞轮"。参照 Datus.ai 的上下文工程理念，构建统一上下文引擎 + 结构化反馈闭环。

### 29.2 ContextEngine (`context_engine.py`, 583 行)

统一上下文引擎，6 个可插拔 Provider：

| Provider | 数据源 | 用途 |
|----------|--------|------|
| SemanticProvider | semantic_layer.py | 语义目录 + 域层级 |
| KnowledgeBaseProvider | knowledge_base.py | 知识库文档检索 |
| KnowledgeGraphProvider | knowledge_graph.py | 图谱关系查询 |
| ReferenceQueryProvider | reference_queries.py | 历史成功查询 few-shot |
| SuccessStoryProvider | feedback.py | 用户正反馈案例 |
| MetricDefinitionProvider | semantic_model.py | 指标定义 + 计算公式 |

Token 预算执行：按 relevance score 排序，截断到预算上限。

### 29.3 FeedbackLoop (`feedback.py`, 368 行)

结构化反馈闭环：
- 前端 `FeedbackBar.tsx`: 每条消息 👍👎 + 可选文本反馈
- `agent_feedback` 表: 存储反馈记录
- **Upvote 路径**: 正反馈查询自动入库 `ReferenceQueryStore` → 后续 NL2SQL few-shot 注入
- **Downvote 路径**: `FailureAnalyzer` 分析失败原因 → 提示词优化建议

### 29.4 ReferenceQueryStore (`reference_queries.py`, 395 行)

参考查询库：embedding 搜索 + 自动/手动策展。成功查询自动入库，为 NL2SQL 提供 few-shot 示例。

### 29.5 MetricFlow 语义模型 (`semantic_model.py`, 338 行)

GIS 扩展 YAML 格式的语义模型定义 + PostGIS 自动生成器。支持指标定义、维度声明、计算公式。
...
三级自适应渲染策略，解决大数据量地图渲染性能问题：

| 数据量 | 策略 | 技术 |
|--------|------|------|
| < 5000 要素 | GeoJSON 直传 | Leaflet / deck.gl |
| 5000-50000 | FlatGeobuf 流式 | 渐进式加载 |
| > 50000 | MVT 矢量切片 | Martin + ST_AsMVT |

### 30.2 Tile Server (`tile_server.py`, 267 行)

- 临时表管理：大数据集导入 PostGIS 临时表
- `ST_AsMVT` 查询：按 z/x/y 生成 MVT 切片
- 过期清理：定时清理过期临时表
- 数据资产编码：`DA-{TYPE}-{SRC}-{YEAR}-{SEQ}` 格式

### 30.3 REST API

5 个切片端点 (`tile_routes.py`, 196 行)：切片生成、资产注册、状态查询。
...
从单一 Gemini 模型扩展为在线/离线多模型统一网关：

```
用户请求 → ModelGateway.select_model(task_type, context)
    ├── 在线: Gemini 2.0 Flash / 2.5 Flash / 2.5 Pro (Vertex AI)
    ├── 离线: Gemma 4 31B (Gemini API + vLLM 双路径)
    └── LiteLLM: 统一适配层 (支持动态注册)
```

### 31.2 ModelConfigManager

DB 持久化管理员模型配置 (`agent_model_configs` 表)：
- 前端交互式模型切换 UI
- 可配置 Intent Router 模型
- 按任务类型自定义模型偏好

### 31.3 任务感知路由

基于 task_type, context_tokens, quality_requirement, budget 自动选择最优模型。成本追踪支持 scenario/project_id 归因。
...
`remote_sensing.py` (974 行) 实现三阶段遥感分析能力：

**Phase 1** (v16.0): 基础遥感工具
- 15+ 光谱指数计算 (NDVI, NDWI, NDBI, EVI, SAVI...)
- 栅格分类、DEM/LULC 下载、经验池

**Phase 2** (v22.0): 时空分析
- 变化检测 (3 方法): 差值法、比值法、分类后比较
- Mann-Kendall 趋势检验 + 断点检测
- 证据评估框架

**Phase 3** (v22.0): 空间约束验证
- 空间约束 fact-check: 结果与已知空间规律交叉验证
- 多 Agent Debate: 多个 Agent 独立分析后辩论达成共识
- 代码沙箱: 安全执行用户自定义遥感分析脚本

### 32.2 RS 知识库

5 光谱指数定义 + 3 分类体系 + 3 处理流程模板，为遥感 Agent 提供领域知识注入。

---

## 33. 离线模式与具身执行

### 33.1 Service Worker (`frontend/public/sw.js`)

v23.0 实现离线模式支持：
- **静态资源缓存**: JS/CSS/HTML 预缓存 (`gis-agent-v23`)
- **地图瓦片缓存**: OpenStreetMap, 高德, 天地图, CartoDB, ESRI (`gis-agent-tiles-v23`)
- **API 数据缓存**: Network-first 策略 + 离线回退 (`gis-agent-data-v23`)

### 33.2 Lite 模式 (`lite_mode.py`, 206 行)

DuckDB 轻量部署，无需 PostGIS：
- `gis-agent init` CLI 入口初始化 DuckDB
- 治理/优化路由降级到 General 管线
- 可选依赖分组: `pyproject.toml` [lite]/[full]/[dev]

### 33.3 具身执行接口 (`embodied.py`, 234 行)

`BaseExecutor` ABC + 注册表模式：
- `MockUAVExecutor`: 无人机航线规划模拟
- `MockSatelliteExecutor`: 卫星任务调度模拟
- 可扩展：实际硬件对接只需实现 `execute()` 方法

### 33.4 标注协同 WebSocket (`annotation_ws.py`, 87 行)

单实例广播模式：标注创建/更新/删除事件实时推送到所有连接客户端。与 REST API 集成。

---

## 34. 分析意图消歧与 NL2Workflow

### 34.1 意图消歧 v2 (v23.0)

复杂分析请求的子任务分解 + wave 按序执行：

```
用户: "分析这个区域的土地利用变化并优化耕地布局"
    ↓ should_decompose() → True
    ↓ decompose_task() → [子任务1: 土地利用变化分析, 子任务2: 耕地布局优化]
    ↓ 用户确认分解方案
    ↓ wave 按序执行 (依赖关系自动排序)
```

### 34.2 NL2Workflow (`nl2workflow.py`, 309 行)

自然语言描述 → 可执行工作流 DAG：
- LLM 解析用户意图为结构化工作流步骤
- 自动匹配可用工具和技能
- 生成 WorkflowEditor 可编辑的 DAG 定义

### 34.3 提示词优化 (`prompt_optimizer.py`, 436 行)

Bad case 收集 + 失败分析 + prompt 自动改进：
- 从 `agent_feedback` 表收集负反馈
- 分析失败模式（工具选择错误、参数错误、幻觉等）
- 生成 prompt 改进建议
...
| 维度 | v16.0 (2026-04-01) | v23.0 (2026-04-10) | 变化 |
|------|--------------------|--------------------|------|
| REST 端点 | 228+ | 276+ | +48 (22 路由子模块) |
| 测试数量 | ~2966 / 135 文件 | ~3586 / 171 文件 | +620 / +36 文件 |
| DataPanel Tabs | 29 (4 组) | 30+ (4 组) | +FeedbackTab |
| 工具集 | 40+ | 41 | 稳定 |
| ADK Skills | 23 | 26 | +3 (spectral-analysis/spatial-clustering/satellite-imagery) |
| 数据库迁移 | 48 SQL | 64 SQL | +16 |
| 数据库表 | 40+ | 50+ | +10 (feedback/reference_queries/semantic_models/lineage/model_configs...) |
| DRL 场景 | 5 + Dreamer | 7 + Dreamer + 约束建模 | +2 场景 + 硬/软约束 |
| 新增后端模块 | — | context_engine, feedback, reference_queries, semantic_model, remote_sensing, annotation_ws, embodied, lite_mode, nl2workflow, prompt_optimizer, evaluator_registry, otel_tracing, tile_server, db_engine_async | +14 模块 |
| 模型网关 | 100 行 (单 Gemini) | 449 行 (Gemini+Gemma+LiteLLM) | 多模型统一网关 |
| 离线支持 | — | Service Worker + DuckDB Lite | 新增 |
| 遥感能力 | Phase 1 | Phase 1-3 (变化检测+时序+Debate) | +2 阶段 |
| API 安全 | — | RateLimitMiddleware + CircuitBreakerMiddleware | 新增 |
| OTel 埋点 | 基础 | 4 级 span (pipeline/agent/tool/llm) | 全覆盖 |
| 上下文工程 | — | ContextEngine 6 provider + FeedbackLoop | 新增 |

---

*本文档基于 GIS Data Agent v23.0 代码库（2026-04-10）更新。系统包含 3948 行 app.py、3438 行 frontend_api.py（276+ 端点）、30+ DataPanel tabs、41 BaseToolset、26 ADK Skills、64 个数据库迁移、171 个测试文件 / 3586+ 测试函数。涵盖 v16.0 → v23.0 全部新增功能：上下文工程、反馈飞轮、矢量切片、统一模型网关、遥感 Phase 2-3、离线模式、具身执行、意图消歧 v2、NL2Workflow、DRL 约束建模、API 安全中间件、OTel 全覆盖。*
