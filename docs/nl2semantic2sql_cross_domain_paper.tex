...
\begin{table}[t]
\centering
\caption{GIS Robustness Success Rate: 15 safety/refusal questions. 95\% Wilson CIs in brackets. McNemar test: $b=0$ (base-OK/full-ERR), $c=7$ (base-ERR/full-OK).}
...
Table~\ref{tab:bird} reports BIRD execution accuracy at $\sim$495-question scale using single-pass mode (P2).
...
\begin{table}[t]
\centering
\caption{BIRD $\sim$495-question benchmark results (single-pass P2 mode). 95\% Wilson CIs shown for overall EX. McNemar test: $b=26$, $c=39$, $n=495$.}
\label{tab:bird}
\begin{tabular}{lcccc}
\toprule
...
For reference, the earlier 500-question multi-pass run showed: baseline $0.458$, full $0.450$ ($-0.008$, McNemar $p=0.8151$). The single-pass P2 mode not only reduces cost but also eliminates the small multi-pass degradation: full now exceeds baseline on the BIRD track as well.
...
\noindent\textbf{Released artifacts.} We release the 100-question GIS benchmark with gold SQL (see project repository, \texttt{benchmarks/} directory), the DIN-SQL runner adapted for PostgreSQL/PostGIS, the evaluation harness with McNemar and Wilson CI tools, and the SQL postprocessor and self-correction logic. Per-question predicted SQL and execution results are available in the project repository.

\noindent All runs use Gemini~2.5~Flash with temperature$=0.0$, default top\_p, and default maximum output tokens. Each question has a 60-second execution timeout; on \texttt{429 RESOURCE\_EXHAUSTED}, the runner retries up to 3 times with 2-second initial delay. On no-SQL output, the question is recorded as \texttt{gen\_status=no\_sql} and EX$=0$. The BIRD evaluation uses the full \texttt{mini\_dev\_postgresql.json} (500 questions); 495 questions completed (5 errors/timeouts). Paired McNemar uses the $n=495$ intersection.
