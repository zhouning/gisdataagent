...
% Final update: GIS split Spatial/Robustness (p=0.0072/p=0.0156), BIRD 495q (EX=0.501), DIN-SQL 20q ref, CIs, token cost.
% =============================================================================
...
\author{Ning Zhou \\ \textit{Beijing SuperMap Software Co., Ltd., Beijing, China} \\ \texttt{zhouning@supermap.com}}
\date{}
...
Table~\ref{tab:dinsql} compares NL2Semantic2SQL against DIN-SQL~\cite{pourreza2023dinsql} as an external prompting baseline on both tracks.

\begin{table}[t]
\centering
\caption{Three-way comparison: Baseline vs.\ DIN-SQL vs.\ Full pipeline. DIN-SQL on GIS is evaluated on the 20-question reference subset only; a 100-question re-run is in progress (see footnote). DIN-SQL on BIRD uses our $\sim$495-question evaluation set.}
\label{tab:dinsql}
\begin{tabular}{lccccc}
\toprule
Track & $N$ & Baseline EX & DIN-SQL EX & Full EX & Full vs.\ DIN-SQL \\
\midrule
GIS Spatial 85q  & 85  & 0.529 & 0.650$^*$ & \textbf{0.682} & $+$0.032 \\
GIS Robust. 15q  & 15  & 0.333 & 0.000$^*$ & \textbf{0.800} & $+$0.800 \\
BIRD $\sim$495q  & $\sim$495 & 0.474 & 0.482 & \textbf{0.501} & $+$0.019 \\
\bottomrule
\end{tabular}

\smallskip
\noindent\footnotesize{$^*$DIN-SQL was originally evaluated on a 20-question subset; a 100-question re-run is reported in Section~\ref{sec:dinsql} once complete. The 20-question result is shown as a reference and should not be directly compared to the Full pipeline's 85-question Spatial EX result. On the same 100-question benchmark, DIN-SQL achieves [TBD-100q] (results pending re-run); on the original 20-question subset, DIN-SQL matches the baseline at EX$=0.650$.}
\end{table}
...
\draw[fill=orange!50](1.4,0) rectangle (2.2,3.25);  \node[above,font=\scriptsize] at (1.8,3.25) {0.650$^*$};
...
\draw[fill=orange!50](5.1,0) rectangle (5.9,0.00);  \node[above,font=\scriptsize] at (5.5,0.05) {0.000$^*$};
...
\end{tikzpicture}
\caption{Three-way cross-domain comparison: Baseline / DIN-SQL / Full pipeline. GIS Spatial 85q: full pipeline Spatial EX$=0.682$ (McNemar $p=0.0072$, significant). GIS Robustness 15q: full pipeline $0.800$ vs.\ DIN-SQL $0.000$ ($p=0.0156$, significant). BIRD $\sim$495q: full pipeline $0.501$ vs.\ DIN-SQL $0.482$ (not significant, $p=0.382$). All three systems are statistically comparable on BIRD. $^*$DIN-SQL GIS evaluated on 20-question reference subset only; 100-question re-run pending.}
\label{fig:cross}
\end{figure}
...
DIN-SQL GIS 100q (pending) & GIS DIN-SQL & \texttt{cq\_din\_sql\_2026-05-04\_*} & Gemini~2.5~Flash \\
