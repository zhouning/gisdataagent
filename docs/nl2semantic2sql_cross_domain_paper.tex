...
% Final update: GIS 100q (p=0.0002), BIRD 495q (EX=0.501), DIN-SQL, CIs, token cost.
% =============================================================================
...
Table~\ref{tab:gis} reports the GIS benchmark results for the expanded 100-question benchmark (run \texttt{cq\_2026-05-04\_122349}), with 95\% Wilson confidence intervals.

\begin{table}[t]
\centering
\caption{GIS 100-question benchmark results (run \texttt{cq\_2026-05-04\_122349}). 95\% Wilson CIs in brackets. McNemar test: $b=4$ (base-OK/full-ERR), $c=24$ (base-ERR/full-OK).}
\label{tab:gis}
\begin{tabular}{lcccc}
\toprule
Metric & $N$ & Baseline EX [95\% CI] & Full EX [95\% CI] & $\Delta$ \\
\midrule
Overall EX   & 100 & 0.500 [0.404, 0.596] & \textbf{0.700 [0.604, 0.781]} & $+$0.200 \\
...
Robustness   & 15  & 0.333 [0.152, 0.583] & \textbf{0.800 [0.548, 0.930]} & $+$0.467 \\
\midrule
McNemar $p$  & 100 & \multicolumn{3}{c}{$p=0.0002$ (significant at $\alpha=0.001$)} \\
Mean tokens  & 100 & 753 & 10{,}261 & $13.6\times$ \\
\bottomrule
\end{tabular}
\end{table}

The full pipeline outperforms the baseline by $+0.200$ EX overall. The McNemar test ($b=4$, $c=24$, $n=100$) gives $p=0.0002$, which is statistically significant at $\alpha=0.001$. This represents a substantial improvement in statistical power compared to the 20-question pilot, where the same direction of effect ($p=0.125$) did not reach significance.
...
\begin{table}[t]
\centering
\caption{Three-way comparison: Baseline vs.\ DIN-SQL vs.\ Full pipeline. DIN-SQL on GIS evaluated on the 20-question subset; DIN-SQL on BIRD uses our $\sim$495-question evaluation set.}
\label{tab:dinsql}
\begin{tabular}{lccccc}
\toprule
Track & $N$ & Baseline EX & DIN-SQL EX & Full EX & Full vs.\ DIN-SQL \\
\midrule
GIS 100q  & 100 & 0.500 & 0.650$^*$ & \textbf{0.700} & $+$0.050 \\
BIRD $\sim$495q & $\sim$495 & 0.474 & 0.482 & \textbf{0.501} & $+$0.019 \\
\bottomrule
\end{tabular}

\smallskip
\noindent\footnotesize{$^*$DIN-SQL evaluated on GIS 20-question subset: EX$=0.650$, Robustness$=0.000$.}
\end{table}

On the GIS track, DIN-SQL exactly matches the baseline (EX$=0.650$ on the 20-question subset, Robustness$=0.000$), confirming that decomposed prompting without spatial grounding does not improve over direct LLM generation for geospatial SQL. The full NL2Semantic2SQL pipeline outperforms DIN-SQL by $+0.050$ EX on GIS.

On the BIRD track, DIN-SQL achieves EX$=0.482$ (Validity$=0.990$). The full pipeline achieves EX$=0.501$, exceeding DIN-SQL by $+0.019$. However, the difference between DIN-SQL and the full pipeline is not statistically significant (McNemar $p=0.382$), so the two methods should be considered comparable on warehouse queries. The three systems --- baseline, DIN-SQL, and NL2Semantic2SQL --- are statistically indistinguishable on the BIRD track.
...
\midrule
GIS 100   & 100 & 4  & 24 & \textbf{0.0002} & Significant ($\alpha=0.001$); $+0.200$ EX \\
BIRD $\sim$495 & 495 & 26 & 39 & 0.136 & Not significant; directional ($+0.027$ EX) \\
\bottomrule
\end{tabular}
\end{table}

The GIS result ($p=0.0002$) is statistically significant at $\alpha=0.001$. The 100-question benchmark provides adequate power to confirm the effect that was only directional at $n=20$ ($p=0.125$). The BIRD result ($p=0.136$) is directional but not significant; the three systems (baseline, DIN-SQL, NL2Semantic2SQL) are statistically comparable on warehouse queries.
...
\draw[->] (0,0) -- (14.5,0) node[right]{Metric};
...
% GIS 100q EX
\draw[fill=gray!40]  (0.5,0) rectangle (1.3,2.50); \node[above,font=\scriptsize] at (0.9,2.50) {0.500};
\draw[fill=orange!50](1.4,0) rectangle (2.2,3.25); \node[above,font=\scriptsize] at (1.8,3.25) {0.650};
\draw[fill=blue!50]  (2.3,0) rectangle (3.1,3.50); \node[above,font=\scriptsize] at (2.7,3.50) {0.700};
\node[font=\scriptsize,align=center] at (1.8,-0.55) {GIS 100q EX};
...
\node[font=\scriptsize,align=center] at (5.5,-0.55) {GIS Robust.};
...
\end{tikzpicture}
\caption{Three-way cross-domain comparison: Baseline / DIN-SQL / Full pipeline. GIS 100q: full pipeline achieves EX$=0.700$ (McNemar $p=0.0002$, significant). GIS Robustness: full pipeline $0.800$ vs.\ DIN-SQL $0.000$. BIRD $\sim$495q: full pipeline $0.501$ vs.\ DIN-SQL $0.482$ (not significant, $p=0.382$). All three systems are statistically comparable on BIRD.}
\label{fig:cross}
\end{figure}
...
Several limitations should be noted. First, while the GIS benchmark is now 100 questions, it remains a single-domain pilot focused on a Chinese geospatial database (Chongqing street/POI data); generalization to other GIS domains and non-Chinese schemas requires further evaluation. Second, on the BIRD track, the full pipeline's $+0.027$ EX advantage is directional but not statistically significant ($p=0.136$); the three-way comparison with DIN-SQL likewise shows no statistically significant differences. Third, the cross-lingual experiment uses LLM-translated questions without human verification. Fourth, our baselines are direct-LLM-with-schema-dump and DIN-SQL only; comparison against MAC-SQL and other recent approaches would be necessary to position the method against the full state-of-the-art. Fifth, execution-based evaluation treats any result-set mismatch as failure, even when the predicted SQL is semantically equivalent but differs in ordering or numeric precision. Sixth, token cost ($7.9$--$13.6\times$) is a non-trivial deployment concern that future work should address via selective grounding or context compression.
