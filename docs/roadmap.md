...
- [ ] **ContextEngine 统一抽象** — 新增 `context_engine.py`: 融合所有知识源为一个检索接口，替代 BCG context_manager.py 的简单实现
- [ ] **6 个 ContextProvider** — SemanticLayerProvider (现有) / KnowledgeGraphProvider (现有) / KnowledgeBaseProvider (现有) / ReferenceQueryProvider (新增) / SuccessStoryProvider (新增) / MetricDefinitionProvider (新增)
- [ ] **相关性排序** — 基于 query embedding + 任务类型对所有 provider 返回的上下文块进行统一排序
- [ ] **Token 预算截断** — 按相关性分数截断到 token_budget，确保不超出 LLM 上下文窗口
- [ ] **上下文缓存** — 相同 query + task_type 组合缓存 3 分钟，避免重复检索
- [ ] **Pipeline 集成** — Planner/Executor 在生成计划和执行工具前自动调用 `context_engine.prepare()`
- [ ] **REST API** — `GET /api/context/prepare` (预览上下文) + `GET /api/context/providers` (列出 provider 状态)
- [ ] **测试** — context_engine 统一检索 + provider 注册 + 排序 + 截断 + 缓存全覆盖
...
- [ ] **前端反馈 UI** — 每条 Agent 回答增加 👍/👎 按钮 + 可选 issue 描述弹窗 (ChatPanel 消息组件扩展)
- [ ] **agent_feedback 表** — Migration: query_text, response_text, vote (up/down), issue_description, pipeline_type, resolved_at, created_by
- [ ] **反馈收集 API** — `POST /api/feedback` (提交反馈) + `GET /api/feedback/stats` (反馈统计)
- [ ] **成功案例自动提取** — upvote 的查询自动提取为参考查询 (query + response + tags)，进入 ReferenceQueryProvider
- [ ] **失败模式分析管道** — 定期聚合 downvote 反馈 → 调用 prompt_optimizer.py FailureAnalyzer → 生成改进建议
- [ ] **反馈→上下文自动更新** — 成功案例 → SuccessStoryProvider；失败模式 → 触发 prompt 优化建议
- [ ] **反馈看板** — DataPanel 新增反馈统计子面板 (满意率趋势 / 高频失败模式 / 最近反馈列表)
- [ ] **测试** — 反馈 CRUD + 成功案例提取 + 失败分析管道 + 统计聚合全覆盖
...
- [ ] **GIS Semantic Model YAML 格式** — 扩展 MetricFlow YAML 规范，增加 `type: spatial` 维度 + `srid` 字段 + `geometry_type` 属性
- [ ] **自动生成器** — `gen_semantic_model` 工具: 从 PostGIS 表结构自动生成语义模型 YAML (entities + dimensions + measures + metrics)
- [ ] **semantic_layer.py 适配** — 现有三级层次结构保留为向后兼容，新增 MetricFlow YAML 读取器
- [ ] **语义模型 CRUD API** — `GET/POST/PUT/DELETE /api/semantic/models` (列出/创建/更新/删除语义模型)
- [ ] **MetricDefinitionProvider** — 从语义模型中提取指标定义，注入 ContextEngine
- [ ] **测试** — YAML 解析 + PostGIS 表结构提取 + 语义模型 CRUD + Provider 注入全覆盖
...
- [ ] **agent_reference_queries 表** — Migration: query_text, description, tags[], verified_by, use_count, success_rate, pipeline_type, created_by
- [ ] **ReferenceQueryProvider** — 实现 ContextProvider 接口，基于 query embedding 检索相似参考查询
- [ ] **自动入库** — 用户 upvote → 查询自动进入参考库 (关联 agent_feedback 表)
- [ ] **手动策展** — `POST /api/reference-queries` 手动添加参考查询 + `PUT` 编辑标签/描述
- [ ] **NL2SQL 增强** — NL2SQLToolset 执行前先检索参考库中的相似查询作为 few-shot 示例
- [ ] **REST API** — 6 端点 (CRUD + search + stats)
- [ ] **测试** — 参考查询 CRUD + 相似度检索 + NL2SQL few-shot 注入全覆盖

### 质量保障

- [ ] **test_context_engine.py** — 统一上下文引擎全流程
- [ ] **test_feedback_loop.py** — 反馈收集→学习→上下文更新闭环
- [ ] **test_semantic_model_metricflow.py** — MetricFlow YAML 解析+生成
- [ ] **test_reference_queries.py** — 参考查询库 CRUD + NL2SQL 集成

---

## v20.0 — 分布式任务队列与缓存 + 体验优化 (搁置)

> **状态**: ⏸️ 搁置 — 等待华为云 Redis 实例开通后启动
>
...
- [~] **celery_app.py** — Celery 应用配置，broker=Redis，backend=Redis *(搁置: 等待 Redis 实例)*
...
- [~] **华为云 DCS Redis** — 托管 Redis 实例 *(搁置: 等待开通)*
- [~] **semantic_layer.py** — 从内存缓存迁移到 Redis，TTL=300s
- [~] **session 存储** — Chainlit session 迁移到 Redis (多 Pod 共享)
- [~] **分布式锁** — Redis SETNX 实现分布式锁 (替代进程内锁)

### P2 — 多 LLM 一键切换体验 (Datus 对标)

> Datus: YAML 配置统一所有 LLM provider，一键切换 OpenAI/Claude/Gemini/DeepSeek/Qwen

- [ ] **统一 LLM 配置 YAML** — `conf/models.yaml` 声明式配置所有 LLM provider (type/base_url/api_key/model)，替代分散的环境变量
- [ ] **前端模型选择器** — UserSettings 面板增加 LLM 模型下拉选择 + 按任务类型分配 (路由=Flash, 分析=Pro, 推理=DeepSeek)
- [ ] **model_gateway.py 适配** — 读取 YAML 配置 + 动态注册，保持现有 API 兼容

### P2 — Agentic/Workflow 双模式 (Datus 对标)

> Datus: Agentic Mode (探索性) vs Workflow Mode (确定性)，用户按需切换

- [ ] **模式切换** — 前端增加 Agentic/Workflow 模式开关 (Header 区域)
- [ ] **Agentic Mode** — 现有语义路由 → Planner 自主决策 → 灵活探索 (默认模式)
- [ ] **Workflow Mode** — 选择预定义工作流 → 确定性步骤执行 → 无 LLM 中间决策，适合生产级批量处理
- [ ] **模式感知路由** — intent_router.py 增加 execution_mode 参数，Workflow 模式跳过 LLM 路由

### P2 — 轻量化部署选项 (Datus 对标)

> Datus: `pip install datus-agent && datus-agent init` 30 秒上手
...
- [ ] **DuckDB 适配器** — db_engine.py 增加 DuckDB 后端 (spatial 扩展加载)
...
### P3 — CLI 终端接口 (Datus 对标)

> Datus-CLI: "Claude Code for Data Engineers" — `/` chat + `@` context + `!` execute 三魔法命令
>
> 我们现状: 纯 Web UI，无终端入口。`cli_tui_evaluation.md` 已评估可行，结合 Datus 成功案例提升优先级。

- [ ] **gis-agent CLI 框架** — `cli/` 包: Click/Typer 命令行框架 + 复用 pipeline_runner.py (零 Chainlit 依赖)
- [ ] **chat 命令** — `gis-agent chat "分析碎片化程度"`: 自然语言对话 → 语义路由 → 管道执行 → 终端渲染
- [ ] **context 命令** — `gis-agent context @parcels.shp`: 加载文件/表到上下文 + `@table` 精确引用
- [ ] **exec 命令** — `gis-agent exec run-workflow --id optimization-01`: 确定性工作流执行
- [ ] **Plan Mode** — 复杂任务分步展示 + 用户确认后执行 (参照 Datus Plan Mode)
- [ ] **Rich 终端渲染** — 表格/进度条/语法高亮 (rich 库)
...
| 标杆能力 | 来源 | v16.0 ✅ | v17.1 ✅ | v18.0 ✅ | v18.5 ✅ | v19.0 🎯 | v20 🎯 | v21+ 🎯 |
|----------|------|-----------|-----------|-----------|-----------|-----------|-----------|-----------|
| 空间数据虚拟化 | SeerAI | 🟢🟢 | 🟢🟢 | 🟢🟢 | 🟢🟢 | 🟢🟢 | 🟢🟢 | 🟢🟢🟢 跨系统 |
...
v19.0: L3+  — + 上下文工程 (统一引擎 + 反馈飞轮 + 语义模型标准化 + 参考查询库) ← Datus 对标
v20.0: L3+  — + 分布式任务队列 (Celery + Redis) + 多 LLM 切换 + 双模式执行 + 轻量部署 ⏸️
...
| v19.0 | 上下文工程 + 反馈飞轮 (Datus) | 3-4 周 | 待排期 | — |
| v20.0 | 分布式队列 + 体验优化 | 3-4 周 | 待 Redis | — |
| v21.0 | 网关 + 追踪 + 血缘 + CLI | 4-5 周 | 待排期 | — |
| v22.0+ | L4 主动式探索 | 待定 | 待定 | 待定 |

**总计**: v19.0 → v21.0 约 10-13 周 (2.5-3 个月)，v19.0 无外部依赖可立即启动
