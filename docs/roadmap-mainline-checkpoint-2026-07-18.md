...
**状态**：Roadmap 战略刷新完成；主线代码开发主动暂停，等待 ArcPy MCP 与 GWM 并行开发稳定。

## 1. 已冻结的最高层产品章程

1. GIS Data Agent 首先是一个优先面向自然资源和城市场景、同时支持空间与非空间多类型数据的 **Geospatial-Native & Agentic-Native Data Platform**。
2. 即使不部署 GWM，平台也必须凭借严格数据治理、数据工程、MMFE、可信数据产品和 Human/Agent/AI 一致消费而独立成立。
...
| 阶段 | 当前定位 |
|---|---|
| NDP-0 | Product Charter、严格治理三维模型、数据类型矩阵、四契约族、PlatformCore/GeoCore/Domain Pack/GWM 边界与资产映射 |
| NDP-1 | Agentic Governance/Cognitive Runtime first slice、治理状态机、Trusted Data Product 与 Human/Agent/AI 消费闭环 |
...
| NDP-4 | Prospective Action/Outcome、现实学习、联邦、互操作、隐私与生态条件路线 |
...
下一窗口应把轻量本体路线更显式地上提到 NDP 主表：

```text
NDP-0  Ontology Charter / Meta-model / GeoCore / Domain / Operational / GWM ontology boundary
NDP-1  PostgreSQL authority + versioned OntologyPackage + Compiler/Resolver
...
NDP-4  RDF/SHACL/GeoSPARQL/federation only when entry gates pass
```
...
3. 先补充 NDP 主表中的轻量本体显式交付，不立即实现 Heavy Ontology。
4. 创建 NDP-0 正式规格：治理能力矩阵、资产/表/API/schema 映射、四契约族和双试点验收集。
5. 第一份代码计划只覆盖 Agentic Governance/Cognitive Runtime first slice，不同时混入 DataProduct、MMFE 和 GWM Kernel 大改。

## 8. 本次提交边界

本 checkpoint 与 `docs/roadmap.md` 是本次主线保存范围。当前工作树中其它 ArcPy、GWM、UWM、Paper58、前端、脚本、报告及测试改动均属于其它窗口或既有工作，不在本次暂存和提交范围内。
