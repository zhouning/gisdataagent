...
同一 ADR 随后用已发布重庆 OSM 道路 `v1.2.0` 完成真实数据面验收。50,366 条道路从受治理 MinIO
GeoJSON 写入隔离 Iceberg v2 full baseline snapshot `4541513947196238885`；第二个 Run 在单次
`MERGE INTO` 中精确执行 1 insert、1 update、1 delete，形成 snapshot
`5267674800802558836`，总行数和唯一道路 ID 仍为 50,366。baseline 与 merge 后 snapshot 均按
snapshot ID time travel 回读，删除、更新和新增状态分别符合预期。

两个真实 provider commit 将 checkpoint 从 0 精确推进到 2。第三个合法 Run 通过
`find_source_slice_commit()` 在 provider 写入前命中 delta SHA-256
`92ad9e56746038a9710bae79494cdde7d50d7ba85a9d57da6631f76ae88fddd8`，未启动第三次写入；跨 Run
commit 恢复返回原 commit，Iceberg history 与 checkpoint 分别保持 2。10 项端到端检查全部通过，
随机 PostgreSQL database、Iceberg table 和 MinIO prefix 已删除，主库 sync 表前后均为 0 行。证据：
