...
ADR-163 将该定义从 ODS 提升为受治理 Silver。认证器把实际 accepted 文件清单和最终状态写成 output
manifest，把 11 项 Flink 行为门写成独立质量 evidence，并把 duplicate/late side output 写成 quarantine
manifest；三份物理文件均计算 SHA-256 并登记为 Run-bound Artifact。Source、Silver target 与 quarantine
分别登记 ResourceVersion，随后记录独立 evaluator 的 passed QualityResult、LineageEvent 和自动
OpenMetadata outbox。`SourceSyncCommitGovernanceEvidence` 与 `SourceSyncQuarantineEvidence` 在提交和
checkpoint 推进的同一事务内绑定。
...
`ce47b6372f2fe5fcf194408b78ac15828273c1d98e8ba8365811b97e2042936a`。
...
Silver output manifest SHA-256 为
`0e30db1c1c5cba5def6130bea4cdfd66b041bcf33bd1c064db2f32fbde5c0148`；quarantine 物理内容
SHA-256 为 `39f2521e73ffe2631cbb886877c5593e7dbacbeb0bc8d66c6b52b1b177253225`。隔离证据精确记录
`records_rejected=2` 和 `{"duplicate":1,"late":1}`，并绑定同一 source slice、Run、target content、
quarantine ResourceVersion 和 Artifact。
...
保持不变。12 项端到端门和 11 项 Flink 行为门全部通过；报告：
...
- 对该 Flink provider，可以声明两条真实 rejected record 已进入物理 quarantine，并与 Silver 的质量、
  血缘、元数据和 SourceSync commit 原子绑定；不能外推到尚未认证的 provider/data kind。
