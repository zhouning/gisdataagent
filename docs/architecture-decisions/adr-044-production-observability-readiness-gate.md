...
attestation 必须绑定调用方提供的当前 source revision、provider versions、backend endpoint/retention 和 runbook version；`evaluate --source-revision` 缺失、格式无效或与 attestation 不一致时必须 fail closed。attestation 还必须记录持续采集、持久存储、历史查询、TLS、workload identity、tenant isolation、dashboard、告警触发通知、恢复通知、DataSLO 评估和 runbook 响应均为 `passed`。观测时间不得早于验证时刻 24 小时，expiry 必须在未来且最长七天，证据 URI 必须为非本机 HTTPS。配置变更、source revision 变化、provider 版本变化、过期或任一检查失败都会关闭门禁。
...
定向测试覆盖 pending profile、完整 profile/缺 attestation、完整且新鲜的绑定 attestation、source revision 不匹配、placeholder、HTTP/loopback endpoint、缺 owner/SLO/runbook、敏感字段、self-asserted claim、过期证明、profile/provider/backend/runbook 漂移、恢复通知失败、报告篡改和 malformed YAML。
