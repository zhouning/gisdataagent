...
直白地说：V5的数据层已经完成，不需要再等下载。四次行动的数据都已做成统一周表，四个轮流留出考试包
也已生成，并通过147项独立检查。运行规则、四折提交格式和评分器也已在模型运行前冻结；下一步只允许
按这些规则实现运行器并生成预测，不能再根据预测结果修改门槛。
...
- 已完成：V5问题定义、四行动协议、源数据预检、RC1数据物化、四折隔离、独立数据验证；
- 已冻结：Runtime-R4合同、四折提交合同、八项行动迁移门槛和正式评分器；
- 尚未完成：Runtime-R4模型运行器、正式模型与七项负对照、预测承诺与重放、最终评分报告；
- 当前允许做的下一步：按冻结合同实现运行器，随后运行全部规定模型、控制、折和种子；
- 当前禁止做的事：模型进程读取当前折答案、根据正式预测修改评分器或门槛，或把回顾性测试宣传成盲测和因果评估。
