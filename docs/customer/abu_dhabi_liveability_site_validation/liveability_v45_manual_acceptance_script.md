...
1. 页面要求登录；使用具有 `admin` 或 `standard_editor` 角色的账号进入。未登录或只读角色不得看到草稿编辑能力。
2. 在“行业本体”下拉框选择 `abu-dhabi-dmt-gis`（DMT），确认标题、领域、概念和关系不再固定为自然资源本体。
3. 切换“草稿编辑”，创建一个临时草稿；修改一个概念标签或属性，确认操作进入草稿而不是直接修改已发布模型。
4. 运行“校验”和“差异”，确认可以看到校验结果和变更摘要；不要在共享环境点击“提交审核”，除非现场明确批准。
5. 回到工作台后，打开“数据” → “语义层” → “业务模型”，确认语义版本和本体版本与预检输出一致。

截图或记录：DMT 页面标题、选中的 `ontology_key`、语义版本、本体版本、概念/关系数量和角色权限表现。
...
6. 若旧浏览器缓存或旧前端 bundle 请求 `/api/semantic/governance/metric-contracts`，确认指标列表仍能加载；服务端会将连字符别名归一化为 `metric_contracts`，但非法 `scope` 仍返回 400。
...
若页面仍显示“需要澄清，未执行 SQL”，记录响应中的 `governance reason`。修复后的运行时应在 admission 中显示 `reviewed_metric_contract_admitted` 或已发布候选集合准入，并在最终报告中显示合同 `LIVEABILITY_FACILITY_COUNT_BY_STAGE_TYPE_V4`；不得通过前端输入固定 SQL 绕过准入。
