...
### 4.8 Metadata Fabric Bridge M1 + M2 + M3-2（本地授权 ingestion 与零写入 replay 已验证）
...
此处 M1 只证明静态合同和只读 HTTP 边界；M2a 只证明本地 live foundation 与 PVC 重挂载连续性；M2b-1/M2b-2 分别限定在同集群新 PVC 和同集群隔离 repository；M2b-3 的 `local_cross_cluster_recovery_verified=true` 只限定在 `local_same_host_distinct_kubernetes_clusters_external_s3_repository`；M2c-1/M2c-2/M2c-3 分别限定本地 provider metrics、临时双周期 OTel 和单 job scrape recovery；M2c-4/M2d-2 只证明 production observability/NetworkPolicy profile 与 attestation 合同可校验；M2d-1 只证明本地两节点 kindnet 的隔离合成流量；M3-1 的 terminal evidence 与 M3-2 的 PolicyDecision/Approval 仍是 deterministic local fixtures。M3-2 只把 projection 写入本地 provider，并证明 retained target 的单次零写入 replay；OpenMetadata 使用 bootstrap admin，Gravitino 未认证且 catalog backend 为 memory，restart persistence 未验证。binding candidate、OpenLineage candidate、ResourceVersion 和任何 GDA/legacy authority 都未写入。生产最小权限、OIDC、持久 catalog、TLS、tenant isolation、真实 alert/SLO、受保护 provider policy、live OpenLineage、生产 ingest/conformance、两项 production gate 和 `production_ready` 仍为 `false`。
