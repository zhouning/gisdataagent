...
### 4.7 authority、mainline 与受保护 staging 发布闭环（golden slice/worker 待接入）
...
当前完成包括本地合同、授权 evidence、outbox/callback 代码、数据库成功终局门、托管 worker 代码、默认关闭的部署模板、candidate/registry/provenance/release/live observation evidence gate、真实 GHCR subject 与 attestation、受保护 staging apply/observe、worker readiness blocked evidence、合成 golden slice、真实 PostgreSQL 16 事务边界和 canonical mainline 治理。真实 IAM/OIDC 与 service token 生命周期、worker/callback 单副本运行、live golden slice、独立 DolphinScheduler metadata PostgreSQL、真实 Artifact/Quality/Lineage 终局证据及 production promotion 仍未完成；`staging_deployed=true` 不改变这些退出门。
