...
### 4.8 Metadata Fabric Bridge M1 + M2a + M2b + M2c-2（本地临时 OTel pipeline 已验证）
...
此处 M1 只证明静态合同和只读 HTTP 边界；M2a 只证明本地 live foundation 与 PVC 重挂载连续性；M2b-1/M2b-2 分别限定在同集群新 PVC 和同集群隔离 repository；M2b-3 的 `local_cross_cluster_recovery_verified=true` 只限定在 `local_same_host_distinct_kubernetes_clusters_external_s3_repository`；M2c-1 的 `local_provider_metrics_verified=true` 只限定在本地 provider-native endpoint；M2c-2 的 `local_otel_metrics_pipeline_verified=true` 只限定在运行后即清理的本地双周期 pipeline。它们没有写入或连接真实 GDA Resource，通用 `otel_pipeline_verified`、`production_metrics_verified`、持久存储、TLS、tenant isolation、alert、SLO、NetworkPolicy enforcement、OIDC、upgrade、GDA write 和 `production_ready` 仍为 `false`。source host/cluster 外的生产 recovery、持续 metrics backend/OTel/alert/SLO 及其后的升级/registry/M3 仍是退出门。
