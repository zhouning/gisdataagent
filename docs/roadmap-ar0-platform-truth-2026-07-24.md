...
### 4.8 Metadata Fabric Bridge M1 + M2 + M3-1（确定性 ingestion projection 已建立，provider apply 仍 blocked）
...
此处 M1 只证明静态合同和只读 HTTP 边界；M2a 只证明本地 live foundation 与 PVC 重挂载连续性；M2b-1/M2b-2 分别限定在同集群新 PVC 和同集群隔离 repository；M2b-3 的 `local_cross_cluster_recovery_verified=true` 只限定在 `local_same_host_distinct_kubernetes_clusters_external_s3_repository`；M2c-1/M2c-2/M2c-3 分别限定本地 provider metrics、临时双周期 OTel 和单 job scrape recovery；M2c-4/M2d-2 只证明 production observability/NetworkPolicy profile 与 attestation 合同可校验；M2d-1 只证明本地两节点 kindnet 的隔离合成流量；M3-1 只证明 synthetic terminal evidence 能确定性生成 provider projection 与 OpenLineage candidate，并且一致 replay 为 no-op。pending profile、合成 attestation、projection plan 和 OpenLineage candidate 都不是生产运行证据。它们没有写入真实 GDA Resource 或 provider，通用 `otel_pipeline_verified`、`production_metrics_verified`、持久存储、TLS、tenant isolation、alert delivery、SLO、生产 NetworkPolicy enforcement、provider policy、OIDC、upgrade、live ingestion/OpenLineage、GDA write、两项 production gate 和 `production_ready` 仍为 `false`。source host/cluster 外的生产 recovery、持续 metrics backend/真实 alert/SLO、受保护环境 policy/tenant isolation，以及带授权的 provider apply、升级/registry/conformance 仍是退出门。
