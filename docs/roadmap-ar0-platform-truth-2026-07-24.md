...
### 4.7 authority、mainline 与受保护 staging 发布闭环（worker 激活边界已实现、运行待审批）
...
3. 已以 tenant-scoped PostgreSQL command outbox 和 authenticated callback 建立耐久 dispatch/reconcile 触发：Run+dispatch、callback observation+reconcile 分别同事务提交，claim 使用 lease/`SKIP LOCKED`，薄 consumer 只调用 adapter；新增 tenant-scoped managed worker（见 [ADR-027](architecture-decisions/adr-027-managed-dolphinscheduler-command-worker.md)），负责严格配置、0600 token 文件、优雅退出、可中断轮询和脱敏 health projection；Kustomize base 已登记默认零副本的 Deployment，并以 CI validator 固定独立 Secret、Pod UID、探针和 NetworkPolicy；staging activation preflight 与 protected readiness workflow 进一步固定单副本首发、release-bound immutable digest、HTTPS provider、ConfigMap fingerprint、新鲜 Secret key attestation、只读 live drift 检查及 no-scale/promotion boundary。受保护 run `31872815368` 已验证 release attestation、observer 最小权限、单副本 manifest render、blocked evidence attestation/upload 和最终 no-scale boundary；[ADR-075](architecture-decisions/adr-075-protected-dolphinscheduler-worker-activation.md) 又建立独立手工 activation workflow，在加载 mutation kubeconfig 前验证 readiness run/artifact digest、逐文件 attestation、release provenance、manifest 白名单和 protected cluster/namespace UID，只允许一次 server-side 单副本 apply，并产出 attested live-readiness evidence。该 workflow 尚未执行；当前精确阻塞仍为外部 ConfigMap snapshot/Secret attestation、真实 provider identity、environment reviewer approval 及 worker Deployment 未部署，仍未配置真实 provider callback 或验证告警 SLO；
...
12. canonical mainline、首次 publisher/verifier、attested release apply 和 live staging observation 已完成；后续仍须在同一 staging 场景跑通 DolphinScheduler 独立 metadata PostgreSQL、托管 outbox worker/callback 和真实 Artifact/Quality/Lineage 终局裁决。worker readiness run `31872815368` 已产出 fingerprint `1babf26cadfe39da364a1505d39cddf44d7e04abd38b32babbc0eadbece1ebce` 的 attested blocked evidence；单副本受控激活的代码、admission 和 CI 合同已经实现，但没有触发 staging mutation。环境须先提供真实 ConfigMap snapshot、Secret key attestation、provider identity 和 reviewer approval，再执行该手工入口并验证唯一 worker ID、status projection、lease 接管和重启 drain；
...
此处 M1 只证明静态合同和只读 HTTP 边界；M2a 只证明本地 live foundation 与 PVC 重挂载连续性；M2b-1/M2b-2 分别限定在同集群新 PVC 和同集群隔离 repository；M2b-3 的 `local_cross_cluster_recovery_verified=true` 只限定在 `local_same_host_distinct_kubernetes_clusters_external_s3_repository`；M2c-1 的 `local_provider_metrics_verified=true` 只限定在本地 provider-native endpoint；M2c-2 的 `local_otel_metrics_pipeline_verified=true` 只限定在运行后即清理的本地双周期 pipeline；M2c-3 的 `local_otel_scrape_failure_recovery_verified=true` 只限定在临时 Collector 的单 job 配置故障与恢复；M2c-4 只证明 production observability 合同可校验；M2d-1 的 `local_network_policy_enforcement_verified=true` 只限定在本地两节点 kindnet 的隔离合成流量；M2d-2 只证明 production NetworkPolicy profile/attestation 合同可校验。两类 pending profile 和合成 attestation 都不是生产运行证据。它们没有写入或连接真实 GDA Resource，通用 `otel_pipeline_verified`、`production_metrics_verified`、持久存储、TLS、tenant isolation、alert delivery、SLO、生产 NetworkPolicy enforcement、provider policy、OIDC、upgrade、GDA write、`production_observability_gate_passed`、`production_network_policy_gate_passed` 和 `production_ready` 仍为 `false`。source host/cluster 外的生产 recovery、持续 metrics backend/OTel/真实 alert/SLO、受保护环境 provider policy/tenant isolation 及其后的升级/registry/M3 仍是退出门。
