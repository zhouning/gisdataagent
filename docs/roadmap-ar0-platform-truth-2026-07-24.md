...
### 4.8 Metadata Fabric Bridge M1 + M2 + M3-21（本地 JDBC/S3 runtime-bound promotion 已验证，生产验证待执行）
...
M3-21 只证明本地同主机双节点上的 JDBC/S3 provider identity、空表 Iceberg metadata 与 restart continuity；它不替代真实 feature-row ingestion、生产对象存储 attestation、独立 failure domain、KMS/TLS/workload identity、备份/PITR、tenant isolation 或权威 promotion ledger。
