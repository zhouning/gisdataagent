...
| 环境发布与晋级 | candidate/registry evidence 绑定 publisher source、本地 image ID 和 GHCR digest；独立 verifier 从自身 revision 执行固定证书策略并 attested 结果；`staging_release_evidence` 再验证 provenance artifact 的 verifier workflow/revision/digest，只从其中取得 image 并绑定 manifest；live collector 约束运行 revision。GitHub 登录与仓库权限已恢复，但新 workflows 未 push、environment 未配置、没有真实 subject/artifact | CI artifact、publisher action 成功、candidate/registry/provenance JSON、离线 activation/bundle report、合成 `verified_for_staging_apply` 和未 attested live observation 都不能单独晋级；publisher SHA 与 verifier SHA 不能混为一个 authority | 受保护 environment 的 DeploymentRevision 绑定已独立验证的 OCI、provenance evidence artifact、release manifest 与全部 live verdict，再由同 revision production approval 成为 promotion authority | Platform/SRE/Security | AR-1 candidate + registry + protected verifier + attested release + live gate 本地已验证 -> 首次 GHCR publish/verify 与真实 staging 待接入 |
