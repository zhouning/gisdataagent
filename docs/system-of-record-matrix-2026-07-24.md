...
| 环境发布与晋级 | `staging_candidate_evidence` 绑定临时 CI candidate；`staging_live_evidence` 以白名单方式采集 cluster/namespace/Deployment/Pod/EndpointSlice、registry digest、source/candidate/platform fingerprint、live schema/config/runtime、health 和 golden-slice，并对每一域 fail closed；旧假部署语义已移除 | CI artifact、candidate summary、离线 activation report、未证明 provenance 的 live observation JSON 都不能单独晋级 | 受保护环境的 DeploymentRevision 绑定全部 live verdict，并由受保护 runner identity 与 artifact attestation 证明采集来源后，才可成为 promotion authority | Platform/SRE/Security | AR-1 candidate + live observation gate 本地已验证 -> 真实 staging/provenance 待接入 |
