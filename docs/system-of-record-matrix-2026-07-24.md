...
| 环境发布与晋级 | `staging_candidate_evidence` 绑定临时 CI candidate；`staging_deployment_bundle` 将 candidate、预期 platform fingerprint 和 registry digest 结构化绑定为 Secret-free release manifest；`staging_live_evidence` 以白名单方式采集 cluster/namespace/Deployment/Pod/EndpointSlice、registry digest、source/candidate/platform fingerprint、live schema/config/runtime、health 和 golden-slice，并对每一域 fail closed；旧假部署语义已移除 | CI artifact、candidate summary、离线 activation/bundle report、未证明 provenance 的 live observation JSON 都不能单独晋级 | 受保护环境的 DeploymentRevision 绑定全部 live verdict，并由受保护 runner identity 与 artifact attestation 证明采集来源后，才可成为 promotion authority | Platform/SRE/Security | AR-1 candidate + bundle + live observation gate 本地已验证 -> 真实 registry/staging/provenance 待接入 |
...
13. `candidate_validated`、CI artifact、离线 preflight、未 attested live observation JSON 或人工批准都不能单独授权 production；缺少同一 source revision 的 registry/live revision/identity/health/golden-slice 绑定及受保护 provenance 时，promotion 必须失败。
