...
阶段：AR-0 `in_progress`；AR-1 gateway、成功终局 evidence gate、DolphinScheduler adapter sandbox POC、Metadata Fabric M1/M2a/M2b、M2c-1 provider metrics、M2c-2 本地临时 OTel pipeline 与 M2c-3 本地 scrape failure/recovery 已验证，生产切换仍 `in_progress`

适用分支：`feat/ar1-metadata-fabric-otel-failure-rehearsal`
...
| 后台运行时清单 | `platform_truth.RUNTIME_INVENTORY` 是代码层登记；`gda_control` 已有受控 PlatformRun 写入口；DolphinScheduler managed worker 已登记但尚无生产调用方；M2b recovery runner、M2c-1 provider probe、M2c-2 `_OtelPortForward` 与 M2c-3 failure rehearsal 均登记为 `local_verification_only`，不是 scheduler、worker、持续监控或状态权威 | AST primitive report、worker status JSON、FrameworkAttemptObservation、DolphinScheduler instance state、本地 recovery/metrics evidence | PlatformRun ledger 唯一登记最终状态；framework/provider attempt 只能回报观测；本地演练进程与 evidence 不得变成生产控制器或监控后端 | Platform Architecture | AR-1 adapter/worker 本地已验证；metadata recovery/metrics runner 仅本地验证 -> staging 控制链待接入 |
...
| 技术元数据 | M1 已冻结 Gravitino table ref/reconciliation；M2a 已运行 Gravitino `1.3.0` + 独立 PostgreSQL；M2b 已完成本地新 PVC、锁定 repository 与独立 kind cluster 恢复；M2c-1 已验证原生指标，M2c-2 已验证 JSON Exporter -> OTel 的本地双周期抓取，M2c-3 已验证隔离的 Gravitino scrape failure detection/recovery，但未创建 production catalog、持续 metrics backend 或真实告警 | harvester 结果、合成 Gravitino response、本地 sandbox/recovery/repository/metrics observation | 源系统技术对象是原始证据；Gravitino 映射并联邦，不能覆盖业务 ResourceVersion；backup object 与 metrics observation 都不是技术对象权威 | Metadata Platform | AR-1 M1 + 本地 M2a/M2b/M2c 已验证 -> 外部生产恢复/持续 metrics/M3 conformance 待执行 |
| 治理目录 | M1 已冻结 OpenMetadata table ref/reconciliation；M2a 已运行 OpenMetadata `1.13.1` + 独立 PostgreSQL/OpenSearch；M2b 已完成本地新 PVC、锁定 repository 与独立 kind cluster 恢复；M2c-1 已验证原生指标，M2c-2 已验证 OpenMetadata -> OTel 本地双周期抓取，M2c-3 已证明 Gravitino scrape 故障时 OpenMetadata 保持 `up=1` 并可恢复，但未接入真实 GDA Resource、持续 metrics backend 或真实告警 | 搜索/页面视图、合成 OpenMetadata response、本地 sandbox/recovery/repository/metrics observation | OpenMetadata 为 owner/glossary/classification/quality discoverability 权威；GDA ledger 保留审批证据；backup repository 和 metrics observation 都不是目录或资源版本权威 | Governance | AR-1 M1 + 本地 M2a/M2b/M2c 已验证 -> 外部生产恢复/持续 metrics/M3 ingestion 待执行 |
...
14. Metadata Fabric M1 只允许 OpenMetadata/Gravitino GET；M2a/M2b 只运行本地 foundation/recovery/repository；M2c-1 只读取 provider-native metrics；M2c-2 只运行结束即删除的 OTel + JSON Exporter 双周期 pipeline；M2c-3 只修改临时 Collector 的 Gravitino scrape address 并验证检测/恢复/cleanup；全部 `writes_to_gda_enabled=false`。M2c-2 的 `local_otel_metrics_pipeline_verified=true` 与 M2c-3 的 `local_otel_scrape_failure_recovery_verified=true` 都不等于通用/生产 OTel、持久存储、TLS、tenant isolation、alert delivery、SLO 或 NetworkPolicy enforcement。上述 evidence 都不等于 production provider、外部生产 backup target、source-host loss、生产跨集群/跨区域恢复、OIDC、upgrade 或写权威。
...
- OpenMetadata/Gravitino 的 source host/cluster 外生产 backup account/bucket、KMS/TLS/workload identity、PITR/source-loss recovery、RPO/RTO、OIDC、NetworkPolicy enforcement、upgrade/rollback、registry provenance、持续 metrics backend/retention/query、真实 alert delivery/SLO owner/runbook，以及 M3 ingestion/OpenLineage/无双写/conformance；M1 fixture、M2a PVC、M2b 本地 recovery、M2c-1 point-in-time metrics、M2c-2 本地临时 OTel 与 M2c-3 本地 scrape failure/recovery evidence 均不计入生产退出门；
