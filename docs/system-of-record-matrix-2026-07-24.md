...
阶段：AR-0 `in_progress`；AR-1 gateway、成功终局 evidence gate、DolphinScheduler adapter sandbox POC、Metadata Fabric M1/M2a/M2b 与 M2c-1 本地 provider metrics 已验证，生产切换仍 `in_progress`

适用分支：`feat/ar1-metadata-fabric-provider-metrics`
...
| 技术元数据 | M1 已冻结 Gravitino table ref/reconciliation；M2a 已运行 Gravitino `1.3.0` + 独立 PostgreSQL；M2b 已完成本地新 PVC、锁定 repository 与独立 kind cluster 恢复；M2c-1 已验证 Gravitino datasource/HTTP/JVM 原生指标，但未创建 production catalog 或持续 metrics pipeline | harvester 结果、合成 Gravitino response、本地 sandbox/recovery/repository/metrics observation | 源系统技术对象是原始证据；Gravitino 映射并联邦，不能覆盖业务 ResourceVersion；backup object 与 metrics observation 都不是技术对象权威 | Metadata Platform | AR-1 M1 + 本地 M2a/M2b/M2c-1 已验证 -> 外部生产恢复/metrics pipeline/M3 conformance 待执行 |
| 治理目录 | M1 已冻结 OpenMetadata table ref/reconciliation；M2a 已运行 OpenMetadata `1.13.1` + 独立 PostgreSQL/OpenSearch；M2b 已完成本地新 PVC、锁定 repository 与独立 kind cluster 恢复；M2c-1 已验证 OpenMetadata health/database/HTTP/JVM 原生指标，但未接入真实 GDA Resource 或持续 metrics pipeline | 搜索/页面视图、合成 OpenMetadata response、本地 sandbox/recovery/repository/metrics observation | OpenMetadata 为 owner/glossary/classification/quality discoverability 权威；GDA ledger 保留审批证据；backup repository 和 metrics observation 都不是目录或资源版本权威 | Governance | AR-1 M1 + 本地 M2a/M2b/M2c-1 已验证 -> 外部生产恢复/metrics pipeline/M3 ingestion 待执行 |
...
14. Metadata Fabric M1 只允许 OpenMetadata/Gravitino GET；M2a/M2b 只运行本地 foundation/recovery/repository，M2c-1 只读取本地 provider-native metrics，且全部 `writes_to_gda_enabled=false`。M2b-3 的 `local_cross_cluster_recovery_verified=true` 只适用于同一 Docker Desktop 主机内的两个 Kubernetes cluster 与 Docker-host repository；M2c-1 的 `local_provider_metrics_verified=true` 不等于持续 scrape、production metrics、OTel、TLS、alert 或 SLO。上述 evidence 都不等于 production provider、外部生产 backup target、生产 retention、TLS/KMS/workload identity、source-host loss、生产跨集群/跨区域恢复、OIDC、upgrade 或写权威。
...
- OpenMetadata/Gravitino 的 source host/cluster 外生产 backup account/bucket、KMS/TLS/workload identity、PITR/source-loss recovery、RPO/RTO、OIDC、NetworkPolicy enforcement、upgrade/rollback、registry provenance、持续 metrics/OTel、alert/SLO owner/runbook，以及 M3 ingestion/OpenLineage/无双写/conformance；M1 fixture、M2a PVC、M2b-1/M2b-2 同集群 evidence、M2b-3 同主机双集群 evidence 与 M2c-1 point-in-time metrics evidence 均不计入生产退出门；
