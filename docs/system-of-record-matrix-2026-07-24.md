...
阶段：AR-0 `in_progress`；AR-1 gateway、成功终局 evidence gate、DolphinScheduler adapter sandbox POC、Metadata Fabric M1/M2a/M2b、M2c-1 provider metrics 与 M2c-2 本地临时 OTel pipeline 已验证，生产切换仍 `in_progress`

适用分支：`feat/ar1-metadata-fabric-otel-metrics-pipeline`
...
14. Metadata Fabric M1 只允许 OpenMetadata/Gravitino GET；M2a/M2b 只运行本地 foundation/recovery/repository；M2c-1 只读取 provider-native metrics；M2c-2 只运行结束即删除的 OTel + JSON Exporter 双周期 pipeline；全部 `writes_to_gda_enabled=false`。M2c-2 的 `local_otel_metrics_pipeline_verified=true` 不等于通用/生产 OTel、持久存储、TLS、tenant isolation、alert、SLO 或 NetworkPolicy enforcement。上述 evidence 都不等于 production provider、外部生产 backup target、source-host loss、生产跨集群/跨区域恢复、OIDC、upgrade 或写权威。
...
- OpenMetadata/Gravitino 的 source host/cluster 外生产 backup account/bucket、KMS/TLS/workload identity、PITR/source-loss recovery、RPO/RTO、OIDC、NetworkPolicy enforcement、upgrade/rollback、registry provenance、持续 metrics backend/retention/query、alert/SLO owner/runbook，以及 M3 ingestion/OpenLineage/无双写/conformance；M1 fixture、M2a PVC、M2b 本地 recovery、M2c-1 point-in-time metrics 与 M2c-2 本地临时 OTel evidence 均不计入生产退出门；
