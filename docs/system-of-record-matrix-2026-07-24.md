...
| 环境发布与晋级 | `staging_candidate_evidence` 绑定临时 CI candidate；`staging_registry_evidence` 将同一 source/candidate/local image ID 与 GHCR manifest digest 绑定，publish workflow 以 GitHub OIDC 为该 subject 请求 provenance；`staging_deployment_bundle` 将 subject 与预期 platform fingerprint 结构化绑定为 Secret-free release manifest；`staging_live_evidence` 以白名单方式验证 live revision；旧假部署语义已移除。当前 GitHub 身份失效，publish workflow 尚未真实运行 | CI artifact、candidate/registry summary、OIDC attestation 请求、离线 activation/bundle report、未独立证明 provenance 的 live observation JSON 都不能单独晋级 | 受保护环境的 DeploymentRevision 绑定全部 live verdict，并由独立 protected runner 验证 OCI subject、repository/workflow identity 与 evidence artifact provenance 后，才可成为 promotion authority | Platform/SRE/Security | AR-1 candidate + registry + bundle + live gate 本地已验证 -> 首次 GHCR publish、protected verification 与真实 staging 待接入 |
...
- GHCR publication contract 已固定单次 application image build、OCI revision/source label、远端 raw manifest `sha256`、candidate-to-subject binding 和 GitHub OIDC provenance；registry report 固定不自证 attestation/digest/deployment/promotion。公开 action/Buildx 契约已通过本机代理在线复核，但 GitHub 登录仍失效，尚无真实 published subject、attestation 或 registry artifact。
...
- 恢复 GitHub/GHCR 身份后的首次 application subject publish，以及 protected runner 对 OCI attestation repository/workflow identity 的独立验证；
- 真实 staging/production 的 schema、config 和 runtime snapshot 产物、registry/live DeploymentRevision 绑定、受保护 runner provenance/artifact attestation 及环境 compare 报告；
