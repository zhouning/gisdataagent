...
| 环境发布与晋级 | candidate/registry evidence 绑定 source、本地 image ID 和 GHCR digest；publisher 请求 GitHub OIDC provenance；独立 `staging_provenance_evidence` 以证书扩展固定 repository、workflow/ref/digest、issuer 和 hosted runner，并 attested 验证结果；release bundle/live collector 分别约束 apply 输入和运行 revision。GitHub 登录与仓库权限已恢复，但新 workflows 未 push、environment 未配置、没有真实 subject | CI artifact、publisher action 成功、candidate/registry/provenance JSON、离线 activation/bundle report 和未 attested live observation 都不能单独晋级；本地合成 `provenance_verified` 也不是远端事实 | 受保护 environment 的 DeploymentRevision 绑定已独立验证的 OCI/provenance evidence artifact 与全部 live verdict，再由同 revision production approval 成为 promotion authority | Platform/SRE/Security | AR-1 candidate + registry + protected verifier + bundle + live gate 本地已验证 -> 首次 GHCR publish/verify 与真实 staging 待接入 |
...
- GHCR publication contract 已固定单次 application image build、OCI revision/source label、远端 raw manifest `sha256`、candidate-to-subject binding 和 GitHub OIDC provenance；独立 verifier 已固定证书 repository/workflow/ref/digest/issuer/runner 策略并对验证 evidence 再 attested。GitHub 登录、仓库 admin/push 和 Actions 权限已恢复验证，但分支未 push、protected environment 未配置，尚无真实 published/verified subject 或 provenance artifact。
