...
FAMILY = "family12_gemma4_26b_host228_productized"
LLM_MODEL_ID = "Gemma4:26b"
...
def configure_environment() -> None:
    ...
    os.environ["BASELINE_HARD_TIMEOUT"] = "300"
...
async def run_mode(run_cq_eval, questions: list[dict], mode: str, out_dir: Path) -> dict:
    ...
    for i, q in enumerate(questions[len(records):], len(records) + 1):
        t0 = time.time()
        try:
            rec = await asyncio.wait_for(run_cq_eval.run_one(q, mode), timeout=780)
