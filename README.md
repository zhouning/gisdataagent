...
基于 **Google Agent Developer Kit (ADK) v1.27.2** 构建的 AI 驱动**时空数据治理平台（Agentic Spatial Data Governance Platform）**。通过多语言语义路由（中/英/日），自动调度三大专业管道完成空间数据治理、用地优化和通用空间智能分析，并以**数据标准全生命周期闭环**为骨干，把数据治理从"用户带数据来"升级为"Agent 主动发现、治理、编排和运营数据"。
...
- **Standards Platform Wave 1 → Wave 8 + Wave 6-eval**：数据标准从 Word 文档升级为全生命周期可治理资产（采集 / 起草 / 审定 / 发布 / 派生 / 数据建模六阶段闭环）。16 张 `std_*` 表 + Outbox 独立 worker + ltree + pgvector(768) + StandardsTab 双 sub-tab + StandardsEditorAgent；6 大派生策略已全部落地（SemanticHint / ValueDomain / Synonym / QcRule / DefectTaxonomy / **DataModel**），**派生覆盖率 100% (6/6)**，并新增 deterministic 派生质量评测底座（gold/prediction schema、P/R/F1、JSON/Markdown 报告、CLI）。新增派生回滚 + 影响图谱 API（`POST /api/std/derive/rollback/{vid}` + `GET /api/std/impact/{kind}/{id}`）。Migrations 067-085 全部落地，326 standards_platform tests passed。
...
| 指标 | 数值 |
|------|------|
| 测试覆盖 | 5400+ tests, 241 test files |
...
| DB 迁移 | 85 个 SQL 迁移（最新 085 — `std_data_model_snapshot`） |
| Standards Platform | 16 张 `std_*` 表 + Outbox 独立 worker + ltree + pgvector(768) + 6 大派生策略（已落地 6/6: SemanticHint / ValueDomain / Synonym / QcRule / DefectTaxonomy / DataModel）+ 派生质量评测 CLI + 派生回滚 + 影响图谱 + StandardsEditorAgent |
